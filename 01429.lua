...
function Lua_AP_Ocean_callBoss()
	local RoomID = ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)
	--local Boss = OwnerID()
	--CastSpell( OwnerID() , OwnerID() , 493492) --只有特效，沒有效果的法術
	sleep(15) --停頓 1.5 秒後才將分身叫出
	local Boss_other = LuaFunc_CreateObjByObj ( 102198, OwnerID() )  --將分身叫出
...
function Lua_AP_jail()
	while 1 do		
		...
		while 1 do	
			sleep(10)
			SetPlot( OwnerID(),"dead","LuaS_AP_jail_2",10 )   --死亡劇情
			local AttackTarget = ReadRoleValue( OwnerID() , EM_RoleValue_AttackTargetID )
...
------------隱形飛劍 -----------------------------	
function Lua_AP_sword_1()
	local RoomID = ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)
	local sword = LuaFunc_CreateObjByObj ( 102199, OwnerID() )  --將隱形的怪物叫出來
		...
		while 1 do
			...
			if  ( AttackTarget ~= 0 ) and ( CheckID( OwnerID() ) ) == true then					
				...
				if counter2 == 25 then
					...
					sleep(10)
					CastSpell( OwnerID() , OwnerID() , 493492) 	 ----嘎巴秘術						
					sleep(10)
					counter2 = 0
				end
				if 	counter3 == 28 then
				...
				if counter5 == 7 then
					...
					CastSpell( OwnerID() , partymember[anyone] , 493499) 	 ----嘎巴漩渦					
					sleep(10)							
					counter5 = 0
				end
			else
