...
--金剛站點：/GM ? Zone 139 0 354.1 1588.9 -705.2
--780911--Flag1：大金剛
...
--※ 控制器 ※--
function sasa_105458_Counter()
	local Entrance = 0
	local EnterFight = 0	
	local Mode = 0												------判斷模式用
	local Room = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	local Boss = CreateObjByFlag( 105458 , 780911 , 1 , 1 )
	AddToPartition( Boss , Room )
	BeginPlot( Boss , "sasa_105458_AI" , 0 )
	Setplot( Boss , "dead" , "sasa_105458_Dead" , 30 )
	...
	local Car01 = CreateObjByFlag( 105665 , 780911 , 11 , 1 )
	...
	Setplot( Car03 , "dead" , "sasa_105665_Broken03" , 30 )
			...
			if EnterFight == 1 then
				if GGCounter >= 15 then
					...
					for d = 1 , 1 do
						...
						local GGP = CreateObjByFlag( 105462 , 780911 , flagNum1 , 1 )	--物理
						GG01[table.getn( GG01 )+1] = GGP
						AddToPartition( GGP , Room )
						BeginPlot( GGP , "sasa_105462_Run" , 0 )
						--Say( GGP , "Yes,Sir!" )
						Sleep(1)
						local GGM = CreateObjByFlag( 105463 , 780911 , flagNum2 , 1 )	--法術
						GG02[table.getn( GG02 )+1] = GGM
						AddToPartition( GGM , Room )
						BeginPlot( GGM , "sasa_105463_Run" , 0 )
						--Say( GGM , "Yes,Boss!" )
					end
					GGCounter = 0
				end
...
--※ 重置死亡 ※--
			...
			if EnterFight == 1 then
				...
				if BossIsDead ~= 1 then
					...
					BeginPlot( OwnerID() , "sasa_105458_CancelBuff" , 0 )		--清除BUFF
					sleep(50)
					...
					sleep(20)
					SetModeEx( Entrance , EM_SetModeType_Obstruct , false )		------阻擋(為了打開狀態所以不勾)
					Boss = CreateObjByFlag( 105458 , 780911 , 1 , 1 )
					AddToPartition( Boss , Room )
					BeginPlot( Boss , "sasa_105458_AI" , 0 )
					Setplot( Boss , "dead" , "sasa_105458_Dead" , 30 )
					...
					Car01 = CreateObjByFlag( 105665 , 780911 , 11 , 1 )
					SetModeEx( Car01 , EM_SetModeType_Move , false )			------不移動
					SetModeEx( Car01 , EM_SetModeType_Strikback , false )		------不反擊
					SetModeEx( Car01 , EM_SetModeType_Fight , false )			------不可砍殺
					SetModeEx( Car01 , EM_SetModeType_ShowRoleHead, false )		--不秀頭像框
					...
					Car02 = CreateObjByFlag( 105665 , 780911 , 12 , 1 )
					SetModeEx( Car02 , EM_SetModeType_Move , false )			------不移動
					SetModeEx( Car02 , EM_SetModeType_Strikback , false )		------不反擊
					SetModeEx( Car02 , EM_SetModeType_Fight , false )			------不可砍殺
					SetModeEx( Car02 , EM_SetModeType_ShowRoleHead, false )		--不秀頭像框
					...
					Car03 = CreateObjByFlag( 105665 , 780911 , 13 , 1 )
					SetModeEx( Car03 , EM_SetModeType_Move , false )			------不移動
					SetModeEx( Car03 , EM_SetModeType_Strikback , false )		------不反擊
					SetModeEx( Car03 , EM_SetModeType_Fight , false )			------不可砍殺
					SetModeEx( Car03 , EM_SetModeType_ShowRoleHead, false )		--不秀頭像框
				...
				elseif BallCounter >= 7 then
					...
					for b = 1 , table.getn(PlayerInHate1) do
						...
						if ReadRoleValue( temp01 , EM_RoleValue_Voc ) ~= 0 					--GM排除
						 ...
						 and ReadRoleValue( temp01 , EM_RoleValue_OrgID ) ~= 105665 then	--車子排除
					...
					for c = 1 , 1 do
						...
						if ( LuckyU[Luckynum] ~= nil )
						 and ( ReadRoleValue( LuckyU[Luckynum] , EM_RoleValue_IsDead ) == 0 ) then
							...
							CastSpellLv( OwnerID() , BossTarget , FireBall , 34 )	--燃炎爆彈
							Sleep(20)
...
function sasa_105462_Run()
	while 1 do
		...
		sleep(10)
			...
			for i = 1 , CarBox do
				if ReadRoleValue( Car[i] , EM_RoleValue_OrgID ) == 105665 then
					--Say( OwnerID() , "GO~~~" )
					SetAttack( OwnerID() , Car[i] )
					PhyTarget = Car[i]
					CastSpellLV( OwnerID() , PhyTarget , 497222 , 11 )		--衝擊
				end
			end
			Sleep(50)
		end
...
--※ 火藥桶滾滾 ※--
function sasa_105458_RunBox()
	local Room = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	for a = 1 , 3 do
		local BoxFlag = { 4 , 5 , 6 }
		local FlagNum = Rand(3)+1
		local BoxBoy = CreateObjByFlag( 105461 , 780911 , BoxFlag[FlagNum] , 1 )
		...
		BeginPlot( BoxBoy , "sasa_105461_Kill" , 0 )
		SetPlot( BoxBoy , "collision" , "sasa_105461_GO" , 10 )
		Setplot( BoxBoy , "dead" , "sasa_105461_BoxDead" , 30 )
...
--※ 火藥桶碰撞 ※--
function sasa_105461_GO()
	SysCastSpellLV( TargetID() , OwnerID() , 496259 , 14 )			--強力撞擊
	sleep(1)
end

--※ 火藥桶炸完掉火藥 ※--
function sasa_105461_BoxDead()
	local Room = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	local Bomb
	...
	Setplot( Bomb , "touch" , "sasa_118460_AI" , 30 )
	WriteRoleValue( Bomb , EM_RoleValue_Livetime , 10 )			--存在秒數
end

--※ 點火藥 ※--
function sasa_118460_AI()
	BeginPlot( TargetID() , "sasa_118460_AI2" , 0)
	WriteRoleValue ( TargetID() , EM_RoleValue_Register1 , OwnerID() )
end

--※ 刪火藥 ※--
function sasa_118460_AI2()
...
function sasa_105458_Dead()
	...
	local Treasure = CreateObjByFlag( 105460 , 780911 , 10 , 1 )			--寶箱
	...
	BeginPlot( Treasure , "sasa_105460_Treasure", 0 )
...
--※ 寶箱 ※--
function sasa_105460_Treasure()
	SetPlot( OwnerID() ,"touch", "sasa_105460_Treasure_open" ,40 )
end

function sasa_105460_Treasure_open()
...
--※ 防車被攻擊用 ※--餘火、岩氣
function sasa_105665_npccheck()
	if ReadRoleValue( TargetID() , EM_RoleValue_OrgID ) == 105665 then
		return false
	else
		return true
	end
...
--※ 車子 ※--
function sasa_105665_Tank01()					--攻城車模版產生劇情
	AddBuff( OwnerID() , 509268 , 9 , -1 )		--攻擊NPC威力UP
	AddBuff( OwnerID() , 509269 , 49 , -1 )		--被NPC攻擊威力UP
end

function sasa_105665_Tank02( Seat , UpDown )	--攻城車主要script
...
--※ 彈藥檢查 ※--
function sasa_105665_Buffcheck()
...
--※ 清BUFF ※--
function sasa_105458_CancelBuff()												--刪除BUFF用
