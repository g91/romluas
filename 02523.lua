function cl_Z21_05_06_NO2_N() --5~6.7王中間巡邏
	local dis = 20
	...
	BeginPlot( OwnerID() , "cl_z21_wark05_06_NO2_N" , 0 )
end
function cl_z21_wark05_06_NO2_N()
	sleep( 10 )
	BeginPlot( OwnerID() , "cl_Z21_05_06_NO2_N" , 0 )
end
function cl_Z21_04_05_NO2_N() --5~6.7王中間巡邏
	local dis = 20
	local count = 5 --陣列的大小，輸入3 會是 3*3 的陣列；不過因為行數必須為奇數，所以輸入 4 會是 5*4 的陣列。
	local checktime = 10 --判斷的時間間距，數值愈低，判斷的頻率愈高，愈自然，但耗的資源也愈多
	local ReviveTime = 600000  --當陣列中的物件全部消失時，進行陣列重置的動作，如果填 -1 ，就會按照 OwnerID() 設定的重生時間重置
	...
	LuaFunc_NPCWalkByMatrix_2( ObjMatrix , Matrix , dis , count , checktime , circle ) 
	LuaFunc_ResetMatrix( ReviveTime , Matrix)
	BeginPlot( OwnerID() , "cl_z21_wark04_05_NO2_N" , 0 )
end
function cl_z21_wark04_05_NO2_N()
	sleep( 10 )
	BeginPlot( OwnerID() , "cl_Z21_04_05_NO2_N" , 0 )
end
function cl_21_106086Archer_N() ---不死夏多爾弓箭手AI
	local me = OwnerID()
				...
				if skill_time2 >= 5 then
					...
					if hate >= 1 then
						...
						if ReadRoleValue( hate_Table[Rand_Num] , EM_RoleValue_IsDead ) ~= 1 then
							CastSpelllv ( me , hate_Table[Rand_Num] , 497589 , 3 ) ---請帽子對自己施放一個AE命中的特效沒有任何傷害
							sleep ( 10 )
							skill_time2 = 0
						end
					end
