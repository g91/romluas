function bk_129_103933()--------------王本身ai
	local RoomID = ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)-----------------
	local start = ReadRoleValue(OwnerID(),EM_RoleValue_Register1)
	local boss = OwnerID()--------控制器本身	
	local CombatBegin=0
	local skill1={495467,495674}------1.迴旋斬擊 2.雙重衝擊
	local SkillIndex=0
	local skilllv={9,15}----招式威力
	...
	while 1 do 
		...
		if HateListCount( boss ) ~= 0 then	---- 確定他現在有打人
			...
			if mob1>= 15 then 
				...
				CastSpellLV( OwnerID() , OwnerID() , 495675 , 5 )
				sleep(30)
				mob1=0
			end		
			if skilltime>= 7 then 
...
function bk_129_103933_dead()
	...
	Delobj(door3)
	sleep(10)
	BeginPlot( boss ,"bk_129_103933_dead_1",20)-------------------
end
function bk_129_103954()
	local RoomID = ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)
	WriteRoleValue( OwnerID() ,EM_RoleValue_IsWalk , 1 )
	MoveToFlagEnabled ( OwnerID(),  false )
	yell(OwnerID(),"[SC_423273_5]",5)
	sleep(30)
	yell(OwnerID(),"[SC_423273_6]",5)
	LuaFunc_MoveToFlag( OwnerID(), 780669 , 1 , 10)--------------------------------走到我想去的位置準備攻擊
	sleep(20)
	DelObj(OwnerID())
end
function bk_129_103954_1()
	LuaFunc_MoveToFlag( OwnerID(), 780669 , 3 , 10)--------------------------------退到旁邊去不攻擊
	sleep(20)
	SetPosByFlag( OwnerID(), 780669 , 3 )
end
function bk_129_103933_dead_1()
	sleep(20)
	DelObj(OwnerID())
end
function bk_129_103954_3()
local RoomID = ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)
local n1 = ReadRoleValue(OwnerID(),EM_RoleValue_PID)--------------讀取n1
local co=OwnerID()
local PPL =0 
local gm=0
	while 1 do 
