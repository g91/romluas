...
function un_zone20_boss5()     --控制器				
	...
	local Boss=CreateObjByFlag(105374,780888,2,1)
	local doorchick1
	local playcount = 0
	--local doorchick2
	local firelose ={}
	...
	SetPlot( boss,"dead","un_zone20_boss5die",0 )	
	while true do
		...
		if hatelistcount(boss)~=0 then
			if CombatBegin==0 then
				...
				CombatBegin=1										--開打了
				BeginPlot(boss , "un_zone20_boss5AI1" , 0)	
				Setplot(Boss,"dead","un_zone20_boss5die",0)
		...
		if hatelistcount(boss)==0 and CombatBegin==1 then			--開戰後仇恨表沒人
			...
			if ReadRoleValue( boss , EM_RoleValue_IsDead)==0 then
				...
				sleep(50)
				Boss=CreateObjByFlag(105374,780888,2,1)
...
function un_zone20_boss5AI1()  --BossAI有穿盔甲階段****************************
	local Boss = OwnerID()
	...
	sleep(10)
			...
			if hp3 - hp1 >4 then                              --5%以後轉P2
				...
				bossskill = LuaFunc_CreateObjByObj ( 105428 , boss )	
				WriteRoleValue( bossskill , EM_RoleValue_HP ,  NowHP)
				AddToPartition(bossskill,RoomID)
				BeginPlot(bossskill , "un_zone20_boss5AI2" , 0)	
				Setplot(bossskill,"dead","un_zone20_boss5die",0)
			...
			CastSpelllv( boss, boss,497312,0 )
			sleep(20)
			...
			while ReadRoleValue(Boss,EM_RoleValue_SpellMagicID)~=0 do--檢查還有沒有在施法
				...
				if GetDistance( Boss, farplayer[1])>150 then
					CastSpell( Boss,farplayer[1], 497313 )
				else 
...
function un_zone20_boss5AI2()  --BossAI沒穿盔甲階段****************************
	local Boss = OwnerID()
	local ctrl = ReadRoleValue(Boss,EM_RoleValue_PID)
	local RoomID=ReadRoleValue(Boss,EM_RoleValue_RoomID)
	...
	sleep(10)
			if skilltime >= 29 then                              --5%以後轉P2
				...
				NowHp = ReadRoleValue(Boss,EM_RoleValue_HP)
				bossskill = LuaFunc_CreateObjByObj ( 105896 , boss )
				WriteRoleValue( bossskill , EM_RoleValue_HP , NowHP)
				AddToPartition(bossskill,RoomID)
				BeginPlot(bossskill , "un_zone20_boss5AI1" , 0)
				Setplot(bossskill,"dead","un_zone20_boss5die",0)
				WriteRoleValue(bossskill,EM_RoleValue_PID,Ctrl)
				WriteRoleValue(Ctrl,EM_RoleValue_PID,bossskill)
			...
			while ReadRoleValue(Boss,EM_RoleValue_SpellMagicID)~=0 do--檢查還有沒有在施法
				...
				farplayer = KS_GetHateList( Boss, 2 )--仇恨表( Type:1 依仇恨; 2 依距離; 3 玩家)
				if GetDistance( Boss, farplayer[1])>150 then
					CastSpell( OwnerID(),farplayer[1], 497313 )
				else 
...
function un_redead()        -----判斷兩層buff以後死亡用
	local own = OWnerID()
	local RoomID=ReadRoleValue(own,EM_RoleValue_RoomID)
	local ZoneID =  ReadRoleValue( OwnerID() , EM_RoleValue_ZoneID )  -- 自己的zone id
	local MonsterID = 105430--139
	if ZoneID == 140 then
		MonsterID = 105887
	end
	local monser = LuaFunc_CreateObjByObj ( MonsterID , own )
	AddToPartition( monser, RoomID)
	SysCastSpellLv(OwnerID(),OwnerID(),497316,0)
end

function Lua_Un_117788()	--掛在任務物品的初始劇情之下
	SetPlot( OwnerID(),"touch","un_117788_do",30 )--觸碰劇情
	WriteRoleValue(OwnerID(),EM_RoleValue_register1,0)
end

function un_117788_do()
	local own = OWnerID()
	local RoomID=ReadRoleValue(own,EM_RoleValue_RoomID)
	local tar = TargetID()
	local ZoneID =  ReadRoleValue( OwnerID() , EM_RoleValue_ZoneID )  -- 自己的zone id
	...
	if ZoneID == 140 then
		...
		if CheckBuff(OwnerID(),509198)==true then
			SetPlot( TargetID(),"touch","un_117788_do",30 )--觸碰劇情
			local monser = LuaFunc_CreateObjByObj ( MonsterID , own )
			AddToPartition( monser, RoomID)
			SysCastSpellLv(OwnerID(),OwnerID(),497316,0)
			...
			if ReadRoleValue(tar,EM_RoleValue_register1) ==0 then
				...
				addbuff(OwnerID(),509201,0,-1)
				delobj(tar)
			end	
		end
end
function Lua_Un_117789()	--掛在任務物品的初始劇情之下
	SetPlot( OwnerID(),"touch","un_117789_do",30 )--觸碰劇情
	WriteRoleValue(OwnerID(),EM_RoleValue_register1,0)
end

function un_117789_do()
	local own = OWnerID()
	local RoomID=ReadRoleValue(own,EM_RoleValue_RoomID)
	local tar = TargetID()
	local ZoneID =  ReadRoleValue( OwnerID() , EM_RoleValue_ZoneID )  -- 自己的zone id
	...
	if ZoneID == 140 then
		...
		if CheckBuff(OwnerID(),509198)==true then
			SetPlot( TargetID(),"touch","un_117789_do",30 )--觸碰劇情
			local monser = LuaFunc_CreateObjByObj ( MonsterID , own )
			AddToPartition( monser, RoomID)
			beginplot(monser,"un_104952",0)
			SysCastSpellLv(OwnerID(),OwnerID(),497316,0)

		else
			if ReadRoleValue(tar,EM_RoleValue_register1) ==0 then
				WriteRoleValue(tar,EM_RoleValue_register1,1)
				addbuff(OwnerID(),509202,0,-1)
				delobj(tar)
			end	
		end
end
function un_zone20_playerdead()
	local own = OWnerID()
	local RoomID=ReadRoleValue(own,EM_RoleValue_RoomID)
	local ZoneID =  ReadRoleValue( OwnerID() , EM_RoleValue_ZoneID )  -- 自己的zone id
	local MonsterID = 105429--139
	if ZoneID == 140 then
		MonsterID = 105886
	end
	local monser = LuaFunc_CreateObjByObj ( MonsterID , own )
	AddToPartition( monser, RoomID)
	beginplot(monser,"un_104952",0)
	ScriptMessage( own, -1 , 2 ,"[SC_105374_07]", 2 )
end
function Lua_un_bossfight_139()    --分攤buff
		say(OwnerID(),"1")
		sleep(1)
		local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)
		--say(TargetID( ),"X"..x)
		local cab=Lua_DW_CreateObj("obj",104507,OwnerID(),0)
		...
		local PPL=SearchRangePlayer(OwnerID(),100)	
		local ppl2 = SearchRangePlayer(OwnerID(),50)
		local x=table.getn(PPL)-----取出來的是數量
		local y=table.getn(ppl2)-----取出來的是數量
		WriteRoleValue(cab,EM_RoleValue_Livetime,3)
		if x>=6 then 
			CastSpellLV(cab,cab,497321,16)
		elseif x==5 then 
			CastSpellLV(cab,cab,497321,32)
		elseif x==4 then 
			CastSpellLV(cab,cab,497321,64)
		elseif x==3 then
			CastSpellLV(cab,cab,497321,127)
		elseif x==2 then
			CastSpellLV(cab,cab,497321,254)
		elseif x==1 then	
			CastSpellLV(cab,cab,497321,499)
		end
		while ReadRoleValue(OWnerID(),EM_RoleValue_SpellMagicID)~=0 do--檢查還有沒有在施法
			sleep(2)
		end
		if y>=2 then
			sysCastSpellLV(cab,cab,497321,299)
		end
end