----------------------------------------------------------Z18古薩拉AI
function lua_Dy_Z135_BossAI_EZ()
	while 1 do 
		sleep(10)
		local counter1 = 0 ----------一般技能計時器
		local counter2 = 0 ----------遠距離直線防止器
		local skill = {496194 , 496348 , 496349 , 496344 } ------------單體中傷害、隨機挑人直接傷害、自身圓形AE、單體瀕死傷害50%
		local skillspeak = { 0 , "[SC_104458_1]" , 0 , "[SC_104458_2]"} 
		local skillLV = { 3 , 0 , 0 , 0 }
		local skilluse = 0
			...
			if  ( AttackTarget ~= 0 ) and ( CheckID( OwnerID() ) ) == true then	
				...
				elseif CH1T == 2 then
					if CH1T_2_Start == 0 then
						...
						SysCastSpellLv(OwnerID(),OwnerID(),496343 , 0 ) --弱化的凝咒炸裂
						--say(OwnerID(),"496200")
						---------
...
-----780385 小狗頭人旗標
-----780386 門的旗標
function lua_Dy_Z135_BossAIC_EZ()----簡單版控制器
...
--↑↑↑↑↑↑↑↑↑↑---------------------------------
	while 1 do
		sleep(10)
		local roomid = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID ) --讀出物件在哪一層
		local Boss = CreateObjByFlag( 104656 , 780696 , 1 , 1 ) ---生出Boss (BOSS編號  ,旗標編號 第N隻旗標 生出幾個怪)
		...
		BeginPlot( Boss, "lua_Dy_Z135_BossAI_EZ", 10) ---Boss的戰鬥Function (  BOSS BOSS的LUA_AI  秒\每10=1秒)
...
---在以下迴圈中，是為了事先設定三個陷阱要在哪幾個旗標位置生出------	
					while true do
						...
						elseif Trap_4 == 0 and Num_Rand ~= 1 and Num_Rand ~= Trap_1 and Num_Rand ~= Trap_2 and Num_Rand ~= Trap_3 then
							Trap_4 = Num_Rand
							table.insert( Rand_Trap_Table , Trap_4 )
						end
					end
...
--↑↑↑↑↑↑↑↑↑↑---------------------------------		
					for i = 1 , 4 , 1 do
						...
						sysCastSpellLV( OwnerID() , BOSSTrap  , 496347 , 1 ) -------控制器對補獸夾的位置，放一個隱形的地雷。
				...
				elseif counter1 == 10 or counter1 == 20  or counter1 == 30 or counter1 == 40 then ---生出小狗頭人的時間流
					...
					if Mob_Num >= 20 then ---小狗頭人在場上的最多隻數，需要控制。
						...
						Mob_Flag = Rand(10)+1
						local Mob = CreateObjByFlag( 104657 , 780385 , Mob_Flag , 1 ) --生出小狗頭人(編號  ,旗標編號 第N隻旗標 生出幾個怪)
						--SetPlot( Boss,"dead","lua_Dy_Z18BossAI_1_dead",10 )----Boss 死亡劇情
						...
						SetPlot( Mob,"dead","Dy_Zone135_Mob_dead_EZ",10 )----小狗頭人死亡劇情，目的在於，死亡後，扣Boss的血量
						AddBuff(Mob , 507866 , 0 , -1) --小狗頭人減傷的Buff
				...
				elseif counter1 == 46 then --判斷為第2階段 持續15秒
					...
					if Mob_Num >= 1 then  ----如果目前的小狗頭人數量大於1，則將此小狗頭人的Gid存入Boss的暫存器內
						...
						--say(Boss,"Chose_Rand ="..Chose_Rand)
...
function Dy_Zone135_ClearTrap() ---當隱藏在捕獸夾下的地雷引爆後，觸發的劇情。
	local Mob_Num = 0
	...
	if Mob_Num >= 1 then
		for i = 1 , Mob_Num , 1 do 
			if	ReadRoleValue(Search[i],EM_RoleValue_OrgID) == 	104460 then
				...
				if	ReadRoleValue(TargetID(),EM_RoleValue_OrgID) == 104657 
					or ReadRoleValue(Search[i],EM_RoleValue_OrgID) == 104656 then
					CancelBuff( TargetID(), 507866 )
...
function Dy_Zone135_Mob_dead_EZ() -----小狗頭人的死亡劇情
	...
	if	ReadRoleValue(TargetID(),EM_RoleValue_OrgID) == 104656 then ---如果小狗頭是Boss殺死的，則不執行這段死亡劇情。
		return
	else
		if Check_HP >= 5 then ---如果Boss血量大於「5%」才可以繼續執行動作
			SysCastSpellLv(Boss,Boss,496033 , 1 )--Boss自己對自己施放扣5%HP的傷害
		else  ----如果血量不足「5%」。則殺死狗頭人的玩家，則把Boss殺了。
...
function AP_Zone134_Mob_104459_EZ() ---小狗頭人出招式前的檢查
	local AttackTarget = ReadRoleValue( OwnerID() , EM_RoleValue_AttackTargetID ) 
	if CheckBuff(AttackTarget , 507619) == true then
		return false
	else
		if CheckBuff(OwnerID() , 507866) == true then
			return true
		else
			return false
		end
