...
--建立一個 table，在想產生物件的位置用 LuaFunc_CreateNPCByMatrix(ObjID,Matrix,A,B ) 產生物件。 其中四個數值依序代表 1.要生成的物件ID 2.作為基準的陣列 3.列位置 4.行位置
	local ObjMatrix ={}
	ObjMatrix[1] = LuaFunc_CreateNPCByMatrix(100908,Matrix,2,-1 )
	ObjMatrix[2] = LuaFunc_CreateNPCByMatrix(100908,Matrix,2, 1 )
	ObjMatrix[3] = LuaFunc_CreateNPCByMatrix(100915,Matrix,2, 0 )
...
--要求重新執行初始劇情
	BeginPlot( OwnerID() , "LuaYuyu_pig_matrix_Reset" , 0 )
end

function LuaYuyu_pig_matrix_Reset()
	sleep( 10 )
	BeginPlot( OwnerID() , "LuaYuyu_pig_matrix" , 0 )
end