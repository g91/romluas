function ic_105226_01()   ------主控器
		...
		local Boss=CreateObjByFlag(105226,780775,1,1) 	-----BOSS創在  旗標1
		...
		SetPlot( Boss,"dead","ic_105226_44",0 )
		...
		while true do
			...
			if hatelistcount(boss)~=0 then    		---讀仇恨人數
				...
				if open==0 then
				 ...
				 Boss100=Lua_DW_CreateObj("flag",104693,780775,1,0)--測試距離用
					...
					BeginPlot(Boss100 , "ic_105226_100" , 0)---測試距離		
				...
				door1=CreateObjByFlag(104198,780775,4,1)-------王身後阻擋門
					...
					BeginPlot(boss , "ic_105226_02" , 0)				------命令角色做事情
				end
			end	
			if hatelistcount(boss)==0 and open==1 or BOSS9898==1 and open==1 then			--開戰後仇恨表沒人
				if ReadRoleValue( boss , EM_RoleValue_IsDead)==0 then   --王沒死 玩家滅團
					...
					sleep(30)                                           --5秒
					Boss=CreateObjByFlag(105226,780775,1,1)           --再創造1之				
					AddToPartition(Boss,RoomID)	                                ---創造王 必要		
					SetPlot( Boss,"dead","ic_105226_44",0 )	
					WriteRoleValue(Ctrl,EM_RoleValue_PID,Boss)
					WriteRoleValue(Boss,EM_RoleValue_PID,Ctrl)
					BeginPlot(OwnerID() , "ic_105226_04" , 0)
			...
			if killidtime1==1 and play44==0 then
				...
				time22=0
					if time10==5 then
						...
						if hatelistcount(boss)==1 then
							...
							if CheckBuff(BossTarget, 507911 ) == true then		
								mob11=CreateObj(105227,X0,Y0,Z0,0,1)-------生隱形球
								...
								AddBuff( mob11,506822,0 ,-1 )
								WriteRoleValue( mob11 , EM_RoleValue_register1 , BossTarget )
								SetPlot( mob11,"dead","ic_105226_06",0 )  
								BeginPlot(mob11 , "ic_105226_05" , 0)
							end	
							end	
					...
					if time3>=11 then--範圍擊飛--第1仇恨 仇恨歸0(暫時)
						...
						if hatelistcount(boss)==1 then
							...
							local MessageString = "[SC_104605_08][$SETVAR1="..PlayerName.."]" 		----在string中可以秀出玩家的名字
									  ScriptMessage( OwnerID() , -1 , 2 , MessageString , 1 )
							CastSpellLV( OwnerID() , BossTarget , 496362 , 20 )
							sleep(20)
							time3=0
						elseif hatelistcount(boss)>1 then
							play1x=DW_HateRemain(1)-----------------------隨機打--不打坦
							...
							local MessageString = "[SC_104605_08][$SETVAR1="..PlayerName .."]" 		----在string中可以秀出玩家的名字
									  ScriptMessage( OwnerID() , -1 , 2 , MessageString , 1 )
							CastSpellLV( OwnerID() , play1x[plays] , 496362 , 20 )
							sleep(30)
							time3=0
						end	
					end
					if time2>=5 then--單體技能--第1仇恨
						...
						CastSpellLV( OwnerID() , BossTarget , 496984, 0 )----單體技能
						sleep(10)
						time2=0
					end
				end
				...
				time33=0 
					if time5==0 then--轉階段放1次
						...
						CastSpellLV( OwnerID() , OwnerID() , 496252, 3 )----全場DOT
						time5=1
					end
					if time22==5 then
							if hatelistcount(boss)==1 then
								...
								if CheckBuff( play1x[plays], 507911 ) == false then	---確認鐵處女BUFF  有不打
									...
									local MessageString = "[SC_104605_07][$SETVAR1="..PlayerName .."]" 		----在string中可以秀出玩家的名字
											  ScriptMessage( OwnerID() , -1 , 2 , MessageString , 1 )
									sleep(20)		  
									CastSpellLV( OwnerID() , play1x[plays] , 496366 ,2 )
									sleep(30)
								end	
							elseif hatelistcount(boss)>1 then	
					...
					if time9>=21 then--針刺--隨機1人
						if hatelistcount(boss)==1 then
								...
								local MessageString = "[SC_104605_07][$SETVAR1="..PlayerName.."]" 		----在string中可以秀出玩家的名字
											  ScriptMessage( OwnerID() , -1 , 2 , MessageString , 1 )
									sleep(20)		  
									CastSpellLV( OwnerID() , play1x[plays] , 496366 , 1 )
									sleep(30)
									time9=0
								end	
						elseif hatelistcount(boss)>1 then
					...
					if time8>=11 then---全體恐懼
						...
						sleep(10)
						CastSpellLV( OwnerID() , OwnerID() , 496251,2)----隨機3人跑吧
						sleep(10)
						time8=0
					end
				end
				if HPPercent <0.60 then--兄弟一起來
					...
					if time44==0 then--鐵處女---隨機1人
						...
						CastSpellLV( OwnerID() , OwnerID() , 496253,5)----全場DOT
						sleep(10)	
						while 	ReadRoleValue(OwnerID() , EM_RoleValue_SpellMagicID  ) ~= 0   do--------有沒有正在施放技能
							sleep(5)
						end
					...
					if time4>=17 then--針刺--隨機1人
						if hatelistcount(boss)==1 then
								...
								ScriptMessage( OwnerID(), -1 , 2 , "[SC_104605_09]" , 0 )----恐懼 畏懼 該讓你們感受看看了
								sleep(10)
								CastSpellLV( OwnerID() , OwnerID() , 496251,2)----隨機3人跑吧
								sleep(10)
								while 	ReadRoleValue(OwnerID() , EM_RoleValue_SpellMagicID  ) ~= 0   do--------有沒有正在施放技能
		...
		for i=1,TempPlayer do
			...
			if CheckID( ThesePlayer[i] ) == true and 
				...
				ReadRoleValue( ThesePlayer[i] , EM_RoleValue_IsPlayer ) == 1 then
					--檢查玩家是不是還在；檢查是否死亡
				if CheckBuff( ThesePlayer[i] , 508689 ) == true  --拷問
				or CheckBuff( ThesePlayer[i] , 507911 ) == true  --鐵處女
				or CheckBuff( ThesePlayer[i] , 507907 ) == true  --鐵反彈
				or CheckBuff( ThesePlayer[i] , 508690 ) == true  --凌遲
				or CheckBuff( ThesePlayer[i] , 508432 ) == true	then  --畏懼
					CancelBuff(ThesePlayer[i], 508689)
					CancelBuff(ThesePlayer[i], 507911)
					CancelBuff(ThesePlayer[i], 507907)
					CancelBuff(ThesePlayer[i], 507934)
					CancelBuff(ThesePlayer[i], 508690)
					CancelBuff(ThesePlayer[i], 508432)
				end
			end					
		end
	end
	function ic_105226_05()
		local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)
		...
		while 1 do
			...
			if time1==0 then
				CastSpellLV( OwnerID() , player , 496368,3)
