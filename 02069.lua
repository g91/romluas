...
function lua_sasa_104659_Boss()
	while 1 do 
		...
		local Skill = { 496031 , 496030 , 496172 , 496027 }	---------------雙角尖刺 , 聖石墬 , 翅毒 , 撼天震地
		local SkillLv = { 0 , 3 , 0 , 0 }
		local SkillString = { "[SC_104228_01]" , "[SC_104228_02]" , "[SC_104228_03]" , "[SC_104228_04]" }
		--銳利的尖刺，毫不留情地朝你襲來。
		--巨大的滾石讓你摸不著從何而生！
		--散落出的翅毒正是危險的象徵。
		--重拍撼地，身為盜墓者的你，對蟲王而言簡直殘弱不堪。
		local LittleWorm1 = 104661			----------自爆小蟲
		local LittleWorm2 = 104662			----------營養小蟲
		...
		Setplot( OwnerID() , "dead" , "lua_sasa_104659_BossDead" , 30 )
		while 1 do	
			sleep(10)
				...
				elseif counter2 >= 13 then												--------蟲蟲危機發動
					...
					for m = 1 , 1 do
						...
						for n = 1 , BugNumber do							
							local LittleWormTemp1 = CreateObjByFlag( 104661 , 780747 , flagNum1 , 1 )
							local LittleWormTemp2 = CreateObjByFlag( 104662 , 780747 , flagNum2 , 1 )
							if ReadRoleValue( PlayerInHate[LuckyGuyNumber] , EM_RoleValue_IsDead ) == 0 then
								...
								Setplot( LittleWormTemp1 , "range" , "lua_sasa_104661_Bomb_1" , 50 )
								Setplot( LittleWormTemp1 , "dead" , "lua_sasa_104661_dead" , 30 )	
								...
								Setplot( LittleWormTemp2 , "dead" , "lua_sasa_104662_dead" , 30 )
...
function lua_sasa_104661_Bomb_1()				-------自爆蟲
	BeginPlot( TargetID(), "lua_sasa_104661_Bomb_2", 1 )
	--Say( OwnerID() , "Bomb1" )
end

function lua_sasa_104661_Bomb_2()				-------自爆蟲
...
function lua_sasa_104659_BossDead()
	local Room = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	local LuckyFlag = {13 , 14 , 15}
	local LuckyNumber1 = Rand(3)+1
	--Say(TargetID() , "LuckyNumber1 ="..LuckyNumber1)
	local treasure1 = CreateObjByFlag( 104663 , 780747 , LuckyFlag[LuckyNumber1] , 1 )			----主掉落，古墓神秘寶箱
	...
	BeginPlot( treasure1 , "lua_sasa_104663_treasure1", 0 )
	table.remove(LuckyFlag , LuckyNumber1)
	local treasure2 = CreateObjByFlag( 104664 , 780747 , LuckyFlag[1] , 1 )			----分身用，古墓守護寶箱
	...
	BeginPlot( treasure2 , "lua_sasa_104663_treasure2", 0 )
	...
	BeginPlot( treasure3 , "lua_sasa_104663_treasure3", 0 )

	local Entrance = ReadRoleValue( OwnerID() , EM_RoleValue_register1 )
	DelObj( Entrance )
end

function lua_sasa_104663_treasure1()		-----主寶箱
	SetPlot( OwnerID() ,"touch", "sasa_104663_treasure1_open" ,40 )
end

function sasa_104663_treasure1_open()
	SetPlot( TargetID() ,"touch", "" ,0 )
	KillID( OwnerID() , TargetID() )
end

function lua_sasa_104663_treasure2()		-----分身寶箱
	SetPlot( OwnerID() ,"touch", "sasa_104663_treasure2_open" ,40 )
end

function sasa_104663_treasure2_open()
	SetPlot( TargetID() ,"touch", "" ,0 )
	KillID( OwnerID() , TargetID() )
	local Room = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	local FakeBoss = star_CreateObj( OwnerID() , 104660 , 0 , 0 , 0 , Room , 0 )
end

function lua_sasa_104663_treasure3()		-----掉髒寶箱
	SetPlot( OwnerID() ,"touch", "sasa_104663_treasure3_open" ,40 )
end

function sasa_104663_treasure3_open()
	SetPlot( TargetID() ,"touch", "" ,0 )
	KillID( OwnerID() , TargetID() )
end