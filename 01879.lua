...
----------產生爵士
	local Temp1 = CreateObjByFlag( 103942, 780668, 1, 1) 
...
----------產生執政官
	local Temp2 = CreateObjByFlag( 103943, 780668, 2, 1)
...
----------啟動王AI
	BeginPlot( Temp1, "Cang_129_2bo_baron", 0)
	BeginPlot( Temp2, "Cang_129_2bo_oh", 0)
	BeginPlot( OwnerID() , "Cang_129_2bo_GD" , 0)
			...
			if	CheckID(Temp1)==false or ReadRoleValue(Temp1,EM_RoleValue_IsDead)==1 then		
				if	CheckID(Temp2)==false or ReadRoleValue(Temp2,EM_RoleValue_IsDead)==1 then
					if res == 0 then
						...
						SetDefIdleMotion( Door4 , ruFUSION_MIME_IDLE_STAND )
						SetModeEx( Door4  , EM_SetModeType_Obstruct, false )--阻擋
						local Box = CreateObjByFlag( 103959, 780670, 12, 1)	--寶箱
...
----------黑光陣
			if countskill == 7 then
				...
				if	ReadRoleValue( OH,EM_RoleValue_IsDead ) == 0	then	--檢查OH是否死亡
					...
					BeginPlot( SpellCaster , "Cang_129_2bo_b_summomkill" , 0)
					countskill = 0
				--如果執政官死了，黑光陣的對象改變	
				...
				--當sawy小裡面有人的時候，隨便抓一個衰小的玩家
					if table.getn(sway1) ~= 0 then
						...
						AddToPartition( SpellCaster , RoomID )	--加進舞台	
						WriteRoleValue( SpellCaster , EM_RoleValue_PID, OwnerID() )
						BeginPlot( SpellCaster , "Cang_129_b_summomkill" , 0)
						countskill = 0
					end	
				end	
			end
...
--------------------------------------------------------------爵士法陣
function Cang_129_2bo_b_summomkill()
	AddBuff( OwnerID() , 505675 , 1 , -1 )
	sleep(30)
	SetPlot( OwnerID(), "range" , "Cang_129_2bo_b_s_range" ,30 )
...
----------搜尋55碼內有無NPC, NPC如果是HO王, 就鎖他移動, 表演
		...
		if table.getn( NPC )~= 0 then
			if NPC[0] == OH then
				...
				BeginPlot( OH , "Cang_129_Dontmove", 0) 
				sleep(10)
				DelObj( OwnerID() )
				break
			end
...
----------黑光陣，對玩家判定
function Cang_129_2bo_b_s_range()	
...
----------玫瑰綻放			
			if roseskill >= 10 then
				--爵士活著的時候，目標是爵士的目標
				--if CheckID( baron ) == true or ReadRoleValue( baron , EM_RoleValue_IsDead ) == 0 then	--檢查OH是否死亡				
				if ReadRoleValue( baron , EM_RoleValue_IsDead ) == 0 then	--檢查OH是否死亡				
					...
					roseskill = 0
				--爵士死之後，目標是自己的目標
				--elseif CheckID( baron ) == false or ReadRoleValue( baron , EM_RoleValue_IsDead ) == 1 then
...
--------------------------------------------------------------執政官流血技能
function Cang_129_2bo_oh_skill()
	BeginPlot( TargetID() , "Cang_129_2bo_oh_skill_do" , 0)
end

function Cang_129_2bo_oh_skill_do()
