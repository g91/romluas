...
function bico_Z24_Boss4_ctrl ()
	local Ctrl = OwnerID()					                      --控制器
	local CombatBegin = 0						      --開戰旗標
	local RoomID = ReadRoleValue(Ctrl,EM_RoleValue_RoomID)
	local opendoor = 0
	local monster = CreateObjByFlag(107229,781015,6,1)                                  ---小怪     5
	local Monster_Table = {}
	local Boss = CreateObjByFlag(106900,781015,2,1)                                         ---BOSS   2
	local hole2 = CreateObjByFlag(107466,781015,6,1)    
	local Hp  = ReadRoleValue(Boss,EM_RoleValue_HP)
	local maxhp = ReadRoleValue(Boss,EM_RoleValue_MaxHP)
	local Hp1  = (Hp / maxhp) *100 
	local Treasure = CreateObjByFlag( 107370 , 781015 , 1 , 1 )	--裂縫破裂產生寶箱
	...
	local hole1 = CreateObjByFlag(107363,781015,6,1)     --異界裂縫   6
...
--	say( hole1 , "hole="..hole1 )
	local hole2 = CreateObjByFlag(107466,781015,6,1)                              --可砍殺的異界裂縫   6 
	...
	WriteRoleValue(hole2,EM_RoleValue_Register1, boss)
...
--				PlayerNum = SetSearchAllPlayer( ReadRoleValue( ctrl , EM_RoleValue_RoomID ) )	--取副本內同層玩家所有資料
				LockHP(boss,0,"")
				sleep(10)
				KillID(boss , boss )
				DelObj(boss)
				...
				Treasure = CreateObjByFlag( 107370 , 781015 , 1 , 1 )
	...
	--			end
				...
				BeginPlot( Treasure , "Z24_107370_Treasure", 0 )
				Sleep(50)
				return
		end
		if HateListCount( boss ) ~= 0 then  --仇恨表內有人
			if CombatBegin == 0 then   
				BeginPlot(Boss , "bico_Z24_Boss4_ai" , 0)
				if opendoor == 0 then
...
--								DebugMsg(0,0,"player="..i)
			...
			if  	ReadRoleValue(ctrl,EM_RoleValue_register2) == 1 then 	-----BOSS第一次死掉產生可被攻擊的裂縫		
				hole2 = CreateObjByFlag(107466,781015,6,1)
	...
	--			SetModeEx( hole2, EM_SetModeType_ShowRoleHead, true )	
				...
				BeginPlot(hole2,"bico_Z24_Hole2AI",0)	
				...
				if CombatBegin == 1 then
					Monster_Table = SearchRangeNPC(ctrl,500)
						for i = 1 , table.getn(Monster_Table) do
							if ReadRoleValue(Monster_Table[i],EM_RoleValue_OrgID) == 107229  then
					...
					DelObj(holeB)
					P2Pick = 0
					hole1 = CreateObjByFlag(107363,781015,6,1)     --異界裂縫   6
					SetModeEx(hole1,EM_SetModeType_Mark, false)
					SetModeEx(hole1,EM_SetModeType_HideName, false)
					SetModeEx(hole1,EM_SetModeType_NotShowHPMP, false)
					SetModeEx(hole1,EM_SetModeType_Strikback, false)
	...
	--				Say( hole1 , "hole"..hole1 )							
					...
					sleep(50)
					Boss=CreateObjByFlag(106900,781015,2,1)
					AddToPartition(Boss,RoomID)
					WriteRoleValue( Boss , EM_RoleValue_Register10 , hole1 )	
...
function bico_Z24_Boss4_ai ()  --BossAI****************************
	...
	while 1 do
		...
		skilltime=skilltime+1	
			...
			if skilltime % 15 == 0 then 	--每15秒產生一隻吞噬者
				...
				monster = CreateObjByFlag(107229,781015,6,1)     --小怪  6
				...
				BeginPlot(monster,"bico_zone24_MonsterAi",0)				
			end
			end
			...
			if Hp1 >= 50 and P2Pick == 0 then                       			  --50%以前
				...
				KS_ScriptMessage("[SC_106900_02]")       --[無知的冒險者！用身體來體會我的槍術吧！]
					...
					local rand = DW_rand(3)
						for i = 1,Target_Num do
							if rand == 1 then
							SyscastSpellLv( boss,PlayerID[i] ,499877,5 )
							sleep(10)
							elseif rand ==  2 then
							SysCastSpellLv( boss, PlayerID[i],499878,5 )
							sleep(10)
							elseif rand ==  3 then
							SysCastSpellLv( boss,PlayerID[i],499879,5 )
							sleep(10)
							end
						end
				...
				if skilltime % 6 ==0 then                               	 --6秒一次rand3招
					...
					local rand = DW_rand(3)
						for i = 1,Target_Num do
							if rand == 1 then
							SyscastSpellLv( boss,PlayerID[i] ,499878,0 )
							sleep(10)
							elseif rand ==  2 then
							SysCastSpellLv( boss, PlayerID[i],499879,0 )
--							sleep(10)
--							elseif rand ==  3 then
--							SysCastSpellLv( boss,PlayerID[i],499877,0 )
							sleep(10)
							end
						end
...
--				say( hole1 , "hole="..hole1 )
				...
				CallPlot( hole1 ,"Z24_hole1_P2AI" , 0 )
				local holeA = CreateObjByFlag( 107440 , 781015 , 15 , 1 )			--A組裂縫
					...
					BeginPlot( holeA , "Z24_HoleA_AI" , 10 )
				local holeB =CreateObjByFlag( 107440 , 781015 , 16 , 1 )			--B組裂縫
					...
					BeginPlot( holeB , "Z24_HoleB_AI" , 10 )
				local MonsterA = CreateObjByFlag( 107439 , 781015 , 7 , 1 )		--A小怪起始點7號
					...
					BeginPlot( MonsterA , "Z24_monsterA_AI" , 0 )
				local MonsterB = CreateObjByFlag( 107438 , 781015 , 14 , 1 )		--B小怪起始點14號
					...
					BeginPlot( MonsterB , "Z24_monsterB_AI" , 0 )
				P2Pick = 1
				end				
			end
	end
...
function bico_zone24_MonsterAi()  --小怪Ai***********************
...
function bico_Z24monsterdie()
	CancelBuff(OwnerID(), 623519)
end

function bico_Z24_Hole2AI()
	local RoomID= ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)
	local ctrl = ReadRoleValue(OwnerID(),EM_RoleValue_PID)
	...
	while 1  do
		...
		holetime = holetime + 1 
			if holetime % 20 == 0 and monstercheck ==0 then
				...
				BeginPlot(monster2,"bico_Z24_monstershowAI",0)
				monstercheck = 1
			end
	end
end

function bico_Z24_monstershowAI()
...
function bico_Z24_Bossround2AI()
	say(OwnerID(),"good")
end

function bico_z24_givecancelbuff()
	local player = {}
...
function Z24_107370_Treasure()
	SetPlot( OwnerID() ,"touch", "Z24_107370_Treasure_open" ,40 )
end

function Z24_107370_Treasure_open()
	SetPlot( TargetID() ,"touch", "" ,0 )
	Sleep(20)
	KillID( OwnerID() , TargetID() )
end

function Z24_mustdie_true()
	return true
end

function Z24_monsterA_AI()
	local own = OwnerID()
while 1 do 
sleep(10)
...
function Z24_Boss_TimeLight_Player()	--傷害對Boss無效	
	local own =OwnerID()
	local tar =TargetID()
	...
	if Job ==0 or Monster ==102642 or Boss == 106900 or own == tar or Monster == 107438 or Ctrl == 106101 or Monster == 107439 or Monster ==106901 or Monster ==106904 or Monster == 106902 or Monster == 106903 or Monster ==107440 then
	return false
	else
	return true
	end	
...
function Z24_Boss_TimeLight_Boss()	--治療對玩家沒有用
	local own =OwnerID()
	local tar =TargetID()
	local Boss = ReadRoleValue( tar , EM_RoleValue_OrgID )
	if Boss == 106900 then
...
function Cl_Resist_Z24_4_HackersBossDead() ---BOSS死亡
	...
	for i = 1 , GlobalNum , 1 do
		if Lua_Cl_Resist_Hackers[RoomID]["BossID"][i] == BossID then --核對全域變數內自己的GUID是第幾順位
		
--			NPCDead( BossID , OwnerID )
