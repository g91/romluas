function LuaZone_1v1PVP_Main()
		...
		while 1 do
			...
			if CheckOK == 0 then
				if Xtime < 30 then
					...
					if Xtime == 5 then
					LuaZone_1v1PVP_CheckOK(30,25,1)
					end
					if Xtime == 10 then
					LuaZone_1v1PVP_CheckOK(30,20,1)
					end
					if Xtime == 15 then
						...
						LuaZone_1v1PVP_CheckOK(30,15,1)
					end
					if Xtime == 20 then
					LuaZone_1v1PVP_CheckOK(30,10,1)
					LuaZone_1v1PVP_CheckOK(10,10,0)
					end
					if Xtime == 25 then
					LuaZone_1v1PVP_CheckOK(30,5,1)
					end
					--檢查是否有一方無人
					local CountOneTeam = GetNumEnterTeamMember(RoomID,1)	--第一隊總和人數
					local CountTwoTeam = GetNumEnterTeamMember(RoomID,2) --第二隊總和人數
			...
			elseif CheckOK == 1 then
				...
				if WinnerCount >= 1 then
					...
					Setp = 2
					DesignLog(OwnerID(), 701215, "BG 1v1 End, Room: "..RoomID);
					break
				end
	...
	if Setp == 2 then
		ScriptMessage( OwnerID(),  -1, 1, "[SC_1V1PVP_CLOSEGROUND]" , C_SYSTEM )	--比賽已結束，競技場將在兩分鐘後關閉
		ScriptMessage( OwnerID(),  -1, 0, "[SC_1V1PVP_CLOSEGROUND]" , C_SYSTEM )	--比賽已結束，競技場將在兩分鐘後關閉
		Sleep(1200)
		CloseBattleGround(RoomID)
	end
end

function LuaZone_1v1PVP_PVPDead( KillerID , DeadID , ZoneID , ParallelZoneID , pRoomID )
	...
	if( KillerTeamID ~= GetTeamID( TargetID() ) ) then
		...
		--2011/08/10 新增旗標..活動需求
		GiveBodyItem( KillerID ,  545619  , 1 ) --給旗標
		GiveBodyItem( DeadID ,  545619  , 1 ) --給旗標
...
function LuaZone_1v1PVP_Revive()
...
function LuaZone_1v1PVP_JoinQueue( dbid, isTeam, numTeamPeople )
...
function LuaZone_World_1v1PVP_JoinQueue( dbid, BattleGroundType, WorldGroupID, LevelGroupID )
...
function LuaZone_1v1PVP_RoleEnterBattleGround( dbid, teamID )
	if( teamID == 1 ) then
...
function LuaZone_1v1PVP_MoveRoleToBattleGround( WorldID, ZoneID, dbid, teamid, roomid )
	local	ReX	= 0;
	local	ReY	= 0;
	local	ReZ	= 0;
	local	ReDir	= 0;
...
function LuaZone_1v1PVP_CheckOK(time,NowTime,type)
	local RoomID = ReadRoleValue( OwnerID(), EM_RoleValue_RoomID );
	local Obj
	local Count = SetSearchAllPlayer(RoomID)
	local	ReX 	= 1171
	...
	if Count ~= 0 then
		for i = 1 , Count , 1 do
			...
			if Obj:IsPlayer() == true then
				...
				StartClientCountDown ( ID, time, NowTime, 0, 1, type, String )	--玩家Client顯示倒數10秒
...
function Lua_PVPAllFull(Owner)
	local Job = ReadRoleValue( Owner , EM_RoleValue_VOC )
	local Job2 = ReadRoleValue( Owner , EM_RoleValue_VOC_SUB )
	local HP = ReadRoleValue( Owner , EM_RoleValue_MaxHP )
	WriteRoleValue( Owner , EM_RoleValue_HP , HP)
	if Job ==2 or Job ==3 then
		WriteRoleValue( Owner , EM_RoleValue_SP , 100)
		if Job2 ==2 or Job2 ==3 then
			WriteRoleValue( Owner , EM_RoleValue_SP_Sub , 100)
		elseif Job2 == 4 or Job2 ==5 or Job2 ==6 then
			local MP = ReadRoleValue( Owner , EM_RoleValue_MaxMP )
			WriteRoleValue( Owner , EM_RoleValue_MP , MP)
		end		
	...
	elseif Job == 0 or Job2 == 0 then
		local MP = ReadRoleValue( Owner , EM_RoleValue_MaxMP )
		WriteRoleValue( Owner , EM_RoleValue_MP , MP)
	end 
end

function LuaZone_1v1PVP_AssignLevelGroup( dbid, lv, worldid )
	local Group = nil;
	if lv < 30 then
		Group = 1
	else
...
function LuaZone_PCall_1v1PVP_CheckJoinBattleGround( iObjID, iBattleGroundID, iPartyID, iSrvID )
