function ic_105366_01()  																	---第10口袋固定死亡劇情用--第9口袋固定測試距離用					--say(OwnerID(),"boss44="..boss44)														
		...
		local Boss=CreateObjByFlag(105366,780886,1,1) 											
		local door=CreateObjByFlag(106006,780886,2,1)											
				...
				door1=CreateObjByFlag(106006,780886,3,1)									
				...
				SetPlot( Boss,"dead","ic_105366_BOSSXX",0 )											
		...
		while true do
			...
			elseif HPPercent>0 and HPPercent<0.3 and timeboss==1 then			  	
					...
					DelObj(boss)  
					bosshp=ReadRoleValue(isaac , EM_RoleValue_register5)
					Boss=CreateObjByFlag(105466,780886,1,1)
					WriteRoleValue( Boss, EM_RoleValue_HP,bosshp)
					AddToPartition(Boss,RoomID)
				...
				if hatelistcount(boss)~=0 then    												---讀仇恨人數
					...
					if  BOSSP1P2==3 then
						...
						if time1>=30 then														---第1階段15秒生3隻小怪
							if mobstop>9 then
								time1=0
							elseif mobstop<9 then
								ScriptMessage( OwnerID(), -1 , 1 , "[SC_105366_04]" , 2 )				 
								Luck1=DW_Rand(20)
								playKKboss=hatelistcount(boss)
								if playKKboss==1 then											
										bosskkpaly = ic_HateListSort(Boss)
										mobkk1=bosskkpaly[1]
										mob1=CreateObjByFlag(105367,780886,Luck1+11,1)
										...
										SetPlot( mob1,"dead","ic_105366_mob1xx",0 )		
										BeginPlot(mob1, "ic_105366_mob_1" , 0)
										SetAttack(mob1 , mobkk1)
										sysCastSpellLV( mobkk1, mob1, 497235, 0 )
										mobstop=ReadRoleValue(isaac , EM_RoleValue_register7)
										WriteRoleValue(isaac,EM_RoleValue_register7,mobstop+1)
										sleep(5)
										mob2=CreateObjByFlag(105368,780886,Luck1+12,1)
										...
										SetPlot( mob2,"dead","ic_105366_mob2xx",0 )			
										BeginPlot(mob2, "ic_105366_mob_2" , 0)
										SetAttack(mob2 , mobkk1)
										sysCastSpellLV( mobkk1, mob2, 497235, 0 )
										mobstop=ReadRoleValue(isaac , EM_RoleValue_register7)
										WriteRoleValue(isaac,EM_RoleValue_register7,mobstop+1)	
										mob3=CreateObjByFlag(105369,780886,Luck1+13,1)
										...
										SetPlot( mob3,"dead","ic_105366_mob3xx",0 )		
										BeginPlot(mob3, "ic_105366_mob_3" , 0)
					...
					Boss100=Lua_DW_CreateObj("flag",104693,780886,1,0)							
						...
						BeginPlot(Boss100 , "ic_105366_100" , 0)
					...
					Boss1=CreateObjByFlag(106108,780886,71,1)
						...
						BeginPlot(Boss1 , "ic_105366_BOSS123" , 0)
					Boss2=CreateObjByFlag(106108,780886,72,1)
						...
						BeginPlot(Boss2 , "ic_105366_BOSS123" , 0)
					Boss3=CreateObjByFlag(106108,780886,73,1)
						...
						BeginPlot(Boss3 , "ic_105366_BOSS123" , 0)			
					end
				end	
				if hatelistcount(boss)==0 and open==1 or BOSS9898==1 and open==1 then			
					if ReadRoleValue( boss , EM_RoleValue_IsDead)==0 then   					
						...
						sleep(30)                                          
						Boss=CreateObjByFlag(105366,780886,1,1)           									
						AddToPartition(Boss,RoomID)	                               					
						SetPlot( Boss,"dead","ic_105366_BOSSXX",0 )								
						WriteRoleValue(isaac,EM_RoleValue_PID,Boss)
						WriteRoleValue(Boss,EM_RoleValue_PID,isaac)
						BeginPlot(isaac, "ic_105366_BUFFXX" , 0)								
						open=0																	
						killidtime=0
						time1=0
						time2=0
...
function ic_105366_BOSS123()
		...
		while true do	
				...
				if mobxx==1 then																
					...
					CastSpellLV( OwnerID() , mobKK , 497244, 50 )							---沙玩家
			...
			for i=1,TempPlayer do
				...
				if CheckID( ThesePlayer[i] ) == true and 										
					...
					if CheckBuff( ThesePlayer[i] , 508381 ) == true or
					   ...
					   CheckBuff( ThesePlayer[i] , 509046 ) == true or
					   CheckBuff( ThesePlayer[i] , 509048 ) == true or
					   CheckBuff( ThesePlayer[i] , 509050 ) == true or
					   CheckBuff( ThesePlayer[i] , 509075 ) == true or
					   CheckBuff( ThesePlayer[i] , 509079 ) == true then						
						CancelBuff(ThesePlayer[i], 508381)										
						CancelBuff(ThesePlayer[i], 509076)
						CancelBuff(ThesePlayer[i], 509046)
						CancelBuff(ThesePlayer[i], 509048)
						CancelBuff(ThesePlayer[i], 509050)
						CancelBuff(ThesePlayer[i], 509075)
						CancelBuff(ThesePlayer[i], 509079)
					end
				end					
			end
end	
function ic_105366_bossnokk()																---不攻擊GM
		if ReadRoleValue(TargetID() , EM_RoleValue_VOC ) ~= 0 and CheckBuff( TargetID(), 509079 ) == false  then 
			return true
		elseif ReadRoleValue(TargetID() , EM_RoleValue_VOC ) ==0 then 
			return false
		elseif CheckBuff( TargetID(), 509079 ) == true then  
			return false
		end
end	
function ic_105366_100()																	
	local isaac=ReadRoleValue(OwnerID(),EM_RoleValue_PID)
...
function ic_105366_mob_1()																	---法師
		...
		while 1 do
			...
			if HPPercent>0.1 then
				...
				if time1>=11 then
					CastSpellLV( OwnerID() , AttackTarget , 497229,3000)
					sleep(40)
					time1=0
				elseif time1>=0 then	
					CastSpellLV( OwnerID() , AttackTarget , 497233,1500)	
					sleep(10)
				end
...
function ic_105366_mob_2()												--戰士					
		...
		while 1 do
			...
			if HPPercent>0.1 then
				...
				SetAttack(OwnerID() , AttackTarget)
				if time1>=20 then
					CastSpellLV( OwnerID() , AttackTarget , 497230,20)
					sleep(10)
					time1=0
				end
			elseif HPPercent<0.1 then
...
function ic_105366_mob_3()																	
		...
		while 1 do
			...
			if HPPercent>0.1 then
				...
				SetAttack(OwnerID() , AttackTarget)
				if time1>=11 then
					CastSpellLV( OwnerID() , AttackTarget , 497231,80)
					sleep(20)
					time1=0
				elseif time1>=0 then	
					CastSpellLV( OwnerID() , AttackTarget , 497232,3)
					sleep(10)
				end	
			elseif HPPercent<=0.1  then
				SetModeEx( OwnerID()  , EM_SetModeType_Move, true )								
			...
			while 1 do
				...
				if HPPercent>0.6 then  
						...
						if time1>= 15 then
							...
							CastSpellLV( OwnerID(), OwnerID(), 497237,3)					
							sleep(10)
							time1=0
						end
						if time2>= 13 then
							if hatelistcount(boss)==1 then
								...
								CastSpellLV( OwnerID(), BossTarget, 497238,3000)				
								sleep(30)
								time2=0
							elseif hatelistcount(boss)>=2 then	
								...
								CastSpellLV( OwnerID(), play1x[plays], 497238,3000)			
								sleep(30)
								time2=0
							end
						end
				...
				elseif HPPercent<0.6 and HPPercent>0.3 then				
					...
					elseif	time20==1 then
						...
						if time4>=9 then
							if hatelistcount(boss)==1 then
								CastSpellLV( OwnerID(), BossTarget, 497058, 11)				
								sleep(30)
								time4=0
							elseif hatelistcount(boss)>=2 then	
								play1x=DW_HateRemain(1)										
								plays=DW_Rand(table.getn(play1x))
								CastSpellLV( OwnerID(), play1x[plays], 497058, 11)			
								sleep(30)
								time4=0
							end	
						end
						if time5>=4  then
							if hatelistcount(boss)==1 then
								CastSpellLV( OwnerID(), BossTarget, 497393,3000)				
								sleep(10)
								time5=0
							elseif hatelistcount(boss)>=2 then	
								play1x=DW_HateRemain(1)										
								plays=DW_Rand(table.getn(play1x))
								CastSpellLV( OwnerID(), play1x[plays], 497393,3000)			
								sleep(10)
								time5=0
							end
						end
						if time6>=0 then
							CastSpellLV( OwnerID(), OwnerID(), 497240, 3000 )					
							sleep(5)
							time6=0
						end
					end	
				elseif HPPercent<=0.3 then
					...
					if HPPercent>0  and HPPercent<0.3  and time3==0 and BOSSXX==0 or 
					   ...
					   HPPercent>0  and HPPercent<0.02 and time3==7 and BOSSXX==0   then
						...
						BeginPlot(mob991, "ic_105366_mob91" , 0)
						BeginPlot(mob992, "ic_105366_mob92" , 0)
						BeginPlot(mob993, "ic_105366_mob93" , 0)
						BeginPlot(mob994, "ic_105366_mob94" , 0)
						BeginPlot(mob995, "ic_105366_mob95" , 0)
						BeginPlot(mob996, "ic_105366_mob96" , 0)
					...
					time2=time2+1
						if time1>=15 then
							if hatelistcount(boss)==1 then
								CastSpellLV( OwnerID(), BossTarget, 497392,5)					
								sleep(10)
								time1=0	
							elseif hatelistcount(boss)>=2 then
								play1x=DW_HateRemain(1)											
								plays=DW_Rand(table.getn(play1x))
								CastSpellLV( OwnerID(), play1x[plays], 497392,5)				
								sleep(10)
								time1=0
							end	
						end
