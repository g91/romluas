function cl_136_105305()
	...
	BeginPlot( cord,"cl_givekeyitem03", 0 )
	local lmst = CreateObjByFlag( 105307 , 780873 , 2 , 1 )  ---生出小王
	...
	BeginPlot( lmst,"cl_105307lmst", 0 )
...
function cl_105307lmst()
	...
	SetPlot( lmst, "dead","cl_136_105307_dead", 0 ) --boss執行死亡劇情,將直傳回給中控器判斷階段
end
function cl_136_105307_dead()
	local lmst = OwnerID()
	local RoomID = ReadRoleValue( lmst , EM_RoleValue_RoomID ) ---宣告這個地圖~是我現在讀到的地圖
	local cordnumber = ReadRoleValue ( lmst , EM_RoleValue_Register1 )
	...
	SetPlot( lmstnpc, "touch","cl_117549_touch", 20 ) ------PlotType 觸發類型 (分為"touch"觸碰、"range"範圍、"dead"死亡、"collision"碰撞四種)
	delobj ( lmst )
end
function cl_117549_touch()
	local player = OwnerID()
	local headgear = TargetID()
	local AI = ReadRoleValue( headgear , EM_RoleValue_PID )
	if	BeginCastBarEvent( player , headgear , "[SC_CORDLOAD_01]" , 20 , ruFUSION_ACTORSTATE_CROUCH_BEGIN , ruFUSION_ACTORSTATE_CROUCH_END,0 , "cl_136_giveitem") ~=1	then
		ScriptMessage( player, player, 1,"[SAY_NPC_BUSY]", 0 );		--目標忙碌中
	end
end
function cl_136_giveitem(Executer,Result) ---嘗試著抓取兔子
	if	Result > 0 then --成功狀態
		EndCastBar(Executer,Result)--清除施法條
		BeginPlot( TargetID() , "cl_117549" , 0)
	else --失敗狀態
		EndCastBar(Executer,Result)--清除施法條
	end
end
...
function cl_talk_cord()
	...
	if ReadRoleValue ( Obj , EM_RoleValue_VOC ) ~= 0 then
		...
		elseif CountBodyItem(Obj,cordcar[cordnumber]) ~= 0 or CountBodyItem(Obj,cordcarII[cordnumber]) ~= 0 then
			...
			AddSpeakOption( Obj,Target, "[SC_CORD_00]", "cl_chitem_talk_0", 0 )
			AddSpeakOption( Obj,Target, "[SC_CORD_01]", "cl_chitem_talk_1", 0 )
			AddSpeakOption( Obj,Target, "[SC_CORD_02]", "cl_chitem_talk_2", 0 )
			AddSpeakOption( Obj,Target, "[SC_CORD_03]", "cl_chitem_talk_3", 0 )
			AddSpeakOption( Obj,Target, "[SC_CORD_04]", "cl_chitem_talk_4", 0 )
			AddSpeakOption( Obj,Target, "[SC_CORD_05]", "cl_chitem_talk_5", 0 )
			AddSpeakOption( Obj,Target, "[SC_CORD_06]", "cl_chitem_talk_6", 0 )
			AddSpeakOption( Obj,Target, "[SC_CORD_07]", "cl_chitem_talk_7", 0 )
			AddSpeakOption( Obj,Target, "[SC_CORD_08]", "cl_chitem_talk_8", 0 )
			AddSpeakOption( Obj,Target, "[SC_CORD_09]", "cl_chitem_talk_9", 0 )
		end
	elseif ReadRoleValue ( Obj , EM_RoleValue_VOC ) == 0 then
		LoadQuestOption( Obj )
		AddSpeakOption( Obj,Target, "[SC_CORD_000]", "cl_chitem_talk_GM", 0 )----給GM迅速測試用
	end
end
function cl_clasetalk()
	if ReadRoleValue(TargetID(),EM_RoleValue_Register2) == 9 then-----
		CloseSpeak( OwnerID() )
	end
end
function cl_chitem_cord()
	local Obj = OwnerID()
	local Target = TargetID()
	...
	if	Input == RightCode[TableNumber][InputNumber]then
		...
		if nocordnumber >= 9 then
			...
			WriteRoleValue ( door , EM_RoleValue_PID, Target )
			WriteRoleValue ( door , EM_RoleValue_Register1 , Obj )
			BeginPlot( door,"cl_105308_big", 0 )
			WriteRoleValue ( door , EM_RoleValue_Livetime , 5 )
...
function cl_105308()
	...
	local joker = 0
	local big_joker = 0
	for i=1 , 3 , 1 do ---取出來的數量代表要做幾次
		joker = CreateObjByFlag( 105229 , 780873 , 3 , 1 )  ---神秘儀器
		AddToPartition ( joker , RoomID ) ----物件加入場景,例行公事
		WriteRoleValue ( joker , EM_RoleValue_Register1 , play )
		big_joker = CreateObjByFlag( 105230 , 780873 , 3 , 1 )  ---神秘儀器
		AddToPartition ( big_joker , RoomID ) ----物件加入場景,例行公事
		WriteRoleValue ( big_joker , EM_RoleValue_Register1 , play )
		BeginPlot( big_joker,"cl_136_joker", 0 )
		sleep ( 1 )
		BeginPlot( joker,"cl_136_joker", 0 )
		sleep ( 1 )
	end
end
function cl_105308_big()
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	local AI = ReadRoleValue ( OwnerID() , EM_RoleValue_PID )
	local play = ReadRoleValue ( OwnerID() , EM_RoleValue_Register1 )
	local joker = CreateObjByFlag( 105263 , 780873 , 3 , 1 )  ---神秘儀器
	AddToPartition ( joker , RoomID ) ----物件加入場景,例行公事
	WriteRoleValue ( joker , EM_RoleValue_PID , AI )
	WriteRoleValue ( joker , EM_RoleValue_Register1 , play )
	BeginPlot( joker,"cl_136_bigjoker", 0 )
	sleep ( 1 )
end
function cl_136_joker() ---
...
function cl_136_bigjoker() ---雜耍師的助手
	local joker = OwnerID() ----我叫做AI
	local RoomID = ReadRoleValue( joker , EM_RoleValue_RoomID ) ----這是AI讀取到的地圖
	local play = ReadRoleValue ( joker , EM_RoleValue_Register1 )
	SetPlot( joker, "dead","cl_136_bigjoker_dead", 0 ) ------PlotType 觸發類型 (分為"touch"觸碰、"range"範圍、"dead"死亡、"collision"碰撞四種)
	SetAttack( joker , play )
	while 1 do
		sleep (10)
		if HateListCount( joker ) ~= 0 then ---如果王的仇恨表內有人就繼續戰鬥
