function un_zone19_boss5()--FOR 湯***********************************	
	SetPlot( OwnerID(),"touch","un_zone19_boss5_1",30 )--觸碰劇情
	AddBuff( OwnerID(), 508370, 0, -1 ) --物品發光
	callplot(OwnerID(),"un_zone19_boss5_15",0)
	WriteRoleValue(OwnerID(),EM_RoleValue_register3,0)
end
function un_zone19_boss5_1()
	local tar = TargetID()

		if CheckBuff(OwnerID(),508371)==true or CheckBuff(OwnerID(),508374)==true or  CheckBuff(OwnerID(),508376)==true or CheckBuff(OwnerID(),508379)==true then
			--say(OwnerID(),"NO")
			SetPlot( TargetID(),"touch","un_zone19_boss5_1",30 )--觸碰劇情
			return
		else
			if ReadRoleValue(tar,EM_RoleValue_register3) ==0 then
				WriteRoleValue(tar,EM_RoleValue_register3,1)	
				AddBuff( OwnerID(), 508371, 0, -1 ) --給技能
				ks_CreateObj_reset( true, TargetID() )
			end	
		end

	--delobj(TargetID())
end
function un_zone19_boss5_2()--清除BUFF
	if	CheckBuff(OwnerID(),508371)==true then
		CancelBuff( OwnerID(), 508371)
	elseif 	CheckBuff(OwnerID(),508374)==true then
		CancelBuff( OwnerID(), 508374)
	elseif 	CheckBuff(OwnerID(),508376)==true then
		CancelBuff( OwnerID(), 508376)
	elseif 	CheckBuff(OwnerID(),508379)==true then
		CancelBuff( OwnerID(), 508379)
	end	
end
function un_zone19_boss5_3()--FOR 酒******************************
	SetPlot( OwnerID(),"touch","un_zone19_boss5_4",30 )--觸碰劇情
	AddBuff( OwnerID(), 508370, 0, -1 ) --物品發光
	callplot(OwnerID(),"un_zone19_boss5_15",0)
	WriteRoleValue(OwnerID(),EM_RoleValue_register3,0)
end
function un_zone19_boss5_4()
	local tar = TargetID()

		if CheckBuff(OwnerID(),508371)==true or CheckBuff(OwnerID(),508374)==true or  CheckBuff(OwnerID(),508376)==true or CheckBuff(OwnerID(),508379)==true then
			--say(OwnerID(),"NO")
			SetPlot( TargetID(),"touch","un_zone19_boss5_4",30 )--觸碰劇情
			return
		else
			if ReadRoleValue(tar,EM_RoleValue_register3) ==0 then
				WriteRoleValue(tar,EM_RoleValue_register3,1)
				AddBuff( OwnerID(), 508374, 0, -1 ) --給技能
				ks_CreateObj_reset( true, TargetID() )
		    end

		end
end
function un_zone19_boss5_5()--FOR蠟燭***************************
	SetPlot( OwnerID(),"touch","un_zone19_boss5_6",30 )--觸碰劇情
	AddBuff( OwnerID(), 508370, 0, -1 ) --物品發光
	callplot(OwnerID(),"un_zone19_boss5_15",0)
	WriteRoleValue(OwnerID(),EM_RoleValue_register3,0)
end
function un_zone19_boss5_6()
	local tar = TargetID()

		if CheckBuff(OwnerID(),508371)==true or CheckBuff(OwnerID(),508374)==true or  CheckBuff(OwnerID(),508376)==true or CheckBuff(OwnerID(),508379)==true then
			--say(OwnerID(),"NO")
			SetPlot( TargetID(),"touch","un_zone19_boss5_6",30 )--觸碰劇情
			return
		else
			if ReadRoleValue(tar,EM_RoleValue_register3) ==0 then
				WriteRoleValue(tar,EM_RoleValue_register3,1)	
				AddBuff( OwnerID(), 508376, 0, -1 ) --給技能
				ks_CreateObj_reset( true, TargetID() )				
			end
	
		end
end
function un_zone19_boss5_7()
	if	CheckBuff(TargetID(),508375)==true then
		AddBuff( TargetID(), 508378, 0, 10 )
	end
end
function un_zone19_boss5_8() --FOR肉************************
	SetPlot( OwnerID(),"touch","un_zone19_boss5_9",30 )--觸碰劇情
	AddBuff( OwnerID(), 508370, 0, -1 ) --物品發光
	callplot(OwnerID(),"un_zone19_boss5_15",0)
	WriteRoleValue(OwnerID(),EM_RoleValue_register3,0)
end
function un_zone19_boss5_9()
	local tar = TargetID()

		if CheckBuff(OwnerID(),508371)==true or CheckBuff(OwnerID(),508374)==true or  CheckBuff(OwnerID(),508376)==true or CheckBuff(OwnerID(),508379)==true then
			--say(OwnerID(),"NO")
			SetPlot( TargetID(),"touch","un_zone19_boss5_9",30 )--觸碰劇情
			return
		else
			if ReadRoleValue(tar,EM_RoleValue_register3) ==0 then
				WriteRoleValue(tar,EM_RoleValue_register3,1)
				AddBuff( OwnerID(), 508379, 0, -1 ) --給技能
				ks_CreateObj_reset( true, TargetID() )
			end
	
		end
end
function un_zone19_boss5_10() --招喚肉
	...
	local meet=CreateObj ( 105108 , x1 , y1 , z1 , 0 , 1) 
	SetModeEx(meet,EM_SetModeType_Mark, true)
	SetModeEx(meet,EM_SetModeType_HideName, true)
	SetModeEx(meet,EM_SetModeType_NotShowHPMP, true)
	SetModeEx(meet,EM_SetModeType_Strikback, false)
	SetModeEx(meet,EM_SetModeType_Move, false)
	SetModeEx(meet,EM_SetModeType_Fight, true)
	SetModeEx(meet,EM_SetModeType_Searchenemy, false)
	AddToPartition(meet,roomID )	
	--BeginPlot( meet , "un_zone19_boss5_12" , 0 )
end
function un_zone19_boss5_11() --肉的範圍嘲諷判斷
	local monsterid = ReadRoleValue(TargetID(),EM_RoleValue_OrgID)
	if monsterid == 105099 then
	--say(OwnerID(),"yy = "..monsterid)
		return false
	elseif monsterid == 105101 or monsterid == 105102 or monsterid == 105103  or  monsterid == 105635 or monsterid == 105636 or monsterid == 105637 then
		return true
	end
end
function un_zone19_boss5_12()
	local ttable = {}
	...
	while 1 do
		...
		if timee%5==0 then
			...
			ttable = SearchRangeNPC ( OwnerID() , 300	)
			for i= 0 , table.getn(ttable) do
				sysCastSpellLv (OwnerID() , ttable[i] , 496758 , 0 )
			end
		end
		if timee>=20 then
			delobj(OwnerID())
...
function un_zone19_boss5_14()
	if ReadRoleValue(TargetID() , EM_RoleValue_OrgID )==105099  or 
	ReadRoleValue(TargetID() , EM_RoleValue_OrgID )==105351 or
    ReadRoleValue(TargetID() , EM_RoleValue_OrgID )==105352	then
		return false
	else
		return true
	end
end
function un_zone19_boss5_15()
local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)
	while 1 do
	sleep(10)
		if un_foodlistcount[RoomID][1]==1 then
		Delobj(OwnerID())
		end
	end
end
function un_zone19_boss5_13()----*****掛在控制器上面讓食物產生
	local FlagObjID = 780793  --旗幟物件編號
	local FlagNum = GetMoveFlagCount( FlagObjID ) --旗幟數量
	local Obj = {117190,117193,117192,117191}--種植物件編號
...
un_foodlistcount = {}
un_zone19skilltime = {}
function un_zone19_boss5_allconreal() 						   ----總中控(控制BOSS+演戲+轉階段)                                  
	local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)  --讀取出現在的RoomID，並宣告成RoomID
	local Ctrl = OwnerID()       							   --宣告Ctrl = OwnerID() ， 避免重複呼叫
	un_zone19skilltime[RoomID] = {}
	un_zone19skilltime[RoomID][1] = 25
	un_foodlistcount[RoomID] = {}
	un_foodlistcount[RoomID][1] = 0
	un_changeskillcon[RoomID] = {}
	un_changeskillcon[RoomID][1] = 0
	--un_zone19skilltime[RoomID] = 25  --小怪階段時間
	--un_foodlistcount[RoomID] = 0
	--un_changeskillcon[RoomID] = 0  
	...
	Beginplot(littlemonster,"un_zone19_boss5_fight",0)         --讓littlemonster1執行範圍劇情的Script
	...
	local Boss                                                 --Boss
	Boss = CreateObjByFlag( 105099, 780838 , 1 , 1 )           --依據旗標生出Boss
	...
	Setplot(Boss,"dead","un_zone190_boss5Die",0) 
	local box
	...
	local door = 0
	--[[local doordel = CreateObjByFlag( 104538, 780838 , 6 , 1 )
	SetModeEx(doordel,EM_SetModeType_Fight, false)       --宣告doordel不能被攻擊/true = 能被攻擊
	SetModeEx(doordel,EM_SetModeType_Mark,false)                  ----標記
	SetModeEx(doordel,EM_SetModeType_Strikback,false)             ---反擊
	SetModeEx(doordel,EM_SetModeType_Move,false)                  ---移動	
	SetModeEx(doordel,EM_SetModeType_SearchenemyIgnore,false)     ---物件不會被搜尋
	SetModeEx(doordel,EM_SetModeType_HideName,false)              ---物件頭上是否顯示名稱
	SetModeEx(doordel,EM_SetModeType_HideMinimap,false)           ---物件是否在小地圖上顯示
	SetModeEx(doordel,EM_SetModeType_Searchenemy,false)           ---鎖敵
	SetModeEx(doordel,EM_SetModeType_NotShowHPMP,false)           ---不顯示血條
	SetModeEx(doordel,EM_SetModeType_show,false)                  --秀出
	Beginplot(doordel,"un_zone190_doordel",0)         --讓doordel執行範圍劇情的Script
	AddToPartition(doordel,RoomID)--]]
	
	while true do                                              --持續執行while迴圈
	--say(Ctrl,"skillcount = "..un_changeskillcon[RoomID][1])
		sleep(10)                                              --讓while迴圈每停一秒後才開始執行
		if un_changeskillcon[RoomID][1] == 1 then                            --小怪階段*********************************
			un_zone19skilltime[RoomID][1]=un_zone19skilltime[RoomID][1]+1                              --計數用
			...
			un_foodlistcount[RoomID][1]  = 0
				...
				Beginplot(foodCtrl,"un_zone19_boss5_13",0)
				...
				Beginplot(door,"un_zone190_doordel",0)
				AddToPartition(door,RoomID) 
				WriteRoleValue(Ctrl,EM_RoleValue_register3,1)              --宣告城1代表出過食物階段了
			end
			if un_zone19skilltime[RoomID][1]%30==0 and un_zone19skilltime[RoomID][1]<270 then          --每30秒執行一次
				for i = 1 , table.getn(monster) do             --出怪的數量 = for迴圈執行次數， table.getn()取得陣列數量
					...
					Beginplot(monster1,"un_zone19_monster",0)
					Beginplot(monster2,"un_zone19_monster",0)
					
				end
			end
			if un_zone19skilltime[RoomID][1] == 250 then  
				ScriptMessage( Boss, -1 , 2 , "[SC_105099_09]" , 2 )
			end
			if un_zone19skilltime[RoomID][1] >= 270 then                           --400秒以後結束出怪階段(判斷玩家有無任務需要進入的階段)
				un_zone19skilltime[RoomID][1] = 25                                  --先階段歸0，避免再出怪
				Delobj(foodCtrl)                               --刪除食物中控器避免再出食物
				un_foodlistcount[RoomID][1]  = 1
				WriteRoleValue(Ctrl,EM_RoleValue_register3,0)  --產出食物變術再宣告回0，讓重置時能再出食物
				un_changeskillcon[RoomID][1] = 2                             --接著改成正常BOSS戰階段
				...
				for i = 1 , player do                          --幾個玩家執行幾次for迴圈，判斷所有玩家
					...
					CheckFlag( playername, 544639 ) ~= true then            --判斷玩家是否有完成此任務的旗標
						un_changeskillcon[RoomID][1] = 3                                   --只要有玩家有任務就跳到任務階段
					end
				end
			end
		elseif un_changeskillcon[RoomID][1] == 2 then                        --Boss階段*********************************
			ScriptMessage( Ctrl, -1 , 2 , skilltalk[2] , 2 )   --Boss開戰對話
			sleep(10)
			
			Beginplot(Boss,"un_zone19_Boss5AI_01",0)           --直接塞Script給Boss執行
			un_changeskillcon[RoomID][1] = 6                                 --再把模式切換到待機，避免重複執行Script
			--Setplot(Boss,"dead","un_zone190_boss5Die2",0)    --丟死亡劇情給BOSS
			
		elseif un_changeskillcon[RoomID][1] == 3 then                        --Boss階段(演戲)***************************
			ScriptMessage( Ctrl, -1 , 2 , skilltalk[2] , 2 )
			Addbuff(Boss,502707,0,-1)                          --給Boss不能回血的BUFF
			Beginplot(Boss,"un_zone19_Boss5AI_02",0)           --直接塞Script給Boss執行
			un_changeskillcon[RoomID][1] = 6                                 --再把模式切換到待機，避免重複執行Script
			--Setplot(Boss,"dead","un_zone190_boss5Die",0)       --丟死亡劇情給BOSS
			
		elseif un_changeskillcon[RoomID][1] == 4 then                        --重置階段*********************************
			ScriptMessage( Ctrl, -1 , 2 , skilltalk[3] , 2 )
			Delobj(Boss)                                        --刪除BOSS
			delobj(door)
			un_changeskillcon[RoomID][1] = 0                                 --再將階段寫回0，避免重生
			Boss = CreateObjByFlag( 105099, 780838 , 1 , 1 )   --依據旗標生出Boss
			WriteRoleValue(Boss,EM_RoleValue_register1,Ctrl)   --將Ctrl寫入Boss的EM_RoleValue_register1
			SetModeEx(Boss,EM_SetModeType_Strikback,false)     ---反擊
			SetModeEx(Boss,EM_SetModeType_Fight,false)    	   ---可砍殺
			SetModeEx(Boss,EM_SetModeType_Searchenemy,false)   ---鎖敵 
			AddToPartition(Boss,RoomID)                        --顯現Boss
			Beginplot(littlemonster,"un_zone19_boss5_fight",0) --讓範圍觸發戰鬥劇情重新Beginplot
			Setplot(Boss,"dead","un_zone190_boss5Die",0) 
			local searnpc = {}
		...
		elseif un_changeskillcon[RoomID][1] == 5 then
			local searnpc = {}
			searnpc = SearchRangeNPC ( OwnerID() , 500 )
			for i=0 , table.getn(searnpc) do
				if ReadRoleValue(searnpc[i],EM_RoleValue_OrgID) ==105106 then
...
function un_zone19_boss5_fight()                     		   --小怪中控器script(範圍觸發)
	...
	--else
		if skill == 0 then
			SetPlot( fight,"range","un_zone19_boss5_fight_2",100 ) --範圍100內觸發script
		end
	--end	
end


function un_zone19_boss5_fight_2()                     	  	        --小怪中控器script
local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID) 
	...
	if ReadRoleValue(OwnerID(),EM_RoleValue_VOC)==0 or ReadRoleValue( OwnerID() , EM_RoleValue_IsDead)==1 then
		--SetPlot( tar,"range","un_zone19_boss5_fight_2",100 )
		Beginplot(tar,"un_zone19_boss5_fight",0)
	else	
		un_changeskillcon[RoomID][1] = 1   	--將控制器的Reg1寫成1代表模式1小怪模式
	end
end


function un_zone19_Boss5AI_01()                                --BossAI_Script正常階段******
	local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)  --讀取出現在的RoomID，並宣告成RoomID 
	...
	while 1 do
		...
		if hatelistcount(Boss)~=0 then                             --Boss仇恨列表有人=進入戰鬥
			...
			if skilltime%7 == 0 then
				...
				Targ=ReadRoleValue(OwnerID(),EM_RoleValue_AttackTargetID)--BOSS的當錢目標
                 --給隨機一人buff
				CastSpelllv( OwnerID(), Targ,496759 ,0 )           --對當前目標放毒爪
			...
			sleep(10)
				...
				if table.getn(Play_Chose)<6 then                   --判斷如果BOSS仇恨列表<6人
					for i=1 , table.getn(Play_Chose) do            --對仇恨列表內所有人放招
						sysCastSpelllv(OwnerID(),Play_Chose[i],496762,0)--系統放傷害招
					end
				end
				if table.getn(Play_Chose)>=6 then                   --判斷如果BOSS仇恨列表>6人
					for i=1 , 6 do                                  --對仇恨列表內6人放招
						sysCastSpelllv(OwnerID(),Play_Chose[i],496762,0)--系統放傷害招
					end
				end
				sleep(30)
		...
		elseif 	hatelistcount(Boss)==0 and skill ==1 then           --Boss仇恨表沒人+曾經戰鬥過 =重置階段
			un_changeskillcon[RoomID][1] = 4                                      --將控制器階段寫到重置4
		end
	end
end


function un_zone19_Boss5AI_02()                                   --BossAI_Script演戲階段******
	local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)  --讀取出現在的RoomID，並宣告成RoomID 
	local Boss = OwnerID()                                     --宣告Boss=OwnerID()避免重複宣告造成效能消耗
	local Ctrl = ReadRoleValue(Boss,EM_RoleValue_register1)    --讀取Boss的reg1為控制器的GID，並宣告成Ctrl
	local skilltime = 0                                        --宣告主要時間 skilltime = 0 
...
function un_zone19_monster()                                      --小怪離開戰鬥判斷重置階段******
local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID) 
	...
	while 1 do                                                    --每秒執行while迴圈
		...
		if hatelistcount(own)==0 then                             --判斷小怪本身有無進入戰鬥，==0則仇恨表無人
			...
			if skilltime ==15 then                                 --未進入戰鬥持續10秒
				...
				un_changeskillcon[RoomID][1] = 0                                --將階段寫回0，階段0 = 待機
				WriteRoleValue(Ctrl,EM_RoleValue_register3,0)
				SetPlot( Ctrlball,"range","un_zone19_boss5_fight_2",100 )--將觸發戰鬥的球重新寫回未觸發
				un_zone19skilltime[RoomID][1] = 25
				Delobj(own)                                       --刪除小怪本身
			end
...
function un_zone19_bossball()    --球紀錄時間要判斷玩家是否BUFF有解除用
	local ttime = 0              --宣告時間ttime = 0
	local player = ReadRoleValue(OwnerID(),EM_RoleValue_register1)    --讀取Ball的reg1為控制器的玩家，並宣告成player
	while 1 do
		sleep(9)
		ttime=ttime+1
		if ttime == 6 then
		--say(player,"time = 5")
			if checkbuff(player,508386)==true or checkbuff(player,509598)==true then --如果玩家身上有BUFF
			--say(player,"y")
				SysCastSpellLv(OwnerID(),player,496760,0)--給玩家一個暢快
				sleep(10)
				Delobj(OwnerID())
			else                                   --如果玩家身上沒BUFF
			--say(player,"n")
...
function un_zone19_hit1()                        --Boss丟毒水的判斷1
	local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)
	...
	AddToPartition(ball,RoomID )
	beginplot(ball,"un_zone19_hit2",0)
end

function un_zone19_hit2()                        --Boss丟毒水的判斷2
	AddBuff( OwnerID() ,506823 ,0 ,-1 )
	while true do
	sleep(10)
		sysCastSpellLv(OwnerID(),OwnerID(),496763,0)
	end	
end

function un_zone19_ika()                        --演戲撿東西
	local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)
...
function un_zone190_boss5Die()
	...
	un_changeskillcon[RoomID][1] = 5
	ScriptMessage( OwnerID(), -1 , 2 ,"[SC_105099_06]" , 2 )
end
function un_zone190_boss5Die2()
local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID) 
	un_changeskillcon[RoomID][1] = 5
	ScriptMessage( OwnerID(), -1 , 2 , "[SC_105099_06]"  , 2 )
end
function un_zone19_npccheck()
	if ReadRoleValue(TargetID() , EM_RoleValue_OrgID )==105069 or
	ReadRoleValue(TargetID() , EM_RoleValue_OrgID )==105070 then
		return false
	else
		return true
	end
end
function un_zone190_doordel()
local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID) 
	while 1 do
		sleep(50)
		if un_changeskillcon[RoomID][1] == 0 or un_changeskillcon[RoomID][1] == 4 or un_changeskillcon[RoomID][1] == 5 then
			delobj(OwnerID())	
		end
	end
end

function un_zone19_boss5use_3()  --直接出任務BOSS
local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID) 
	un_changeskillcon[RoomID][1] = 3
end

function un_zone19_boss5use_1()  --結束出小怪階段
local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID) 
	un_zone19skilltime[RoomID][1]= 268
end