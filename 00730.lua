--陣型產生範例
function LuaSuez_test_matrix() --物件的 PID ，register+9 , register + 8 要留給陣型紀錄資料用
...
--  [Count][-Count/2... -2 , -1 , 0 , 1 , 2 ... Count/2]
	...
	local dis = 15 --陣列的間距
	local checktime = 30 --判斷的時間間距，數值愈低，判斷的頻率愈高，愈自然，但耗的資源也愈多
	local ReviveTime = 200 --當陣列中的物件全部消失時，進行陣列重置的動作，如果填 -1 ，就會按照 OwnerID() 設定的重生時間重置
	local circle = 3 --多少週期完整重生一次，如果填 0 ，則只有當所有物件消失才進行重生
...
--建立一個 table，在想產生物件的位置用 LuaFunc_CreateNPCByMatrix(ObjID,Matrix,A,B ) 產生物件。 其中四個數值依序代表 1.要生成的物件ID 2.作為基準的陣列 3.列位置 4.行位置
	local ObjMatrix ={}
	ObjMatrix[1] = LuaFunc_CreateNPCByMatrix(113165,Matrix,3, 0 )
--	ObjMatrix[2] = LuaFunc_CreateNPCByMatrix(113165,Matrix,3, 0 )
--	ObjMatrix[3] = LuaFunc_CreateNPCByMatrix(110001,Matrix,1, 0 )
--	ObjMatrix[4] = LuaFunc_CreateNPCByMatrix(110001,Matrix,2,-1 )
...
--要求重新執行初始劇情
	BeginPlot( OwnerID() , "LuaSuez_test_matrix_Reset" , 0 )
end

function LuaSuez_test_matrix_Reset()
	sleep( 10 )
	BeginPlot( OwnerID() , "LuaSuez_test_matrix" , 0 )
end
function LuaP_Suez_test_matrix() --要週期重生的話，放在第一個移動點的script
	LuaFunc_MatrixCircleRecord()
end