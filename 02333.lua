function cl_136_105205_E() ---控制器AI
	...
	for i = 1 , 6 do
		...
		BeginPlot( rabbit,"cl_136_rabbit_E", 0 )
		SetPlot( rabbit, "dead","cl_136_opendoor_dead_E", 0 ) --boss執行死亡劇情,將直傳回給中控器判斷階段
		sleep ( 1 )
	end
	while ReadRoleValue ( AI , EM_RoleValue_Register1 ) ~= 4 do
		...
		if showAI >= 6 then
			wark_rand = DW_Rand(30)
			boss = CreateObjByFlag( 105578 , 780767 , wark_rand , 1 )  ------------生出魔術帽
			AddToPartition ( boss,RoomID ) ----物件加入場景,例行公事
			WriteRoleValue ( boss , EM_RoleValue_PID, AI ) --------對BOSS說是中控器AI產生的
			BeginPlot( boss,"cl_136_105204_E", 0 )  -----------將王的scpipt塞進去王身上,讓王執行
			SetPlot( boss, "dead","cl_136_105204_dead_E", 0 ) --boss執行死亡劇情,將直傳回給中控器判斷階段
		...
		if stage == 1 then --如果stage等於1執行以下
			if attackflag == 0 then --如果戰鬥開始等於0執行以下
				...
				BeginPlot( cck , "cl_105209_chack_E" , 0 ) ----執行檢查BOSS距離是否超出房間
				...
				BeginPlot( AI2 , "cl_105209_P_E" , 0 ) --------執行每秒檢查AI的stage的直是多少
			...
			if del_boss == 1 then
				...
				if HPPercent >= 1 then
					sleep ( 30 )
					delobj ( boss )
					boss = CreateObj( 105578 , xx1, yy2 , zz3 , 0 , 1 )
					AddToPartition ( boss,RoomID ) ----物件加入場景,例行公事
		...
		elseif stage == 3 then ---如果stage等於3代表進入重置階段---BOSS離開戰鬥後會回傳3
			...
			sleep ( 30 ) ---停留3秒
			cazry = 0
			boss = CreateObjByFlag( 105578 , 780844 , 0 , 1 ) ------------重新產生BOSS
			AddToPartition ( boss,RoomID ) ----物件加入場景,例行公事
			WriteRoleValue ( boss , EM_RoleValue_PID, AI ) --------對BOSS說是中控器AI產生的
			WriteRoleValue ( cck , EM_RoleValue_Register1, boss ) --------對BOSS說是中控器AI產生的
			BeginPlot( boss,"cl_136_105204_E", 0 )  -----------將王的scpipt塞進去王身上,讓王執行
...
function cl_136_105204_E() ---bossAI
	...
	SetPlot( boss, "dead","cl_136_105204_dead_E", 0 ) ------PlotType 觸發類型 (分為"touch"觸碰、"range"範圍、"dead"死亡、"collision"碰撞四種)
	while 1 do
		sleep ( 10 )
		local AttackTarget = ReadRoleValue( boss , EM_RoleValue_AttackTargetID )-----AttackTarget=當前攻擊目標
		...
		if HateListCount( boss ) ~= 0 then ---如果王的仇恨表內有人就繼續戰鬥
			...
			if CombatBegin == 1 then 
				...
				if skill >= 33 then --每33秒一到就將玩家用帽子蓋起
					...
					cl_magicInHand_BIG_EZ() ----讀取仇恨表內誰要蓋帽誰要變大
					skill99_9 = 2
					sleep ( 30 ) --
					skill = 0
					skill2 = 0
...
function cl_magicInHand_BIG_EZ()
	local boss = OwnerID()
	local RoomID = ReadRoleValue( boss , EM_RoleValue_RoomID ) ----這是AI讀取到的地圖
	local AI = ReadRoleValue ( boss , EM_RoleValue_PID ) --PID的值代表中控器
	local player_table = {}
	...
	if player_number < 3 then
		...
		for i = 1 , kill_nob , 1 do ---取出來多少數量就做多少次
			...
			BeginPlot( headgear,"cl_105206_E", 0 )  ----執行20秒後將玩家傳出來的動作
			sleep ( 1 )
			...
			BeginPlot( rabbit , "cl_136_105208_E" , 0 )  ----執行兔子本身的SCRIPT
			sleep ( 1 )
		end
		...
		if Nobuff_play > 0 then
			...
			if In_magic > 0 then ----讀取到的數量大於0的話就執行
				...
				for i = 1 , In_magic , 1 do
					...
					WriteRoleValue ( headgear , EM_RoleValue_Register2 , AI ) ---在R2紀錄中控器的值
					BeginPlot( headgear,"cl_105206_E", 0 ) ---執行帽子在20秒後傳出玩家的動作
					sleep ( 1 )
					...
					BeginPlot( rabbit , "cl_136_105208_E" , 0 )
					sleep ( 1 )
				end
			end
		end
...
function cl_105206_E()  ----魔術帽25秒後把人船出來
	...
	SetPlot( headgear, "touch","cl_105206_touch_E", 40 ) ------PlotType 觸發類型 (分為"touch"觸碰、"range"範圍、"dead"死亡、"collision"碰撞四種)
	while 1 do
	sleep ( 10 )
		...
		if stage == 1 then
			skill = skill +1
			if skill >= 20 then
				cl_chbuff_117260_E() ----20秒後執行將玩家傳出來並且檢查BUFF的動作
				...
				BeginPlot( ball , "cl_136_105242ball_E" , 0 )
			end
		else
			delobj( headgear )
		end
...
function cl_105206_touch_E()  --觸碰劇情要讓玩家表演施法動作,然後讓小兔子到處亂跑cl_136_496883
	...
	if CheckBuff(  player , 508520 ) == true then
		if	BeginCastBarEvent( player , headgear , "[SC_13604_12]" , 20 , ruFUSION_ACTORSTATE_CROUCH_BEGIN , ruFUSION_ACTORSTATE_CROUCH_END,0 , "cl_136_117377_E") ~=1	then
...
function cl_136_117377_E(Executer,Result) ---嘗試著抓取兔子
	if	Result > 0 then --成功狀態
		EndCastBar(Executer,Result)--清除施法條
		BeginPlot( TargetID() , "cl_136_496883_E" , 0)
	else --失敗狀態
		EndCastBar(Executer,Result)--清除施法條
	end
end
function cl_136_105204_dead_E()  ---boss的死亡劇情
	local AI = ReadRoleValue ( OwnerID() , EM_RoleValue_PID ) --PID的值代表中控器
	WriteRoleValue ( AI , EM_RoleValue_Register1 , 2 )
end
function cl_136_496883_E()  ----施法結束後檢查請帽子執行將玩家傳送出來的動作
	local headgear= OwnerID()
	local player = TargetID()
	CancelBuff( player , 508520 )
	BeginPlot( headgear,"cl_chbuff_117260_E", 0 )
end
function cl_chbuff_117260_E() ---將玩家傳送出來~檢查身上有沒有BUFF沒有的話補一個變身法術給玩家
	local headgear = OwnerID()
	local RoomID = ReadRoleValue( headgear , EM_RoleValue_RoomID )
	local player = ReadRoleValue( headgear , EM_RoleValue_Register1 )
	local boss = ReadRoleValue( headgear , EM_RoleValue_PID )
	...
	local yy1 = 0
	local zz1 = 0
	local randbuff = { 508522 , 508603 , 509439 }
	local rbuff = 0
	xx = ReadRoleValue ( headgear, EM_RoleValue_X)
	yy = ReadRoleValue ( headgear, EM_RoleValue_Y)
	zz = ReadRoleValue ( headgear, EM_RoleValue_Z)   ---------這裡是捉出Attackplayer裡面的那個人的座標
	...
	sleep ( 5 )
	if CheckBuff(  player , 509440 ) == true then
		CancelBuff_NoEvent( player , 509440 ) ---清除BUFF
		sysCastSpelllv ( headgear , headgear , 496884 , 0 ) ---帽子自己表演一個
		--WriteRoleValue ( boss , EM_RoleValue_Register3 , 2 )
		sleep ( 5 )
	elseif CheckBuff(  player , 509440 ) == false then
...
function cl_105208_dead_E() ----兔子的死亡劇情要給玩家一個檢查的BUFF
	addbuff ( TargetID() , 509440 , 0 , -1 )
	delobj ( OwnerID() )
end
function cl_136_105209_AI2_E() ---BOSS離開戰鬥要傳到這裡
...
function cl_105209_chack_E() ---檢查BOSS是否距離超出房間避免BOSS追去打被傳入帽子內的玩家
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID ) ---宣告這個地圖~是我現在讀到的地圖
...
function cl_105209_P_E() ----檢查AI的stage的直是多少
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID ) ---宣告這個地圖~是我現在讀到的地圖
	local AI = ReadRoleValue( OwnerID() , EM_RoleValue_PID)  ------>我去讀取控制器裡面的PID值告訴BOSS王是控制器生出來的
	while 1 do
		sleep (5)
...
function cl_136_105208_E() ---兔子的AI
	...
	SetPlot( rabbit, "dead","cl_105208_dead_E", 0 )
	while 1 do
		sleep ( 10 )
		skill = skill + 1
	end
end
function cl_496924_E() --檢查範圍內有沒有帽子
	local boss = OwnerID() ----我叫做boss
	local AI = ReadRoleValue ( boss , EM_RoleValue_PID ) --PID的值代表中控器
...
function cl_136_496926_E()  ---抓六個人丟牌
	local boss = OwnerID() ----我叫做boss
	local RoomID = ReadRoleValue( boss , EM_RoleValue_RoomID ) ----這是AI讀取到的地圖
	local AI = ReadRoleValue ( boss , EM_RoleValue_PID ) --PID的值代表中控器
	local AI2 = ReadRoleValue ( boss , EM_RoleValue_Register2 ) ---我的Register2裡的值為AI2
	...
	if hset2 >= 1 then
		for i = 1 , hset2 , 1 do
			...
			local boss2 = CreateObj ( 105578 , x+rand[1] , y+30 , z+rand[2] , 0 , 1) --隨機字身周圍生怪
			...
			BeginPlot( boss2,"cl_136_hset2_table_E", 0 )
			sleep ( 1 )
		end
	end
end
function cl_136_hset2_table_E() ---分身對玩家施放紙牌技能
	local player = ReadRoleValue ( OwnerID() , EM_RoleValue_Register1 )
	CastSpelllv ( OwnerID() , player , 496926 , 0 )
	sleep ( 30 )
	delobj ( OwnerID() )
end
function cl_117377_touch_E()  --觸碰劇情要讓玩家表演施法動作,然後讓小兔子到處亂跑
	local player = OwnerID()
	local headgear = TargetID()
	local AI = ReadRoleValue( headgear , EM_RoleValue_PID )
	--CastSpelllv ( player , headgear , 496928 , 0 )
	if	BeginCastBarEvent( player , headgear , "[SC_13604_09]" , 20 , ruFUSION_ACTORSTATE_CROUCH_BEGIN , ruFUSION_ACTORSTATE_CROUCH_END,0 , "cl_136_496928_E") ~=1	then
		ScriptMessage( player, player, 1,"[SAY_NPC_BUSY]", 0 );		--目標忙碌中
	end
end
function cl_496928_E() ---施法結束後叫兔子自己執行劇情
	local headgear = OwnerID() --SC_13604_08
	local player  = TargetID()
	local AI = ReadRoleValue( headgear , EM_RoleValue_PID )
	BeginPlot( headgear , "cl_117377_rabbit_E" , 0)
end
function cl_117377_rabbit_E() --讓自己使用碰法術在變大然後刪掉自己
...
function cl_136_rabbit_E() ---抓到真的兔子才會變出BOSS來
	...
	local fiag = 0
	SetPlot( rabbit, "touch","cl_117377_touch_E", 20 ) ------PlotType 觸發類型 (分為"touch"觸碰、"range"範圍、"dead"死亡、"collision"碰撞四種)
...
function cl_136_496928_E(Executer,Result) ---嘗試著抓取兔子
	if	Result > 0 then --成功狀態
		local oo = GetDistance( OwnerID() , TargetID() );
		if oo <= 40 then ----------------------------------------------------------------------------------------------------------要修改回20
			EndCastBar(Executer,Result)--清除施法條
			BeginPlot( TargetID() , "cl_496928_E" , 0)
		else
...
function cl_136_opendoor_dead_E()
	local rabbit = OwnerID()
	local AI = ReadRoleValue ( rabbit , EM_RoleValue_PID )
