...
--控制對話內容--
	if ( ReadRoleValue(OwnerID(), EM_RoleValue_SkillMax_Herblism ) >= 80 )  then --檢查有大師以上資格的話
		SetSpeakDetail( OwnerID(), GetString ( "ST_110164_5" )   );--#你應該已經能接觸到這個自然的奧秘了吧？我衷心地替你感到高興。
	elseif ( ReadRoleValue( OwnerID() , EM_RoleValue_Skill_Herblism ) == 60 ) then  --等級60提示要玩家去黑曜石要塞
		SetSpeakDetail( OwnerID(), GetString ( "ST_110164_4" )   );--#你真不簡單，連海柏女士都在誇獎你呢！不過接下來要更上一層，就得去找尋傳說中的採藥大師，現在沒有人知道他在哪裡，只有黑曜石要塞好像還有一些他的傳言。
	elseif ReadRoleValue( OwnerID() , EM_RoleValue_Skill_Herblism ) >= 40  then --等級大於40則叫玩家去找另一個NPC
		SetSpeakDetail( OwnerID(), GetString ( "ST_110164_3" )   );--#你真的很優秀，雖然海柏女士有點嘮叨，但她的知識遠勝於我，我希望你能跟她好好學習。
	elseif(    ReadRoleValue( OwnerID() , EM_RoleValue_Skill_Herblism ) == 20 ) then  --等級等於20時判斷是否可接任務
		...
		SetSpeakDetail( OwnerID(), GetString("ST_110164_0")   );--/*我是採藥師娜塔，我很樂意與你分享關於藥草的知識。
	end

--控制對話選項--
	if( ReadRoleValue ( OwnerID(), EM_RoleValue_SkillMax_Herblism ) >= 40) then
		AddSpeakOption( OwnerID(), TargetID( ), GetString("SO_DEGRADE_DIALOG" ), "LuaS_110164_2", 0 ) --/*我想放棄目前的資格
	...
	if ( CheckFlag( OwnerID(), 540333) == true ) then --如果會採藥
		...
		AddSpeakOption( OwnerID(), TargetID( ), GetString("SO_110164_1") , "LuaS_110164_1", 0 );	--/*我想學習採藥。
	end

--常態對話選項--
	AddSpeakOption( OwnerID(), TargetID( ), GetString("SO_FORMULA_STORE") , "Lua_110164_RecipeList_Touch", 0 )  --/*我想看看你賣的配方。--讓不會的人也能看看能做什麼東西	
...
function LuaS_110164_2()
	...
	AddSpeakOption( OwnerID(), TargetID( ), GetString("SO_GETVOC_NO"), "LuaS_110164_0", 0 ); --/*我再考慮看看。
