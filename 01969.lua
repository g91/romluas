...
--控制對話內容--
	if ReadRoleValue( OwnerID() , EM_RoleValue_Skill_Mining ) == 80  then --等級小於60則叫玩家去找另一個NPC
		SetSpeakDetail( OwnerID(), "[ST_115960_2]"   );--#
	elseif(    ReadRoleValue( OwnerID() , EM_RoleValue_Skill_Mining  ) == 60 ) then  --等級等於60時判斷是否可接任務
		--###技能達到進階標準時加入任務###--
		if not tolimit  then
			LoadQuestOption( OwnerID() );
		else --告知玩家所學工作技能超過了
			SetSpeakDetail( OwnerID(), "[ST_115960_1]"   );--#
		end
		--################################--
	else --60~80級的對話
		SetSpeakDetail( OwnerID(), "[ST_115960_0]"   );		--/*
	end

--控制對話選項--
	if( ReadRoleValue ( OwnerID(), EM_RoleValue_SkillMax_Mining ) >= 40) then --擁有工匠以上資格則出現可以放棄資格的選項
		AddSpeakOption( OwnerID(), TargetID( ), GetString("SO_DEGRADE_DIALOG") , "LuaS_115960_1", 0 ) --/*降低目前資格
	end 
...
function LuaS_115960_1()
	...
	AddSpeakOption( OwnerID(), TargetID( ), GetString("SO_GETVOC_NO"), "LuaS_115960_0", 0 ); --/*我再考慮看看。
