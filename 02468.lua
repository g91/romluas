function ic_106015_01()  																	---第10口袋固定死亡劇情用--第9口袋固定測試距離用					--say(OwnerID(),"boss44="..boss44)														
		...
		local Boss=CreateObjByFlag(106015,780886,1,1) 											---BOSS創在  旗標1
		local door=CreateObjByFlag(106006,780886,2,1)											---阻擋門(王死才會消失)	509083
				...
				SetPlot( Boss,"dead","ic_106015_BOSSXX",0 )											---掛死亡劇情
		...
		while true do
			...
			if  HPPercent<0.6 and HPPercent>0.3 and timeboss==0 then
					...
					Boss=CreateObjByFlag(106016,780886,1,1)
			...
			elseif HPPercent>0 and HPPercent<0.3 and timeboss==1 then
					...
					DelObj(boss)  
					bosshp=ReadRoleValue(isaac , EM_RoleValue_register5)
					Boss=CreateObjByFlag(106017,780886,1,1)
					WriteRoleValue( Boss, EM_RoleValue_HP,bosshp)
					AddToPartition(Boss,RoomID)
					SetPlot( Boss,"dead","ic_106015_BOSSXX",0 )											---掛死亡劇情
					AddBuff(Boss,509424,0 ,-1 )
				...
				if hatelistcount(boss)~=0 then    												---讀仇恨人數
						...
						if time1>=30 then														---第1階段15秒生3隻小怪
							...
							elseif mobstop<6 then
								...
								if playKKboss==1 then											
										...
										mobkk1=bosskkpaly[1]
										mob1=CreateObjByFlag(106018,780886,Luck1+11,1)
										...
										SetPlot( mob1,"dead","ic_106015_mob1xx",0 )		
										BeginPlot(mob1, "ic_106015_mob_1" , 0)
										SetAttack(mob1 , mobkk1)
										sysCastSpellLV( mobkk1, mob1, 497235, 0 )
										mobstop=ReadRoleValue(isaac , EM_RoleValue_register7)
										WriteRoleValue(isaac,EM_RoleValue_register7,mobstop+1)
										sleep(5)
										mob2=CreateObjByFlag(106019,780886,Luck1+12,1)
										...
										SetPlot( mob2,"dead","ic_106015_mob2xx",0 )			
										BeginPlot(mob2, "ic_106015_mob_2" , 0)
										SetAttack(mob2 , mobkk1)
										sysCastSpellLV( mobkk1, mob2, 497235, 0 )
										mobstop=ReadRoleValue(isaac , EM_RoleValue_register7)
										WriteRoleValue(isaac,EM_RoleValue_register7,mobstop+1)	
										mob3=CreateObjByFlag(106020,780886,Luck1+13,1)
										...
										SetPlot( mob3,"dead","ic_106015_mob3xx",0 )		
										BeginPlot(mob3, "ic_106015_mob_3" , 0)
					...
					Boss100=Lua_DW_CreateObj("flag",104693,780886,1,0)							
						...
						BeginPlot(Boss100 , "ic_106015_100" , 0)
					...
					Boss1=CreateObjByFlag(106108,780886,71,1)
						...
						BeginPlot(Boss1 , "ic_106015_BOSS123" , 0)
					Boss2=CreateObjByFlag(106108,780886,72,1)
						...
						BeginPlot(Boss2 , "ic_106015_BOSS123" , 0)
					Boss3=CreateObjByFlag(106108,780886,73,1)
						...
						BeginPlot(Boss3 , "ic_106015_BOSS123" , 0)			
					end
				end	
				if hatelistcount(boss)==0 and open==1 or BOSS9898==1 and open==1 then			
					if ReadRoleValue( boss , EM_RoleValue_IsDead)==0 then   					
						...
						for i = 0 , NPCXX do
							ID=GetSearchResult()
							ID2=ReadRoleValue(ID,EM_RoleValue_OrgID)
							if ID2==106018 or  ID2==106019 or ID2==106020 or ID2==106108  then     
							DelObj(ID)
							end
						end
						...
						sleep(30)                                          
						Boss=CreateObjByFlag(106015,780886,1,1)           									
						AddToPartition(Boss,RoomID)	                               					
						SetPlot( Boss,"dead","ic_106015_BOSSXX",0 )								
						WriteRoleValue(isaac,EM_RoleValue_PID,Boss)
						WriteRoleValue(Boss,EM_RoleValue_PID,isaac)
						BeginPlot(isaac, "ic_106015_BUFFXX" , 0)								
						open=0																	
						killidtime=0
						time1=0
						time2=0
				...
				if BOSSXX==1  then															   ---無敵殺了王
						...
						WriteRoleValue(isaac,EM_RoleValue_register9,0)
						WriteRoleValue(isaac,EM_RoleValue_register10,0)
						BeginPlot(isaac, "ic_106015_BUFFXX" , 0)							   ---刪除BUFF
						open=0	
						killidtime=0
						BOSSXX=2
						time1=0
...
function ic_106015_mob_1()																	---法師
		...
		while 1 do
			...
			if HPPercent>0.1 then
				...
				if time1>=11 then
					CastSpellLV( OwnerID() , AttackTarget , 497229,2000)
					sleep(40)
					time1=0
				elseif time1>=0 then	
					CastSpellLV( OwnerID() , AttackTarget , 497233,500)	
					sleep(10)
				end
...
function ic_106015_mob_3()																	
		...
		while 1 do
			...
			if HPPercent>0.1 then
				...
				SetAttack(OwnerID() , AttackTarget)
				if time1>=11 then
					CastSpellLV( OwnerID() , AttackTarget , 497231,10000)
					sleep(20)
					time1=0
				elseif time1>=0 then	
					CastSpellLV( OwnerID() , AttackTarget , 497232,20)
					sleep(10)
				end	
			elseif HPPercent<=0.1  then
				SetModeEx( OwnerID()  , EM_SetModeType_Move, true )								
			...
			while 1 do
				...
				if HPPercent>0.6 then  
						...
						if time2>= 13 then
							if hatelistcount(boss)==1 then
								...
								CastSpellLV( OwnerID(), BossTarget, 497238,250)				
								sleep(30)
								time2=0
							elseif hatelistcount(boss)>=2 then	
								...
								CastSpellLV( OwnerID(), play1x[plays], 497238,250)			
								sleep(30)
								time2=0
							end
						end
						if time3>=7 then
							...
							CastSpellLV( OwnerID(), BossTarget, 497239,6)					
							sleep(10)
							time3=0
						end
				elseif HPPercent<0.6 and HPPercent>0.3 then				
					...
					elseif	time20==1 then
						...
						if time5>=4  then
							if hatelistcount(boss)==1 then
								CastSpellLV( OwnerID(), BossTarget, 497393,2500)				
								sleep(10)
								time5=0
							elseif hatelistcount(boss)>=2 then	
								play1x=DW_HateRemain(1)										
								plays=DW_Rand(table.getn(play1x))
								CastSpellLV( OwnerID(), play1x[plays], 497393,2500)			
								sleep(10)
								time5=0
							end
						end
						if time6>=0 then
							CastSpellLV( OwnerID(), OwnerID(), 497240, 2500 )					---旋風盾擊
							sleep(5)
							time6=0
						end
					end	
				elseif HPPercent<=0.3 then
					...
					if HPPercent>0  and HPPercent<0.3  and time3==0 and BOSSXX==0 or 
					   ...
					   HPPercent>0  and HPPercent<0.02 and time3==7 and BOSSXX==0   then
						...
						BeginPlot(mob991, "ic_106015_mob91" , 0)
						BeginPlot(mob992, "ic_106015_mob92" , 0)
						BeginPlot(mob993, "ic_106015_mob93" , 0)
						BeginPlot(mob994, "ic_106015_mob94" , 0)
						BeginPlot(mob995, "ic_106015_mob95" , 0)
						BeginPlot(mob996, "ic_106015_mob96" , 0)
						SetModeEx( Boss , EM_SetModeType_Fight, false) 						---砍殺
						CastSpellLV( OwnerID(),OwnerID(), 497242, 0 )						---
						sleep(70)
						SetModeEx( Boss , EM_SetModeType_Fight, true) 						---砍殺
						time3=time3+1
					end
					time1=time1+1
					time2=time2+1
						if time1>=15 then
							if hatelistcount(boss)==1 then
								CastSpellLV( OwnerID(), BossTarget, 497392,5)					---打%數
								sleep(10)
								time1=0	
							elseif hatelistcount(boss)>=2 then
								play1x=DW_HateRemain(1)											
