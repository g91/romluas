function ic_105224_01()   ------主控器
		...
		local Boss=CreateObjByFlag(105224,780775,1,1) 	-----BOSS創在  旗標1
		...
		SetPlot( Boss,"dead","ic_105224_44",0 )
		...
		while true do
			...
			if hatelistcount(boss)~=0 then    		---讀仇恨人數
				...
				if open==0 then
				 ...
				 Boss100=Lua_DW_CreateObj("flag",104693,780775,1,0)--測試距離用
					...
					BeginPlot(Boss100 , "ic_105224_100" , 0)---測試距離			
				...
				door1=CreateObjByFlag(104198,780775,4,1)-------王身後阻擋門
					...
					BeginPlot(boss , "ic_105224_02" , 0)				------命令角色做事情
				end
			end	
			if hatelistcount(boss)==0 and open==1 or BOSS9898==1 and open==1 then			--開戰後仇恨表沒人
				if ReadRoleValue( boss , EM_RoleValue_IsDead)==0 then   --王沒死 玩家滅團
					...
					sleep(30)                                           --5秒
					Boss=CreateObjByFlag(105224,780775,1,1)           --再創造1之				
					AddToPartition(Boss,RoomID)	                                ---創造王 必要		
					SetPlot( Boss,"dead","ic_105224_44",0 )	
					WriteRoleValue(Ctrl,EM_RoleValue_PID,Boss)
					WriteRoleValue(Boss,EM_RoleValue_PID,Ctrl)
					BeginPlot(OwnerID() , "ic_105224_04" , 0)
			...
			if killidtime1==1 and play44==0 then
				...
				time22=0
					if time10==5 then
						...
						if hatelistcount(boss)==1 then
							...
							if CheckBuff(BossTarget, 507911 ) == true then		
								mob11=CreateObj(105225,X0,Y0,Z0,0,1)-------生隱形球
								...
								AddBuff( mob11,506822,11 ,-1 )
								WriteRoleValue( mob11 , EM_RoleValue_register1 , BossTarget )
								SetPlot( mob11,"dead","ic_105224_06",0 )  
								BeginPlot(mob11 , "ic_105224_05" , 0)
							end	
							end	
					...
					if time3>=11 then--範圍擊飛--第1仇恨 仇恨歸0(暫時)
						...
						if hatelistcount(boss)==1 then
							...
							local MessageString = "[SC_104605_08][$SETVAR1="..PlayerName.."]" 		----在string中可以秀出玩家的名字
									  ScriptMessage( OwnerID() , -1 , 2 , MessageString , 1 )
							CastSpellLV( OwnerID() , BossTarget , 496362 , 180 )
							sleep(20)
							time3=0
						elseif hatelistcount(boss)>1 then
							play1x=DW_HateRemain(1)-----------------------隨機打--不打坦
							...
							local MessageString = "[SC_104605_08][$SETVAR1="..PlayerName .."]" 		----在string中可以秀出玩家的名字
									  ScriptMessage( OwnerID() , -1 , 2 , MessageString , 1 )
							CastSpellLV( OwnerID() , play1x[plays] , 496362 , 180 )
							sleep(30)
							time3=0
						end	
					end
					if time2>=5 then--單體技能--第1仇恨
						...
						CastSpellLV( OwnerID() , BossTarget , 496361, 500 )----單體技能
						sleep(10)
						time2=0
					end
				end
				...
				time33=0 
					if time5==0 then--轉階段放1次
						...
						sleep(10)
						CastSpellLV( OwnerID() , OwnerID() , 496250, 7 )----全場DOT
						time5=1
					end
					if time22==5 then
							if hatelistcount(boss)==1 then
					...
					while 	ReadRoleValue(OwnerID() , EM_RoleValue_SpellMagicID  ) ~= 0   do--------有沒有正在施放技能
						...
						CastSpellLV( OwnerID() , OwnerID() , 496250,9)----全場DOT
						time7=0
					end
					if time8>=11 then---全體恐懼
						...
						sleep(10)
						CastSpellLV( OwnerID() , OwnerID() , 496249,9)----隨機3人跑吧
						sleep(10)
						time8=0
					end
				end
				if HPPercent <0.60 then--兄弟一起來
					...
					if time44==0 then--鐵處女---隨機1人
						...
						CastSpellLV( OwnerID() , OwnerID() , 496367,9)----全場DOT
						sleep(10)	
						while 	ReadRoleValue(OwnerID() , EM_RoleValue_SpellMagicID  ) ~= 0   do--------有沒有正在施放技能
							sleep(5)
						end
					...
					if time4>=17 then--針刺--隨機1人
						if hatelistcount(boss)==1 then	
							...
							ScriptMessage( OwnerID(), -1 , 2 , "[SC_104605_09]" , 2 )----恐懼 畏懼 該讓你們感受看看了
							sleep(10)
							CastSpellLV( OwnerID() , OwnerID() , 496249,2)----隨機3人跑吧
							sleep(10)
							while 	ReadRoleValue(OwnerID() , EM_RoleValue_SpellMagicID  ) ~= 0   do--------有沒有正在施放技能
							sleep(5)
							end
				...
				if time33>=10 then---單體技能
					while 	ReadRoleValue(OwnerID() , EM_RoleValue_SpellMagicID  ) ~= 0   do--------有沒有正在施放技能
						sleep(5)
					end
						CastSpellLV( OwnerID() , BossTarget , 496361,100)----單體技能
						sleep(10)
						time33=0
					end	
				end
end
	function ic_105224_04()----刪除BUFF用
		local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)
		...
		for i=1,TempPlayer do
			...
			if CheckID( ThesePlayer[i] ) == true and 
				...
				ReadRoleValue( ThesePlayer[i] , EM_RoleValue_IsPlayer ) == 1 then
					--檢查玩家是不是還在；檢查是否死亡
				if CheckBuff( ThesePlayer[i] , 508686 ) == true --拷問
				or CheckBuff( ThesePlayer[i] , 507911 ) == true --鐵處女
				or CheckBuff( ThesePlayer[i] , 507907 ) == true --鐵反彈
				or CheckBuff( ThesePlayer[i] , 508687 ) == true --凌遲
				or CheckBuff( ThesePlayer[i] , 508381 ) == true	then --畏懼
					CancelBuff(ThesePlayer[i], 508686)
					CancelBuff(ThesePlayer[i], 507911)
					CancelBuff(ThesePlayer[i], 507907)
					CancelBuff(ThesePlayer[i], 507934)
					CancelBuff(ThesePlayer[i], 508687)
					CancelBuff(ThesePlayer[i], 508381)
				end
			end					
		end
	end
	function ic_105224_05()
		local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)
		...
		while 1 do
			...
			if time1==0 then
				CastSpellLV( OwnerID() , player , 496368,15)
				WriteRoleValue(  OwnerID() , EM_RoleValue_register4 ,11)
