	...
	local AttackTarget = ReadRoleValue( OwnerID() , EM_RoleValue_AttackTargetID ) --讀正在攻擊的目標
		if  AttackTarget ~= 0  and ( CheckID( OwnerID() ) ) == true then	--如果目標人數>0，而且自己存在時，判定進入戰鬥
			...
			WriteRoleValue( CH1, EM_RoleValue_Register2, 1) --回傳控制器說已經在戰鬥中了
				...
				if Time02 == 26 then --檢查王身上有無九宮格檢查
					...
					elseif CheckBuff( OwnerID() ,  508612 ) == false then --如果沒有的話，執行以下事情
						if Boss25 == 1 then --施放藍色未點傷害
							...
							CastSpellLV( OwnerID() , OwnerID(), 496986 , 4 )
						...
						elseif Boss25 == 2 then --施放紫色未點傷害
							...
							CastSpellLV( OwnerID() , OwnerID() , 497407 , 4 )
							--CancelBuff( TargetID() ,  508607 ) ----刪除九宮格檢查
						...
						elseif Boss25 == 3 then --施放黃昏未點傷害
							...
							CastSpellLV( OwnerID() , OwnerID() , 497408 , 4 )
							--CancelBuff( TargetID() ,  508607 ) ----刪除九宮格檢查
				...
				if Time02 == 25 then --射鏢靶點盒子
					...
					BL = Rand(3)
						...
						SetPlot( bluebox ,"touch","Dy_Z19_BOSS1_BLUE", 50 ) --塞入藍盒子觸碰劇情
					...
					PL = Rand(3)
						...
						SetPlot( redbox ,"touch","Dy_Z19_BOSS1_RED", 50 ) --塞入紅盒子觸碰劇情
					...
					RL = Rand(3)
						...
						SetPlot( yellowbox ,"touch","Dy_Z19_BOSS1_YELLOW", 50 ) --塞入黃盒子觸碰劇情
				...
				if Time01 == 7 then --施放普招
					...
					if Luck >= 0 and Luck <50 then --施放隨機3人飛刀傷害
						...
						if choose_p > 3 then
							...
							for i = 1 , 3 , 1 do 
								...
								if ( player[Lucky2] ~= nil ) and ( ReadRoleValue( player[Lucky2] , EM_RoleValue_IsDead ) == 0 ) then
									SysCastSpellLv( OwnerID() , player[Lucky2] , 496861 , 0 )
								end
						...
						elseif choose_p <=3 then
							...
							for i = 1 , table.getn(player) , 1 do
								sysCastSpellLV( OwnerID() , player[i] , 496861 , 0  )
								--say(player[i] , "<=3")
							end
						end
					elseif Luck >50  and Luck <=75 then --施放自身AE80
						...
						CastSpellLV( OwnerID() , OwnerID() , 496864 , 25 )
						sleep(30)
						--say( OwnerID(), "AE80")
					elseif Luck > 75 and Luck <=100 then --施放前方扇形傷害
						...
						CastSpellLV( OwnerID() , OwnerID() , 496865 , 50 )
						--say( OwnerID(), "Fan60")
					end
				Time01 = 0
				end
			...
			if FightON == 1 and  AttackTarget == 0 then --如果在戰鬥中而沒有任何目標的話
				...
				BeginPlot( Playbye, "Dy_Z19_BOSS1_DEAD_2", 10)
				Time01 = 0
				Time02 = 0
				Time03 = 0
...
function Dy_Z19_BOSS1_BLUE() --藍盒子的碰撞劇情 
	...
	elseif CheckBuff( OwnerID() , 508608) == false then --沒有時在執行以下檢查
		...
		elseif CheckBuff( OwnerID() , 508604) == false then
			...
			elseif CheckBuff( OwnerID() , 509321) == false then
				sysCastSpellLV( TargetID() , OwnerID() , 496858 , 0 ) --施放點錯的法術 -80%HP
			end
		end
	end
end

function Dy_Z19_BOSS1_RED() --紫盒子的碰撞劇情
	if CheckBuff( OwnerID() , 508608) == true then ---有恐懼Bff時什麼都不做
	elseif CheckBuff( OwnerID() , 508608) == false then --沒有時在執行以下檢查
...
function Dy_Z19_BOSS1_YELLOW() --黃昏盒子的碰撞劇情
	if CheckBuff( OwnerID() , 508608) == true then ---有恐懼Bff時什麼都不做
	elseif CheckBuff( OwnerID() , 508608) == false then --沒有時在執行以下檢查
...
--組合A組
function Dy_Z19_BOSS1_DQ1_B_A() --走到藍色方形刪除紫與黃昏Debuff
	if CheckBuff( TargetID() ,  509172 ) == true then
		CancelBuff( TargetID() ,  509172 ) --刪除藍色B-B
	else
	
	end
	if CheckBuff( TargetID() ,  509174 ) == true then
		CancelBuff( TargetID() ,  509174  ) --刪除藍色B-C
		CancelBuff( TargetID() ,  509175  ) --刪除藍色B-C
	else
	
	end
	if CheckBuff( TargetID() ,  509167 ) == true then
		CancelBuff( TargetID() ,  509167 ) --刪除紫色P-A
	else
	
	end
	if CheckBuff( TargetID() ,  508924 ) == true then
		CancelBuff( TargetID() ,  508924 ) --刪除紫色P-B
	else
	
	end
	if CheckBuff( TargetID() ,  509169 ) == true then
		CancelBuff( TargetID() ,  509169 ) --刪除紫色P-C
		CancelBuff( TargetID() ,  509170 ) --刪除紫色P-C
	else
	
	end
	if CheckBuff( TargetID() ,  509165 ) == true then
		CancelBuff( TargetID() ,  509165 ) --刪除黃昏R-A
	else
	
	end
	if CheckBuff( TargetID() ,  509163 ) == true then
		CancelBuff( TargetID() ,  509163 ) --刪除黃昏R-B
	else
	
	end
	if CheckBuff( TargetID() ,  508926 ) == true then
		CancelBuff( TargetID() ,  508926 ) --刪除黃昏R-C
		CancelBuff( TargetID() ,  508957 )
	else
	
	end

end
function Dy_Z19_BOSS1_DQ1_P_A() --走到紫色方形刪除藍與黃昏Debuff
	if CheckBuff( TargetID() ,  508958 ) == true then
		CancelBuff( TargetID() ,  508958 ) --刪除藍色B-A
	else
...
function Dy_Z19_BOSS1_DQ1_R_A() --走到黃昏方形刪除紫與藍Debuff
	if CheckBuff( TargetID() ,  508958 ) == true then
		CancelBuff( TargetID() ,  508958 ) --刪除藍色B-A
	else
...
--組合B組
function Dy_Z19_BOSS1_DQ1_B_B() --走到藍色方形刪除紫與黃昏Debuff
	if CheckBuff( TargetID() ,  508958 ) == true then
		CancelBuff( TargetID() ,  508958 ) --刪除藍色B-A
	else
...
function Dy_Z19_BOSS1_DQ1_P_B() --走到紫色方形刪除藍與黃昏Debuff
	if CheckBuff( TargetID() ,  508958 ) == true then
		CancelBuff( TargetID() ,  508958 ) --刪除藍色B-A
	else
...
function Dy_Z19_BOSS1_DQ1_R_B() --走到黃昏方形刪除紫與藍Debuff
	if CheckBuff( TargetID() ,  508958 ) == true then
		CancelBuff( TargetID() ,  508958 ) --刪除藍色B-A
	else
...
--組合C組
function Dy_Z19_BOSS1_DQ1_B_C() --走到藍色方形刪除紫與黃昏Debuff
	if CheckBuff( TargetID() ,  508958 ) == true then
		CancelBuff( TargetID() ,  508958 ) --刪除藍色B-A
	else
...
function Dy_Z19_BOSS1_DQ1_P_C() --走到紫色方形刪除藍與黃昏Debuff
	if CheckBuff( TargetID() ,  508958 ) == true then
		CancelBuff( TargetID() ,  508958 ) --刪除藍色B-A
	else
...
function Dy_Z19_BOSS1_DQ1_R_C() --走到黃昏方形刪除紫與藍Debuff
	if CheckBuff( TargetID() ,  508958 ) == true then
		CancelBuff( TargetID() ,  508958 ) --刪除藍色B-A
	else
...
function Dy_Z19_BOSS1_DQ1_Check() --檢查屍體與GMssssssssssss
...
function Dy_Z19_BOSS1_25Check() --施放-80%HP的檢查
...
function Dy_Z19_BOSS1_25DEL() ----刪除九宮格檢查
	CancelBuff( TargetID() ,  508607 ) ----刪除九宮格檢查
	--say( TargetID() , "123")
end