...
local time3=0
local time4=0
	boss1=CreateObjByFlag(105731,780886,6,1)												---中間   EM_SetModeType_DisableRotate
	...
	if boss3xx==1 and time3>=2 or boss4xx==1 and time3>=2 or boss5xx==1 and time3>=2 or boss6xx==1 and time3>=2 then
		time2=1
		if boss3xx==1 then
			boss3=CreateObjByFlag(105703,780886,93,1)
			...
			BeginPlot(boss3 , "ic_780937_boss35" , 0)
			WriteRoleValue(isaac,EM_RoleValue_register3,0)
		end
		if boss4xx==1 then
			boss4=CreateObjByFlag(105702,780886,94,1)
			...
			BeginPlot(boss4 , "ic_780937_boss46" , 0)
			WriteRoleValue(isaac,EM_RoleValue_register4,0)
		end
		if boss5xx==1 then
			boss5=CreateObjByFlag(105703,780886,95,1)
			...
			BeginPlot(boss5, "ic_780937_boss35" , 0)
			WriteRoleValue(isaac,EM_RoleValue_register5,0)
		end
		if boss6xx==1 then
			boss6=CreateObjByFlag(105702,780886,96,1)
			...
			BeginPlot(boss6 , "ic_780937_boss46" , 0)
	...
	if boss1xx==0 then	
		if time4==0 and mob9898==1 then
			...
			boss7=CreateObjByFlag(105681,780886,97,1)
			...
			BeginPlot(boss7, "ic_780937_boss78" , 0)
			boss8=CreateObjByFlag(105681,780886,98,1)
			...
			BeginPlot(boss8, "ic_780937_boss78" , 0)
			time4=1
		end
		if mob9898==1 and time0==0 then
			...
			SetModeEx( mob1  , EM_SetModeType_Move, true )									--移動
				time0=1
				BeginPlot(mob1 , "ic_780937_mob1" , 0)
				sleep(5)
				---sysCastSpellLV( mob1 , mob1 , 497656  , 5 )
				sleep(15)
			...
			SetModeEx( mob2  , EM_SetModeType_Move, true )									--移動
				time0=1
				BeginPlot(mob2 , "ic_780937_mob2" , 0)
				sleep(5)
				---sysCastSpellLV( mob2 , mob2 , 497656  , 5 )
				sleep(15)
			...
			SetModeEx( mob3  , EM_SetModeType_Move, true )									--移動
				time0=1
				BeginPlot(mob3 , "ic_780937_mob3" , 0)
				sleep(5)
				---sysCastSpellLV( mob3 , mob3 , 497656  , 5 )
				sleep(15)
	...
	elseif boss1xx==1 then
		...
		if mob9898==1 and time0==0 then
			...
			SetModeEx( mob1  , EM_SetModeType_Move, true )									--移動
				time0=1
				BeginPlot(mob1 , "ic_780937_mob1_1" , 0)
				sleep(5)
				---sysCastSpellLV( mob1 , mob1 , 497656  , 5 )
				sleep(15)
			mob1=CreateObjByFlag(105651 ,780886,1,1)												
			...
			SetModeEx( mob2  , EM_SetModeType_Move, true )									--移動
				time0=1
				BeginPlot(mob2 , "ic_780937_mob2_1" , 0)
				sleep(5)
				---sysCastSpellLV( mob2 , mob2 , 497656  , 5 )
				sleep(15)
			mob2=CreateObjByFlag(105651 ,780886,2,1)
			...
			SetModeEx( mob3  , EM_SetModeType_Move, true )									--移動
				time0=1
				BeginPlot(mob3 , "ic_780937_mob3_1" , 0)
				sleep(5)
				---sysCastSpellLV( mob3 , mob3 , 497656  , 5 )
				sleep(15)
			mob3=CreateObjByFlag(105651 ,780886,3,1)
...
function ic_780937_BOSS1()	
	...
	if HPPercent<0.5 then
		SetIdleMotion( OwnerID() ,ruFUSION_ACTORSTATE_ACTIVATE_BEGIN)
		SetModeEx( OwnerID() , EM_SetModeType_Obstruct, false) 								---阻擋(是)
		WriteRoleValue(isaac,EM_RoleValue_register10,1)
		SetModeEx( OwnerID() , EM_SetModeType_ShowRoleHead, false)
		SetModeEx( OwnerID() , EM_SetModeType_Mark, false)									---可點選(否)
		SetModeEx( OwnerID() , EM_SetModeType_NotShowHPMP, false)							---可點選(否)
		sleep(50)
		break
	end
	end
end
function ic_780937_BOSS3XX()																	
	local isaac=ReadRoleValue(OwnerID(),EM_RoleValue_PID)	
		WriteRoleValue(isaac,EM_RoleValue_register3,1)
end
function ic_780937_BOSS4XX()																	
	local isaac=ReadRoleValue(OwnerID(),EM_RoleValue_PID)	
		WriteRoleValue(isaac,EM_RoleValue_register4,1)
end
function ic_780937_BOSS5XX()																	
	local isaac=ReadRoleValue(OwnerID(),EM_RoleValue_PID)	
		WriteRoleValue(isaac,EM_RoleValue_register5,1)
end
function ic_780937_BOSS6XX()																	
	local isaac=ReadRoleValue(OwnerID(),EM_RoleValue_PID)	
		WriteRoleValue(isaac,EM_RoleValue_register6,1)
end
function ic_780937_boss35()	--射手
...
local time1=0
	while 1 do
		if hatelistcount(OwnerID())~=0 then 
			...
			sleep(10)
			CastSpellLV( OwnerID() , AttackTarget , 497650, 30 )
		elseif hatelistcount(OwnerID())==0 then
			sleep(10)
		end	
	end
end
function ic_780937_boss46()	--法師
	local AttackTarget = ReadRoleValue(OwnerID(), EM_RoleValue_AttackTargetID )    		 ---找尋目前的攻擊目標
	local time1=0
	while 1 do
		if hatelistcount(OwnerID())~=0 then 
			AttackTarget = ReadRoleValue(OwnerID(), EM_RoleValue_AttackTargetID )
			sleep(10)
			CastSpellLV( OwnerID() , AttackTarget , 497651, 600 )
		elseif hatelistcount(OwnerID())==0 then
			sleep(10)
		end	
	end
end
function ic_780937_boss78()	--補師
local time1=0
local p1p2=0
local boss1 = ReadRoleValue(OwnerID(), EM_RoleValue_PID )    		 ---找尋目前的攻擊目標
	while 1 do
		...
		if p1p2==0 then
			CastSpellLV( OwnerID() , boss1 , 497657,4)
			sleep(20)
		elseif	p1p2==1 then
			CastSpellLV( OwnerID() , OwnerID() , 497658,4)
			sleep(20)
		end
	end
end
function ic_780937_boss1234()	--門神
	local AttackTarget = ReadRoleValue(OwnerID(), EM_RoleValue_AttackTargetID )    		 ---找尋目前的攻擊目標
	local MaxHP = ReadRoleValue( OwnerID() , EM_RoleValue_MaxHP )							 ---讀取血量
	local NowHP = ReadRoleValue( OwnerID() , EM_RoleValue_HP )     						 ---讀取目前血量
	local HPPercent = NowHP / MaxHP	
		...
		if hatelistcount(OwnerID())~=0 then 
			if HPPercent>0.6 then
				...
				if time2>=20 then
					...
					CastSpellLV( OwnerID() , OwnerID() , 497652, 199 )
					sleep(50)
			...
			elseif HPPercent<0.6 then
				...
				if time2>=20 then
					...
					CastSpellLV( OwnerID() , OwnerID() , 497652, 199 )
					sleep(50)
					CastSpellLV( OwnerID() , OwnerID() , 497653, 1 )
					time2=0
				end
				if time3>=10 then
					while 	ReadRoleValue(OwnerID() , EM_RoleValue_SpellMagicID  ) ~= 0   do--------有沒有正在施放技能
					...
					CastSpellLV( OwnerID() , OwnerID() , 497654, 3 )
					sleep(20)
					time3=0
				end	
			end
...
function ic_780937_boss1234_3()	--門神~斜坡顧門用
	...
	SetPlot( OwnerID(),"dead","ic_780937_boss1234_33",0 )		
	while 1 do
		AttackTarget = ReadRoleValue(OwnerID(), EM_RoleValue_AttackTargetID )
		MaxHP = ReadRoleValue( OwnerID() , EM_RoleValue_MaxHP )						 		---讀取血量
		NowHP = ReadRoleValue(  OwnerID() , EM_RoleValue_HP )     					 		---讀取目前血量
...
function ic_780937_boss1234_33()
...
function ic_780937_mob1()																		
local Luck=0	
	Luck=DW_Rand(3)
	if Luck==1 then
		DW_MoveToFlag( OwnerID() , 780886 ,11,0)
		SetPlot( OwnerID(), "range","ic_780937_mob123", 30 )
...
local mob10 = star_CreateObj_NoAdd( mob , 105704 , 0 , 0 , 0 , Room , 0 ) 			----小怪死掉生球
	if ReadRoleValue( OwnerID() , EM_RoleValue_IsDead ) == 0 then
		...
		CastSpellLv( mob10 , mob10 , 497655 , 7 )
	end	
end