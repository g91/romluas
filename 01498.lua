...
function LuaI_102430_0()
	...
	for i = 1 , 2 , 1 do
		BeginPlot( Door[i] , "LuaI_102430_CheckBoss" , 0 )
	end

	BeginPlot( Door[3] , "LuaI_102430_BlockCheck" , 0 )
	-------- 執行流程控制迴圈 ------------------------------------
	BeginPlot( OwnerID() , "LuaAI_102430_Battle" , 0 )
end
--------------------------------- 王的死亡劇情 ------------------------------------------
function LuaI_102430_Dead()
	---死亡紀錄
	local PlayerLV = ReadRoleValue( TargetID() , EM_RoleValue_LV )
	local PlayerSUBLV = ReadRoleValue( TargetID() , EM_RoleValue_LV_SUB )
	DesignLog( OwnerID() , 102430 , " Killer Lv is "..PlayerLV.." and SubLv is "..PlayerSUBLV )
	---清除第二階段的 Buff 
...
--------------------------------- 王的戰鬥流程控制 AI --------------------------------------
function LuaAI_102430_Battle()

	local Step = 1
	local Count = 0
	local State = "OutBattle"
	local GuardianRanger = {102432,102433,102434,102435,102436,102437} --小怪的 ID
	local Rangers = {} -- 6 隻小怪
	...
	while 1 do
		...
		if HateListCount( OwnerID() ) > 0 then
			...
			if State == "OutBattle" then
				...
				Slayer = LuaAI_102430_Slayer_0( RoomID )
				State = "InBattle"
			end
			-------------------------------第一階段--------------------------------------------------------
			if Step == 1 then 
				...
				if Count == 5 then
					...
					for i = 1 , 3 , 1 do
						...
						BeginPlot( Rangers[i] , "LuaAI_102430_Mobs_Check" , 0 )
						SetPlot( Rangers[i] , "dead" , "LuaAI_102430_StepOne_Dead" , 0 )
			...
			if Step == 2 then
				...
				for i = 1 , 3 , 1 do
					...
					BeginPlot( TriBall[i] , "LuaAI_102430_BallAround" , 0 )
					SetPlot( TriBall[i] , "dead" , "LuaAI_102430_BallDead" , 0 )
				end
				---------------------------------- 隨機對所有玩家上 buff -------------------------------------
				local PPL = SetSearchAllPlayer(RoomID )
				local TempPPL = {}
			...
			if Step == 3 then
				...
				if Count == 0 or Count == 20 or Count == 40 then
					...
					if Num > 1 then
						if Lua_CheckObj_Exist( TempBall[Num-1] , 102431 , 0 , RoomID ) == true then
							DelObj( TempBall[Num-1] )
						end
						if Lua_CheckObj_Exist( TempBall[Num-1] , 102663  , 0 , RoomID ) == true then
							DelObj( TempBall[Num-1] )
						end
						if Lua_CheckObj_Exist( TempBall[Num-1] , 102664 , 0 , RoomID ) == true then
							DelObj( TempBall[Num-1] )
						end
					end
				end
			...
			if Count >= 60 then
				...
				if Step == 3 then
					for pry , obj in pairs(TempBall) do
						if Lua_CheckObj_Exist( obj , 102431 , 0 , RoomID ) == true then
							DelObj( obj )
						end
						if Lua_CheckObj_Exist( obj , 102663 , 0 , RoomID ) == true then
							DelObj( obj )
						end
						if Lua_CheckObj_Exist( obj , 102664 , 0 , RoomID ) == true then
							DelObj( obj )
						end
					end
...
-------------------------------- 小兵死後如果死在正確位置產生光圈的函式 -----------------------------------------------
function LuaAI_102430_StepOne_Dead()
	local Boss = ReadRoleValue( OwnerID() , EM_RoleValue_PID )
	local RoomID = ReadRoleValue( OwnerID() , EM_Rolevalue_RoomID )
	local Range = 100
	if Lua_CheckObj_Exist( Boss , 102430 , 1 , RoomID ) == true and ReadRoleValue( Boss , EM_RoleValue_PID ) == 1 then
		local FlagCombo = {[102432] = 1,[102433] = 2,[102434] = 3 ,[102435] = 4 ,[102436] = 5 ,[102437] = 6} 
		...
		if ( (X-Me:X())^2+(Z-Me:Z())^2 ) <= Range^2 then
			...
			BeginPlot( Auro , "LuaAI_102430_Auro_Check" , 0 )
		end
	end
...
------------------------------ 光環消除玩家身上 Buff 的法術 Script -----------------------------------------------------
function LuaM_504146_0()
	BeginPlot( TargetID() , "LuaM_504146_1" , 0 )
end

function LuaM_504146_1()
	local OrgID = ReadRoleValue( TargetID() , EM_RoleValue_OrgID )
	local BuffTable = {[102454] = 504139 , [102455] = 504140 , [102456] = 504141 , [102457] = 504142 , [102458] = 504143 , [102459] = 504144 }
	if CheckBuff( OwnerID() , BuffTable[OrgID] ) == true then
		if CancelBuff_NoEvent( OwnerID() , BuffTable[OrgID] ) then
			ScriptMessage( OwnerID()  , OwnerID()  , 1 , "[SC_102430_6]" , 0 ) --你體內的能量被結界所吸收了
			ScriptMessage(  OwnerID()  , OwnerID()   , 0 , "[SC_102430_6]" , 0 )
		end
	end
end
------------------------------ 第二階段王把玩家身上的 Buff 消掉的 Script ----------------------------------------------
function LuaM_504147_0()
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	local PPL = SetSearchAllPlayer(RoomID )
	local StepTwoBuff = { 504139 , 504140 , 504141 , 504142 , 504143 , 504144 }
	for i = 1 , PPL , 1 do
...
------------------------------ 玩家身上造成傷害的檢查 Script ------------------------------------
function LuaM_504148_Check()
	if ReadRoleValue( TargetID() , EM_RoleValue_ZoneID ) ~= 117 and ReadRoleValue( TargetID() , EM_RoleValue_ZoneID ) ~= 123 then
		return false
	end
...
-------------------------- 控制光圈消失的 AI ---------------------------------------------------
function LuaAI_102430_Auro_Check()
	local Boss = ReadRoleValue(OwnerID() , EM_RoleValue_PID )
	local RoomID = ReadRoleValue( OwnerID()  , EM_RoleValue_RoomID )

	while 1 do
		if Lua_CheckObj_Exist( Boss , 102430 , 1 , RoomID ) == false then
			break
		end
...
-------------------------- 控制最後的門開關 ------------------------------------------------------
function LuaI_102430_CheckBoss() --最後的門檢查王是否已經死了
	local Boss = ReadRoleValue(OwnerID() , EM_RoleValue_PID )
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	while 1 do
		if Lua_CheckObj_Exist( Boss , 102430 , 1 , RoomID ) == false then
...
------------------------------ 控制阻擋開關的 AI --------------------------------------------------
function LuaI_102430_BlockCheck()
	local Boss = ReadRoleValue(OwnerID() , EM_RoleValue_PID )
...
---------------------------------- 阻擋機械人的 AI ----------------------------------------------------
function LuaAI_102430_Slayer_0( RoomID )
	local Slayer = {}
	...
	for i = 1 , 4 , 1 do 
		...
		BeginPlot( Slayer[i] , "LuaAI_102430_Slayer_1" , 0 )
	end

	return Slayer
end
--------------------------- 檢查王是否存在或離開戰鬥的 AI -------------------------------------------
function LuaAI_102430_Mobs_Check()
	local Boss = ReadRoleValue(OwnerID() , EM_RoleValue_PID )
	local RoomID = ReadRoleValue( OwnerID()  , EM_RoleValue_RoomID )
...
------------------------------ 控制消滅者的 AI ---------------------------------------------------------
function LuaAI_102430_Slayer_1()
...
------------------------------ 檢查某物件是否存在的通用函式 -----------------------------------------------
function Lua_CheckObj_Exist( Obj , OrgID , DeadCheck , RoomID )
	if RoomID == nil or Type( RoomID ) ~= Number or RoomID < 0 then
		RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	end

	if CheckID( Obj ) == false or ReadRoleValue( Obj, EM_RoleValue_RoomID )~= RoomID or ReadRoleValue( Obj ,EM_RoleValue_OrgID ) ~= OrgID then
		return false
	end
...
--------------------------------------- 第三階段球的控制 AI -----------------------------------------------
function LuaAI_102430_BallAround()
	...
	-------------------- 控制圓周運動的迴圈 ------------------------------------
	while 1 do
		if Lua_CheckObj_Exist( Boss , 102430 , 1 , RoomID ) == false or HateListCount( Boss ) < 1 then
			Delobj( OwnerID() )
			return
		end
	...
	------------------- 執行檢查 boss 狀態的回圈 -------------------------------
	BeginPlot( OwnerID() , "LuaAI_102430_Mobs_Check" , 0 )
end
-------------------------- 球死亡後同時消全場與自己有關的 Buff ----------------
function LuaAI_102430_BallDead()
	local FlagNum = ReadRoleValue( OwnerID() ,EM_RoleValue_Register )
	local AllAreaBuff = {504173 , 504175 , 504177 }
