function bk_132_104095()---------------------------------------133RAID王
...
--	AddToPartition( afdoor , RoomID )
	local boss = CreateObjByFlag(104094,780687,2,1)------------------------------創立王
	...
	--		if gatime >= 600 then 
		...
		elseif x==2 then --玩家死亡王離開戰鬥
			...
			BeginPlot( boss ,"bk_132_104094",0)---------------------------------王劇情模式
			SetPlot( boss , "dead" , "bk_132_104094_dead" , 10 )
			x=0
			DelObj(door)
		...
		if HateListCount( boss ) ~= 0 then	---- 確定他現在有打人
			...
			--	say(ownerid(),"xxx"..mobt1)
				...
				---	say(ownerid(),"xxx"..Num)
					...
					magic=DW_Rand(4)
					if magic==1 then 
						BeginPlot( magi ,"bk_133_104093_mfin_ea",0)---------------------------------法陣劇情
					elseif magic==2 then 
						BeginPlot( magi ,"bk_133_104093_mfin_ea",0)---------------------------------法陣劇情
					elseif magic==3 then 
						BeginPlot( magi ,"bk_133_104093_mfin_ea",0)---------------------------------法陣劇情
					...
					CastSpellLV( OwnerID() , OwnerID() , 495871 , 1 )---------------雷電交加   
					sleep(60)
					...
					ScriptMessage( OwnerID() , -1 , 2 , "[SC_104090_03]" , 0 )
					sleep(20)
					CastSpellLV( OwnerID() , OwnerID() , 496141 , 1 )-----------雷霆震擊
					sleep(40)	
