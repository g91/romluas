--------------------------------------------------------------------------------主控制器
function Cang_130_tree_control()
	...
	local Buff = 507380
	--生出boss
	Boss = CreateObjByFlag( 104144, 780711, 0, 1) 
	AddToPartition( Boss , RoomID )	
	WriteRoleValue( Boss , EM_RoleValue_Register1, OwnerID() )
	BeginPlot(Boss,"Cang_130_tree",0)
	SetPlot( Boss, "dead" , "Cang_130_tree_dead" ,0 )	
	
	local Door = CreateObjByFlag( 104226 , 780711 , 5 , 1 )
...
--------------開場執行：time時間階段，buff不可用物品，cy水晶產生，盾檢查，生門
				BeginPlot( OwnerID() , "Cang_130_tree_time", 0)
				ScriptMessage( OwnerID() , -1 , 2 , CountSay[2], 1 )
				BeginPlot( OwnerID() , "Cang_130_tree_shief", 0)
				Door = CreateObjByFlag( 104226 , 780711 , 6 , 1 )
				SetModeEX( Door  , EM_SetModeType_Gravity , false )--重力
				SetModeEx( Door  , EM_SetModeType_ShowRoleHead, false )--不秀頭像框
				SetModeEx( Door  , EM_SetModeType_Obstruct, true )--阻擋
				SetModeEx( Door  , EM_SetModeType_Mark, false )--標記
				...
				BeginPlot( Door , "Cang_130_tree_summomkill", 0)				
				Doorstart = 1		
			end	
			count2 = count2 + 1
			count3 = count3 + 1
			...
			elseif CheckBuff( OwnerID(), Mode2) == true then
				...
				if count4 >= 8 then
					...
					if table.getn(mid_tree_1) < 4 then
						...
						BeginPlot( M_tree1 , "Cang_130_Midtree_skill", 0)					
						count4 = 0
					end	
				end
				if count3 >= 15 then
...
function Cang_130_tree_time()
	...
	BeginPlot( OwnerID() , "Cang_130_tree_buff", 0)
	BeginPlot( OwnerID() , "Cang_130_tree_cy", 0)
	while true do
		sleep(10)
		C_time = C_time + 1 
		M1time = M1time + 1
...
function Cang_130_tree_cy()
	...
	while true do
		...
		if count1 >= 5 then
			...
			SetPlot( cy, "touch" , "Cang_130_item_touch" ,30 )
			count1 = 0
		end
	end
end
...
--------------------------------------------------------------------------------王死自己刪除自己
function Cang_130_tree_summomkill()
...
--------------------------------------------------------------------------------中樹
function Cang_130_Midtree_skill()
	BeginPlot( OwnerID() , "Cang_130_tree_summomkill", 0)
	SetPlot( OwnerID() , "dead" , "Cang_130_Midtree_dead" ,0 )	
	local Boss = ReadRoleValue( OwnerID() , EM_RoleValue_PID )	
...
function Cang_130_Midtree_dead()
	local RoomID = ReadRoleValue(OwnerID() , EM_RoleValue_RoomID)
	local count = 0
	...
	BeginPlot( SpellCaster , "Cang_mage_deadtime" , 0)
	DelObj( OwnerID() )
end

function Cang_mage_deadtime()
	sleep(30)
	Delobj(OwnerID())
end
--------------------------------------------------------------------------------小樹死亡劇情
function Cang_130_smalltree_dead()	
	--local RoomID = ReadRoleValue(OwnerID() , EM_RoleValue_RoomID)
	--local count = 0
	--local SpellCaster = star_CreateObj( OwnerID() , 104195 , 0 , 0 , 0 , RoomID , 90)
	--SetModeEx( SpellCaster  , EM_SetModeType_HideName, false )--名稱
	--SetModeEX( SpellCaster  , EM_SetModeType_Gravity , false )--重力
	--SetModeEx( SpellCaster  , EM_SetModeType_NotShowHPMP, true )--血條
	--SetModeEx( SpellCaster  , EM_SetModeType_ShowRoleHead, false )--不秀頭像框
	...
	--AddToPartition( SpellCaster , RoomID )	--加進舞台
	--if count ==0 then	
	--CastSpellLV( SpellCaster , SpellCaster , 496067 , 29 )
	--	count = 1
	--end		
	--BeginPlot( SpellCaster , "Cang_mage_dead" , 0)	
	DelObj(OwnerID())
end

--------------------------------------------------------------------------------小樹
function Cang_130_Littletree_skill()
	BeginPlot( OwnerID() , "Cang_130_tree_summomkill", 0)
	SetPlot( OwnerID() , "dead" , "Cang_130_smalltree_dead" ,0 )
	local count = 0
	local count1 = 0
	local Boss = ReadRoleValue( OwnerID() , EM_RoleValue_PID )		
...
--------------------------------------------------------------------------------物品
--生命水晶
function Cang_130_item_use_01()	--解除毒霧用水晶
...
--劇毒水晶
function Cang_130_item_use_02()	--破盾用水晶
	local Buff1 = 507381
