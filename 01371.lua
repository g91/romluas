function LuaI_102021()
	...
	for i = 1 , 2 , 1 do
		...
		BeginPlot( Wall[i] , "LuaI_102021_Wall" , 0 )
	end
	...
	while 1 do
		...
		if Count > 0 then
			...
			if CheckBuff( OwnerID() , 503723 ) == false then
				ScriptMessage( OwnerID() , -1 , 2 , "[SC_102021_2]" , 0 ) --偉大的海洋！我將自己完全奉獻於您！讓我的身軀成為您消滅一切的武器！
				ScriptMessage( OwnerID() , -1 , 0 , "[SC_102021_2]" , 0 )
				AddBuff( OwnerID() , 503724 , 1 , -1 )
			end
			...
			if Timer == 30-WaveTime then
				...
				Wave[1][1] = CreateObjByFlag(102039 , 780361 ,RandNum1, 1 )
				Wave[2][1]= CreateObjByFlag(102039 , 780361 ,RandNum1 + WavePos*2 , 1 )
				Wave[3][1] = CreateObjByFlag(102039 , 780361 ,RandNum2 , 1 )
				Wave[4][1] = CreateObjByFlag(102039 , 780361 ,RandNum2 + WavePos*2 , 1 )
				...
				for j = 1 , 4 , 1 do
					...
					Wave[j][2] =  CreateObj ( 102039 , obj:X() - d* math.sin( Cal ), Y1, obj:Z() - d * math.cos( Cal ) , obj:Dir() , 1)
					Wave[j][3] =  CreateObj ( 102039 , obj:X() + d* math.sin( Cal ), Y2  , obj:Z() + d * math.cos( Cal ) , obj:Dir() , 1)
				end
				for j = 1 , 4 , 1 do
					for i = 1 , 3 , 1 do
			...
			if OnBattle == 1 then --結束戰鬥清除玩家身上buff
				...
				for i = 1 , PPL , 1 do
					local ID = GetSearchResult()
					if CheckBuff( ID ,503433 ) == false then
						CancelBuff_NoEvent( ID , 503433 )
					end
				end
...
function LuaI_102038()
	local Bubble = {}
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )

	for i = 1 , GetMoveFlagCount(780360) -1 , 1 do
		Bubble[i] = CreateObjByFlag( 102038 , 780360 , i , 1 )
		Lua_ObjDontTouch( Bubble[i] )
		Show(  Bubble[i] , RoomID )
--		Yell( Bubble[i] , "I have Bubble" , 3 )
		AddBuff( Bubble[i] , 503431, 1 , -1 )
	end

	local TempPos = {}

	while 1 do
		for i = 1 , table.getn(Bubble) , 1 do
			Hide(  Bubble[i] )
		end
		for i = 1 , table.getn(Bubble) , 1 do
			TempPos[i] = Bubble[i]
		...
		for i = 1 , 4 , 1 do
			local RandNum = DW_Rand( table.getn(TempPos) )
			Show(TempPos[RandNum] , RoomID )
			table.remove(TempPos, RandNum )
		end
		sleep( 120 )
		TempPos = {}
	end
...
function LuaI_102021_Funny()
	MoveToFlagEnabled( OwnerID() , false)
	LuaFunc_MoveToFlag( OwnerID(), 780361 , 2 ,0 )
end

function LuaI_102021_Wall()
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	local Boss = TargetID()
	...
	while 1 do
		if ReadRoleValue( Boss, EM_RoleValue_IsDead ) == 1 or CheckID(Boss) == false or ReadRoleValue( Boss, EM_RoleValue_OrgID )~= 102021 or ReadRoleValue( Boss, EM_RoleValue_RoomID )~= RoomID then
			Switch = "Open"
			break
		end
