function Lua_Az132_104165_Ctrl()							--Raid金旺
	local Ctrl=OwnerID()									--控制器
	local CombatBegin=0										--開戰旗標
	local SoulCount=0
	local RoomID=ReadRoleValue(Ctrl,EM_RoleValue_RoomID)
	local Boss=CreateObjByFlag(104165,780701,0,1)			--王
	...
	SetPlot(Boss,"dead","Lua_Az132_104165_dead",0)
	local BuffID={507568,507570,507569}						--獅子猩猩羚羊
	...
	while true do
		...
		if hatelistcount(boss)~=0 then
			if Combatbegin==0 then
				...
				BeginPlot(Boss,"Lua_Az132_104165",0)
...
--				say(Ctrl,"BuffCount="..BuffCount)
				...
				if BuffCount%31==0 then
					...
					if ReadRoleValue(Boss,EM_RoleValue_SpellMagicID)==0 then
						...
						elseif BuffID[BuffNum]==507569 then
							if CheckBuff(Boss,507569) then
		...
		elseif hatelistcount(boss)==0 then
			if Combatbegin==1 then
				if ReadRoleValue( boss , EM_RoleValue_IsDead)==0 then
					...
					DelObj(door1)
					sleep(50)
					Boss=CreateObjByFlag(104165,780701,0,1)			--王
...
function Lua_Az132_104165()
	...
	while true do
		...
		SkTgt={}
		SkStr={}
--		if CheckBuff(boss,507569)==true then			--獅子
--			table.insert(SkSel,496140)
--			table.insert(SkSel,496140)
--			table.insert(SkCd,0)
--			table.insert(SkCd,0)
...
--			table.insert(SkStr,"[SC_104142_03]")
--		end
		if CheckBuff(boss,507568)==true then		--猩猩
			table.insert(SkSel,496152)
--			table.insert(SkSel,496152)
...
--			table.insert(SkStr,"[SC_104142_10]")
		...
		if CheckBuff(boss,507570)==true then		--蹬羚
			table.insert(SkSel,496139)
			table.insert(SkSel,496139)
...
--					ScriptMessage(boss,-1,2,"[SC_104142_12]",2)
				elseif SkCount%17==0 then
					...
					if ReadRoleValue(Boss,EM_RoleValue_SpellMagicID)==0 and ReadRoleValue(Boss,EM_RoleValue_register4)~=0 then
						...
						for i=1,EveryOne,1 do
							...
							SysCastSpellLv(boss,NoWayRun[i],496089,4)
						end
				...
				elseif SkCount%7==0 then
					RND=DW_Rand(SkNum)
					if SkSel[RND]==496152 then
						while ReadRoleValue(Boss,EM_RoleValue_SpellMagicID)~=0 do
							sleep(2)
						end
						CastSpellLv(boss,boss,496140,4)
...
--								CastSpellLv(Boss,Boss,495873,0)
							...
							elseif ReadRoleValue(Boss,EM_RoleValue_register4)==1 and x==8 then
								...
								if ReadRoleValue(Boss,EM_RoleValue_SpellMagicID)==0 then
									CastSpellLv(boss,target,496152,0)
					...
					elseif SkSel[RND]==496139 then
						while ReadRoleValue(Boss,EM_RoleValue_SpellMagicID)~=0 do
							sleep(2)
						end
						CastSpellLv(boss,PickOne[1],496139,0)
...
function Lua_Az132_Enter()
	SetPlot(OwnerID(),"range","Lua_Az132_Tele",50)
end

function Lua_Az132_Back()
	SetPlot(OwnerID(),"range","Lua_Az132_TeleBack",30)
end

function Lua_Az132_Tele()
...
function Lua_Az132_Soul()
	beginplot(TargetID(),"Lua_Az132_SoulDrain",0)
end

function Lua_Az132_SoulDrain()
	while true do
		sleep(10)
...
function Lua_Az132_104165_dead()
