function bk_127_103875()
	...
	local CombatBegin=0
	local door=0
	local afdoor = CreateObjByFlag(103874,780613,1,1)
	...
	local boss = CreateObjByFlag(103873,780613,3,1)------------------------------創立王
	...
	BeginPlot( boss ,"bk_127_103873",0)
	SetPlot( boss , "dead" , "bk_127_103873_dead" , 10 )
	...
	while 1 do 
		...
		if x==1 then --王正在戰鬥之中
			if CombatBegin==0 then 
				door = CreateObjByFlag(103874,780613,2,1)
		...
		elseif x==2 then --玩家死亡王離開戰鬥
			sleep(50)
			boss = CreateObjByFlag(103873,780613,3,1)
			SetModeEx( boss , EM_SetModeType_Gravity, true) ---重力
			SetModeEx( boss , EM_SetModeType_Mark, true)
			SetModeEx( boss , EM_SetModeType_HideName, true)
			SetModeEx( boss , EM_SetModeType_ShowRoleHead, true)
			...
			BeginPlot( boss ,"bk_127_103873",0)---------------------------------王劇情模式
			SetPlot( boss , "dead" , "bk_127_103873_dead" , 10 )
			x=0
			sleep(10)
			DelObj(door)
		...
		if HateListCount( boss ) ~= 0 then	---- 確定他現在有打人
			...
			if mob == 0 then 
				...
				if mob1>= 5 then
					if ms==0 then 
						Ls1=DW_Rand(20)
						tar1=CreateObjByFlag(103876,780599,Ls1,1)
					...
					elseif ms==1 then 					
						...
						Ls2=DW_Rand(20)
						elseif Ls1~=Ls2 then 
							tar2=CreateObjByFlag(103876,780599,Ls2,1)
					...
					elseif ms==2 then 	
						...
						elseif Ls1~=Ls3 and Ls2~=Ls3 then 
							tar3=CreateObjByFlag(103876,780599,Ls3,1)
					...
					elseif ms==3 then 	
						...
						elseif Ls1~=Ls3 and Ls2~=Ls3 and Ls3~=LS4 then 
							tar4=CreateObjByFlag(103876,780599,Ls4,1)
			...
			elseif mob== 1 then 
				...
				if mob2>=30 then 
					...
					CastSpellLV(OwnerID() ,OwnerID(),495525 , 4 )---------------------------------劇烈的攻擊
					mob=0
					mob2=0
				end
