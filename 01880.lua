...
----------普通技能施放
			if skillDot >= 15 then
				...
				CastSpellLV( OwnerID() , OwnerID() , 495135 , 3 )		
				skillDot = 0
			end
			if skill >= 5 then
...
--------------------------------------------------------------爵士法陣
function Cang_129_b_summomkill()
	AddBuff( OwnerID() , 505675 , 1 , -1 )
	local Baron = ReadRoleValue( OwnerID() , EM_RoleValue_PID )	
	sleep(30)	
	SetPlot( OwnerID(), "range" , "Cang_129_b_s_range" ,30 )	
----------如果王離開戰鬥或是王死亡，刪除自己	
	while 1 do
		sleep(10)
----------搜尋55碼內有無NPC, NPC如果是自己, 就補血
		local NPC = SearchRangeNPC ( OwnerID() , 45 )
...
function Cang_129_baron_dead()		
	...
	BeginPlot( corpse, "Cang_129_baron_corpse", 0)
	--SetPlot( corpse, "dead", "Cang_129_baron_corpse", 0) 
	--KillID( corpse, corpse )
	--寶箱
	local Box = CreateObjByFlag( 103958, 780670, 11, 1)	--寶箱
...
function Cang_129_baron_corpse()	
