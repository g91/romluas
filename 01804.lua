function bk_127_103831()
	...
	local s2=0
	local s3=0
	local afdoor = CreateObjByFlag(103823,780597,2,1)
	...
	local boss = CreateObjByFlag(103170,780597,3,1)------------------------------創立王
...
--	SetModeEx( boss , EM_SetModeType_NotShowHPMP, true)
	...
	BeginPlot( boss ,"bk_127_103170",0)
	SetPlot( boss , "dead" , "bk_127_103170_dead" , 10 )
	...
	while 1 do 
		...
		if x==1 then --王正在戰鬥之中
			if CombatBegin==0 then 
				door = CreateObjByFlag(103823,780597,1,1)
		...
		elseif x==2 then --玩家死亡王離開戰鬥
			...
			s2 = ReadRoleValue(start,EM_RoleValue_Register8)
			s3 = ReadRoleValue(start,EM_RoleValue_Register9)
			boss = CreateObjByFlag(103170,780597,3,1)
			AddToPartition( boss , RoomID )
			WriteRoleValue( boss, EM_RoleValue_PID,start)
			BeginPlot( boss ,"bk_127_103170",0)---------------------------------王劇情模式
			SetPlot( boss , "dead" , "bk_127_103170_dead" , 10 )
			x=0
			sleep(10)
			Delobj(s1)
	...
	while 1 do 
		...
		if HateListCount( boss ) ~= 0 then	---- 確定他現在有打人
			...
			if mob == 1 then 
				...
				local Temp = Lua_Davis_BornByMultilateral( 103279 , 3 , 25 )	--在自己身上出4怪
				WriteRoleValue( Temp[1] ,EM_RoleValue_PID, start)	--寫入PID
				WriteRoleValue(start, EM_RoleValue_Register7,Temp[1])	
				SetPlot( Temp[1] , "dead" , "bk_127_103279_true" , 10 )
				BeginPlot( Temp[1] , "bk_127_103279_tu",20)---------------------------------施放法術劇情
			...
			---------	AddBuff(Temp[1], 506757 , 1 , -1 )
				...
				SetPlot( Temp[2] , "dead" , "bk_127_103279_flase" , 10 )
				BeginPlot( Temp[2] , "bk_127_103279",20)---------------------------------施放法術劇情
				...
				SetPlot( Temp[3] , "dead" , "bk_127_103279_flase" , 10 )
				BeginPlot( Temp[3] , "bk_127_103279",20)---------------------------------施放法術劇情
...
function bk_127_103279_true()--------------小怪劇情
	local RoomID = ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)
	local start = ReadRoleValue(OwnerID(),EM_RoleValue_PID)
