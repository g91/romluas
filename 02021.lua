function Lua_Az130_103948_Ctrl()							--Raid金旺
	local Ctrl=OwnerID()									--控制器
	local CombatBegin=0										--開戰旗標
	local SoulCount=0
	local RoomID=ReadRoleValue(Ctrl,EM_RoleValue_RoomID)
	local Boss=CreateObjByFlag(103948,780701,0,1)			--王
	...
	SetPlot(Boss,"dead","Lua_Az130_103948_dead",0)
	local BuffID={507561,507567,507566}						--獅子猩猩羚羊
	...
	while true do
		...
		---		BeginPlot( boss ,"bk_hacker_17_130_03",0)--------------------------------防駭客機制
				BeginPlot(Boss,"Lua_Az130_103948",0)
...
--				say(Ctrl,"BuffCount="..BuffCount)
				...
				if BuffCount%31==0 then
					...
					if ReadRoleValue(Boss,EM_RoleValue_SpellMagicID)==0 then
						...
						elseif BuffID[BuffNum]==507566 then
							if CheckBuff(Boss,507566) then
...
--						say(boss,"NewBuff="..BuffNum)
						local Buffs=table.getn(BuffID)
--						say(boss,"Buffs="..Buffs)
						SysCastSpellLv(ObjID[BuffNum],Boss,495972,0)					--雕像表演對BOSS上法術
						--say(objid[buffnum],"me")
		...
		elseif hatelistcount(boss)==0 then
			if Combatbegin==1 then
				if ReadRoleValue( boss , EM_RoleValue_IsDead)==0 then
					...
					DelObj(door1)
					sleep(50)
					Boss=CreateObjByFlag(103948,780701,0,1)			--王
...
function Lua_Az130_103948()
	...
	while true do
		...
		SkTgt={}
		SkStr={}
--		if CheckBuff(boss,507566)==true then			--獅子
...
--			table.insert(SkStr,"[SC_104142_03]")
--		end
		if CheckBuff(boss,507561)==true then		--猩猩
			table.insert(SkSel,496151)
			table.insert(SkSel,496151)
		...
		if CheckBuff(boss,507567)==true then		--蹬羚
			table.insert(SkSel,496135)
			table.insert(SkSel,496135)
		...
		if mode==1 then
			...
			elseif ReadRoleValue(Boss,EM_RoleValue_register4)==1 then
				...
				elseif SkCount%17==0 then
					...
					if ReadRoleValue(Boss,EM_RoleValue_SpellMagicID)==0 and ReadRoleValue(Boss,EM_RoleValue_register4)~=0 then
						...
						for i=1,EveryOne,1 do
							...
							SysCastSpellLv(SoulBall[i],boss,495972,0)
							SysCastSpellLv(boss,NoWayRun[i],496089,9)
						end
...
--								CastSpellLv(Boss,Boss,495873,0)
								--say(OwnerID(),"break")
								break
							elseif ReadRoleValue(Boss,EM_RoleValue_register4)==1 and x<8 then
								SkCount=SkCount+1
								sleep(10)
								if SkCount%17==0 then
									while ReadRoleValue(Boss,EM_RoleValue_SpellMagicID)~=0 do
										sleep(2)
									end
									if ReadRoleValue(Boss,EM_RoleValue_SpellMagicID)==0 and ReadRoleValue(Boss,EM_RoleValue_register4)~=0 then
							...
							elseif ReadRoleValue(Boss,EM_RoleValue_register4)==1 and x==8 then
								...
								if ReadRoleValue(Boss,EM_RoleValue_SpellMagicID)==0 then
									CastSpellLv(boss,target,496151,0)
					...
					elseif SkSel[RND]==496135 then
						while ReadRoleValue(Boss,EM_RoleValue_SpellMagicID)~=0 do
							sleep(2)
						end
						CastSpellLv(boss,PickOne[1],496135,0)
					...
					elseif SkSel[RND]==495970 then
						...
						CastSpellLv(boss,boss,495970,15)
						sleep(10)
					end
				end
			end
		elseif mode==2 then
			...
			elseif ReadRoleValue(Boss,EM_RoleValue_register4)==1 then
				...
				elseif SkCount%19==0 then 
					...
					CastSpellLv(boss,boss,495468,5)
...
function Lua_Az_pplcheck_130()
	...
	elseif x==5 then 
		CastSpellLV(cab,cab,495470,179)
	elseif x==4 then 
		CastSpellLV(cab,cab,495470,224)
	elseif x==3 then
		CastSpellLV(cab,cab,495470,299)
	elseif x==2 then
		CastSpellLV(cab,cab,495470,449)
	elseif x==1 then	
		CastSpellLV(cab,cab,495470,899)
	end
end

function Lua_Az130_Enter()
	SetPlot(OwnerID(),"range","Lua_Az130_Tele",50)
end

function Lua_Az130_Back()
	SetPlot(OwnerID(),"range","Lua_Az130_TeleBack",30)
end

function Lua_Az130_Tele()
...
function Lua_Az130_Soul()
	beginplot(TargetID(),"Lua_Az130_SoulDrain",0)
end

function Lua_Az130_SoulDrain()
	while true do
		sleep(10)
...
function Lua_Az130_103948_dead()
