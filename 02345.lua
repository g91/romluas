...
function Dy_Z19_BOSSN_AIC()
	...
	local BossDoor2 --後門
	local Bossr
	local Boss = CreateObjByFlag( 105616 , 780696 , 1, 1 ) -- (BOSS編號  ,旗標編號 第N隻旗標 生出幾個怪)
		...
		setplot( Boss , "dead" , "Dy_Z19_BOSSN_DEAD", 0)
		BeginPlot( Boss, "Dy_Z19_BOSSN_AI", 10) ---Boss的戰鬥Function (  BOSS BOSS的LUA_AI  秒\每10=1秒)
		...
		if Fight == 1 then --Boss戰鬥中
			...
			if madtime == 360  then
				ScriptMessage( Boss, -1 , 2 , "[SC_105617_07] " , 2 )
				sleep(10)
			...
			if Fight1 == 0 then
				ScriptMessage( OwnerID(), -1 , 2 , "[SC_105617_00] " , 2 )
				--種出前門
		...
		elseif Fight == 2 then --玩家滅團
			ScriptMessage( OwnerID(), -1 , 2 , "[SC_105617_09]" , 2 )
			DelObj ( Boss )
			DelObj ( BossDoor1 )
			DelObj ( dart_1 )
			DelObj ( dart_2 )
			...
			sleep(50)
			Boss =CreateObjByFlag( 105616 , 780696 , 1, 1 ) -- (BOSS編號  ,旗標編號 第N隻旗標 生出幾個怪)
			WriteRoleValue( Boss, EM_RoleValue_PID, OwnerID()) --將控制器寫入Boss
			WriteRoleValue( OwnerID(), EM_RoleValue_Register1, Boss) --將Boss寫入控制器
			AddToPartition( Boss , roomID )
			setplot( Boss , "dead" , "Dy_Z19_BOSSN_DEAD", 0) --死亡劇情
			BeginPlot( Boss, "Dy_Z19_BOSSN_AI", 10) ---Boss的戰鬥Function (  BOSS BOSS的LUA_AI  秒\每10=1秒)
			--say(Boss,"123")
			Fight = 0 
			Fight1 = 0
		...
		elseif Fight == 4 then --換完區域的方形傷害隨機
			ScriptMessage( Boss, -1 , 2 , "[SC_105617_11] " , 2 )
			Q3 = Rand(4)
			if Q3 == 0 then --2
				...
				for i = 1 , 3 ,1 do 
					AddBuff ( purple[i], 509496, 0, -1 )
					--刪除509496以外的Buff--
					if CheckBuff(purple[i],  509506 ) == true then
						CancelBuff( purple[i] ,  509506 ) --刪除藍色B-A
						--say (TargetID() , "P_B")
					else
					
					end
					if CheckBuff( TargetID() ,  509508 ) == true then
						CancelBuff( TargetID() ,  509508 ) --刪除藍色B-B
						--say (TargetID() , "P_B")
					else
					
					end
					if CheckBuff( purple[i] ,  509510 ) == true then
						CancelBuff( purple[i] ,  509510  ) --刪除藍色B-C
						--say (TargetID() , "P_B")
					else
					
					end
					if CheckBuff(purple[i] ,  509498 ) == true then
						CancelBuff( purple[i] ,  509498 ) --刪除紫色P-A
						--say (TargetID() , "P_B")
					else
					
					end
					if CheckBuff( purple[i] ,  509500 ) == true then
						CancelBuff( purple[i] ,  509500 ) --刪除紫色P-C
						--say (TargetID() , "P_B")
					else
					
					end
					if CheckBuff( purple[i] ,  509491 ) == true then
						CancelBuff( purple[i] ,  509491 ) --刪除黃昏R-A
						--say (TargetID() , "P_B")
					else
					
					end
					if CheckBuff( purple[i] ,  509489 ) == true then
						CancelBuff( purple[i] ,  509489 ) --刪除黃昏R-B
						--say (TargetID() , "P_B")
					else
					
					end
					if CheckBuff( purple[i] ,  509486 ) == true then
						CancelBuff(purple[i] ,  509486 ) --刪除黃昏R-C
						--say (TargetID() , "P_B")
					else
					
					end
					--
					AddBuff ( blue[i] , 509510 , 0, -1 )
					--刪除509510以外的Buff--
					if CheckBuff( blue[i] ,  509506 ) == true then
						CancelBuff( blue[i] ,  509506 ) --刪除藍色B-A
						--say (TargetID() , "B_C")
					else
					
					end
					if CheckBuff( blue[i] ,  509508 ) == true then
						CancelBuff( blue[i] ,  509508 ) --刪除藍色B-B
						--say (TargetID() , "B_C")
					else
					
					end
					if CheckBuff( blue[i] ,  509498 ) == true then
						CancelBuff( blue[i] ,  509498 ) --刪除紫色P-A
						--say (TargetID() , "B_C")
					else
					
					end
					if CheckBuff( blue[i] ,  509496 ) == true then
						CancelBuff( blue[i] ,  509496 ) --刪除紫色P-B
						--say (TargetID() , "B_C")
					else
					
					end
					if CheckBuff( blue[i] ,  509500 ) == true then
						CancelBuff( blue[i] ,  509500 ) --刪除紫色P-C
						--say (TargetID() , "B_C")
					else
					
					end
					if CheckBuff( blue[i] ,  509491 ) == true then
						CancelBuff( blue[i] ,  509491 ) --刪除黃昏R-A
						--say (TargetID() , "B_C")
					else
					
					end
					if CheckBuff( blue[i] ,  509489 ) == true then
						CancelBuff( blue[i],  509489 ) --刪除黃昏R-B
						--say (TargetID() , "B_C")
					else
					
					end
					if CheckBuff( blue[i] ,  509486 ) == true then
						CancelBuff( blue[i] ,  509486 ) --刪除黃昏R-C
						--say (TargetID() , "B_C")
					else
					
					end					
					--
					AddBuff ( red[i] , 509491, 0, -1 )
					----刪除509491以外的Buff--
					if CheckBuff( red[i]  ,  509506 ) == true then
						CancelBuff( red[i] ,  509506 ) --刪除藍色B-A
						--say (TargetID() , "R_A")
					else
					
					end
					if CheckBuff( red[i] ,  509508 ) == true then
						CancelBuff( red[i]  ,  509508 ) --刪除藍色B-B
						--say (TargetID() , "R_A")
					else
					
					end
					if CheckBuff( red[i]  ,  509510 ) == true then
						CancelBuff( red[i]  ,  509510  ) --刪除藍色B-C
						--say (TargetID() , "R_A")
					else
					
					end
					if CheckBuff( red[i]  ,  509498 ) == true then
						CancelBuff( red[i]  ,  509498 ) --刪除紫色P-A
						--say (TargetID() , "R_A")
					else
					
					end
					if CheckBuff( red[i]  ,  509496 ) == true then
						CancelBuff( red[i]  ,  509496 ) --刪除紫色P-B
						--say (TargetID() , "R_A")
					else
					
					end
					if CheckBuff(red[i]  ,  509500 ) == true then
						CancelBuff( red[i]  ,  509500 ) --刪除紫色P-C
						--say (TargetID() , "R_A")
					else
					
					end
					if CheckBuff( red[i]  ,  509489 ) == true then
						CancelBuff( red[i]  ,  509489 ) --刪除黃昏R-B
						--say (TargetID() , "R_A")
					else
					
					end
					if CheckBuff( red[i]  ,  509486 ) == true then
						CancelBuff( red[i]  ,  509486 ) --刪除黃昏R-C
						--say (TargetID() , "R_A")
					else
			...
			elseif Q3 == 1 then --4
				...
				--刪除509498以外的Buff--
					...
					if CheckBuff( purple[i] ,  509508 ) == true then
						CancelBuff( purple[i] ,  509508 ) --刪除藍色B-B
						--say (TargetID() , "P_A")
					else
					...
					if CheckBuff( purple[i] ,  509496 ) == true then
						CancelBuff( purple[i] ,  509496 ) --刪除紫色P-B
						--say (TargetID() , "P_A")
					else
				...
				--
					...
					--刪除509508以外的Buff--
						...
						if CheckBuff( blue[i] ,  509510 ) == true then
							CancelBuff( blue[i] ,  509510  ) --刪除藍色B-C
							--say (TargetID() , "B_B")
						else
					...
					--刪除509486以外的Buff--
						...
						if CheckBuff( red[i] ,  509491 ) == true then
							CancelBuff( red[i] ,  509491 ) --刪除黃昏R-A
							--say (TargetID() , "R_C")
						else
			...
			elseif Q3 == 4 then --6
				...
				--刪除509506以外的Buff--
					...
					if CheckBuff( blue[i] ,  509491 ) == true then
						CancelBuff( Tblue[i] ,  509491 ) --刪除黃昏R-A
						--say (TargetID() , "B_A")
					else
