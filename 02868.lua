...
function lua_World5_MTSnoopSZ()
	...
	local Detail = ""
		Detail = lua_Snoopy_SpeakDetail()
		SetSpeakDetail( Player , Detail )			--呼～小子，你想做什麼快說；我，可是很沒耐心的。
		AddSpeakOption( Player, Player,  "[SC_111256_S]", "LuaS_111256_S", 0 ); --我要將回程術地點登記於此
			--開啟此地傳送門
			if CheckFlag( Player , Flag ) == false then
				SetFlag( Player , Flag , 1)
				ScriptMessage( Player, Player, 1, "[SC_111256_OT]", C_SYSTEM );	--你成功開啟了此地的旅者之門。
			end
			--列出傳送選項(501638=騎程禁止)
			if CheckBuff( Player , 501638 ) == false then
				for i=1,#tmpPoint do
