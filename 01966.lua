...
--控制對話內容--
	if ReadRoleValue( OwnerID() , EM_RoleValue_Skill_Carpenter ) == 80  then --等級小於60則叫玩家去找另一個NPC
		SetSpeakDetail( OwnerID(), "[ST_115957_2]"   );--#
	elseif(    ReadRoleValue( OwnerID() , EM_RoleValue_Skill_Carpenter ) == 60 ) then  --等級等於40時判斷是否可接任務
		--###技能達到進階標準時加入任務###--
		if not tolimit  then
...
--控制對話選項--
	if( ReadRoleValue ( OwnerID(), EM_RoleValue_SkillMax_Carpenter ) >= 40) then --擁有工匠以上資格則出現可以放棄資格的選項
		AddSpeakOption( OwnerID(), TargetID( ), GetString("SO_DEGRADE_DIALOG") , "LuaS_115957_1", 0 ) --/*我想放棄目前的資格
	end 
...
function LuaS_115957_1()
	...
	AddSpeakOption( OwnerID(), TargetID( ), GetString("SO_GETVOC_NO"), "LuaS_115957_0", 0 ); --/*我再考慮看看。
