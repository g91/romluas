...
--	MoveToFlagEnabled( OwnerID(), false )
	...
	sleep(30) 
	MoveDirect( OwnerID(), -13086.6 , 2117.9 , -10499.8 )
	local Box = LuaFunc_SearchNPCbyOrgID( OwnerID() , 100695 , 200 , 1)            
	local Golu = LuaFunc_SearchNPCbyOrgID( Box[0] , 101028 , 200 , 1)             
	sleep(10)
	local StoneMaker = LuaFunc_SearchNPCbyOrgID( Golu[0] , 101054 , 200 , 1)           
	local Coolbo = LuaFunc_SearchNPCbyOrgID( Golu[0] , 100044 , 200 , 1)           

	WriteRoleValue( OwnerID() , EM_RoleValue_Register  , StoneMaker[0])                       --記住石像的編號
	WriteRoleValue( OwnerID() , EM_RoleValue_Register+1  , Coolbo[0]  )              
	WriteRoleValue( OwnerID() , EM_RoleValue_Register+2  , Golu[0] )           
    
	WriteRoleValue( Coolbo[0] ,EM_RoleValue_PID  , OwnerID()  )

	local CheckTime = 10	
	local AttackTarget = ReadRoleValue( OwnerID() , EM_RoleValue_AttackTargetID )

	local BossB = ReadRoleValue( Golu[0] , EM_RoleValue_PID )
	...
	local Distance = GetDistance( OwnerID(), Player[0] )
		...
		If CheckID( Player[0] ) == true and Distance > 180  then
			...
			if  Distance < 150  then
				CastSpelllv( OwnerID() , Player[0] , 490620 , 100 )    
				SetAttack( OwnerID() , Player[0]  ) --單純攻擊
				sleep(30)
				break
			end
	...
	local stoneBoss  = ReadRoleValue( Golu[0], EM_RoleValue_PID  )                      
	WriteRoleValue( OwnerID() ,EM_RoleValue_PID  , stoneBoss )                       --寫入沃特的PID
	local DIS = 0
	local OB = 0
	local X = 0  
...
--				SAY(OwnerID() , XX )
					...
					elseif XX >= 3 and XX < 5 then --20%	
						...
						sleep(40)
							if  CheckID( OB ) == true then
								CastSpelllv( OwnerID() , OB , 490620 , 150 )    --衝鋒陷陣	
								sleep(40)
								local HateC = HateListCount( OwnerID() )
								local HC = Rand(HateC)
								...
								CastSpelllv( OwnerID() , AttackHC , 490620 , 150 )    
								sleep(40)	
								--SetStopAttack( OwnerID() ) 
								sleep(20)	
								CastSpelllv( OwnerID() , Tank , 490620 , 150 )    
								sleep(50)	
							else
								local HateC = HateListCount( OwnerID() )
								local HC = Rand(HateC)
					...
					elseif XX >= 1 and XX < 3 then --20%
						SAY(OwnerID() , GetString("SC_101222_YU_01") )  		--滾開！戰神 ─ 沃特‧賓普斯克來了！
						ScriptMessage(OwnerID() , 0 , 1 , GetString("SC_101222_YU_01") , C_SYSTEM )
						sleep(10)	
...
--				SAY(OwnerID() , XX )
					...
					elseif XX >= 2 and XX < 4 then --20%	
						SAY(OwnerID() , GetString("SC_101222_YU_01") )  
						ScriptMessage(OwnerID() , 0 , 1 , GetString("SC_101222_YU_01") , C_SYSTEM )
						sleep(10)	
