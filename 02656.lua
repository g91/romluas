--門口：/GM ? Zone 146 0 -539.7 1060.1 -478.4
--781125--Flag1：Anton
--781125--Flag2：前門
--781125--Flag3：後門
--781125--Flag4+5：出兵點
...
function sasa_106382_Counter()
	local Count = OwnerID()
	local Room = ReadRoleValue( Count , EM_RoleValue_RoomID )
	local Anton = CreateObjByFlag( 106382 , 781125 , 1 , 1 )
	AddToPartition( Anton , Room )
	BeginPlot( Anton , "sasa_106382_AI" , 0 )
	Setplot( Anton , "dead" , "sasa_106382_Dead" , 30 )
...
--※ 抬走 ※--
				if TakeCounter >= 23 then
					...
					if table.getn(TakePlayer) >= 2 then						--大於2人就rand 2個
						for xx = 1 , 2 do
							...
							if ( TakePlayer[Lucky] ~= nil ) and ( ReadRoleValue( TakePlayer[Lucky] , EM_RoleValue_IsDead ) == 0 ) then
								local Ant03 = CreateObjByFlag( 106383 , 781125 , 4 , 1 )
								...
								BeginPlot( Ant03 , "sasa_106383_takeaway" , 0 )
								--Setplot( Ant03 , "collision" , "sasa_106383_takeaway" , 20 )
								Setplot( Ant03 , "dead" , "sasa_106383_Dead" , 30 )
...
--※ 喚醒小怪 ※--
				elseif WakeCounter >= 10 then
					...
					if table.getn( Ant1 ) < 4 then
						for d = 1 , 2 do
							local Ant01 = CreateObjByFlag( 106387 , 781125 , 4 , 1 )
							Ant1[table.getn( Ant1 )+1] = Ant01
							AddToPartition( Ant01 , Room )
							BeginPlot( Ant01 , "sasa_106387_Eaten" , 0 )
							--Setplot( Ant01 , "dead" , "sasa_106387_Dead" , 30 )
							WriteRoleValue( Anton , EM_RoleValue_register3 , Ant01 )
							Sleep(10)
							local Ant02 = CreateObjByFlag( 106386 , 781125 , 5 , 1 )
							Ant1[table.getn( Ant1 )+1] = Ant02
							AddToPartition( Ant02 , Room )
							BeginPlot( Ant02 , "sasa_106386_poison" , 0 )
							Sleep(10)
						end
					elseif table.getn( Ant1 ) >= 4 then								--超過4隻螞蟻就檢查螞蟻死了沒，把死掉的從Ant1中移除
...
--※ 重置死亡 ※--
			...
			if EnterFight == 1 then
				...
				if BossIsDead ~= 1 then
					...
					Sleep(20)
					Anton = CreateObjByFlag( 106382 , 781125 , 1 , 1 )
					AddToPartition( Anton , Room )
					BeginPlot( Anton , "sasa_106382_AI" , 0 )
					Setplot( Anton , "dead" , "sasa_106382_Dead" , 30 )
					Sleep(20)
...
--※ AE ※--
				elseif WindCounter >= 16 then
					...
					--慢慢來，別急！
					BossTarget = OwnerID()
					CastSpellLv( OwnerID() , BossTarget , Wind , 16 )		--鐮風懸影
					Sleep(10)
					WindCounter = 0
--※ RAND ※--
				elseif RandCounter >= 7 then
					sasa_106382_AntonSkill()
					RandCounter = 0
				end
			end
		end
...
--※ RAND 7S. SKILL ※--
function sasa_106382_AntonSkill()
			...
			if PoisonCount >= 6 then
				...
				if 	( AntTarget ~= nil) 
				 and ( ReadRoleValue( AntTarget , EM_RoleValue_IsDead ) == 0 ) then
					CastSpellLV( OwnerID() , AntTarget , 496407 , 11 )				--腐蝕黏液
					Sleep(10)
				end
				PoisonCount = 0
			end
