function ic_103945_01()   ------主控器     701986
	...
	local Boss=CreateObjByFlag(103945,780699,1,1) 	-----BOSS創在  旗標1
            ...
            local NPCXX = SearchRangeNPC ( OwnerID() , 300 )----自身範圍300  掃區域找NPC
				for i = 0 , table.getn(NPCXX)	do
					if NPCXX[i] ~= OwnerID() then
						if	ReadRoleValue(NPCXX[i],EM_RoleValue_OrgID) == 	104180	then  ---找固定ID NPC
							DelObj( NPCXX[i] )
						end
						if	ReadRoleValue(NPCXX[i],EM_RoleValue_OrgID) == 	104181	then  ---找固定ID NPC
							DelObj( NPCXX[i] )
						end
					end
				end		
				...
				DelObj(door2)										--刪除阻擋門
				sleep(50)                                           --5秒
				Boss=CreateObjByFlag(103945,780699,1,1)           --再創造1之				
	...
	while 1 do 
		...
		if HPPercent >=0.71 then  ------血量>=71% 用這技能
			...
			if time1>=7 then 
				...
				--say(OwnerID(),"1")
				CastSpellLV( OwnerID() , BossTarget , 495838, 300 )----單體技能
				elseif Luck == 2 then 
				while 	ReadRoleValue(OwnerID() , EM_RoleValue_SpellMagicID  ) ~= 0   do--------有沒有正在施放技能
					sleep(10)
				end
...
function ic_103945_05 () -----小怪AI-1 法師
...
function ic_103945_06 () -----小怪AI-2 戰士
	local boss=ReadRoleValue(OwnerID(),EM_RoleValue_PID)
	local AttackTarget = ReadRoleValue( boss , EM_RoleValue_AttackTargetID )   ------ 找尋BOSS的攻擊目標  
	local BossTarget=0
