--門口：/GM ? Zone 148 0 -539.7 1060.1 -478.4
--781125--Flag1：Anton
--781125--Flag2：前門
--781125--Flag3：後門
--781125--Flag4+5：出兵點
...
function sasa_107307_Counter()
	local Count = OwnerID()
	local Room = ReadRoleValue( Count , EM_RoleValue_RoomID )
	local Anton = CreateObjByFlag( 107307 , 781125 , 1 , 1 )
	AddToPartition( Anton , Room )
	BeginPlot( Anton , "sasa_107307_AI" , 0 )
	Setplot( Anton , "dead" , "sasa_107307_Dead" , 30 )
					...
					if table.getn(TakePlayer) >= 1 then						--大於1人就rand 1個
						for xx = 1 , 1 do
							...
							if ( TakePlayer[Lucky] ~= nil ) and ( ReadRoleValue( TakePlayer[Lucky] , EM_RoleValue_IsDead ) == 0 ) then
								local Ant03 = CreateObjByFlag( 107308 , 781125 , 4 , 1 )
								...
								BeginPlot( Ant03 , "sasa_107308_takeaway" , 0 )
								--Setplot( Ant03 , "collision" , "sasa_106383_takeaway" , 20 )
								Setplot( Ant03 , "dead" , "sasa_107308_Dead" , 30 )
...
--※ 重置死亡 ※--
			...
			if EnterFight == 1 then
				...
				if BossIsDead ~= 1 then
					...
					Sleep(20)
					Anton = CreateObjByFlag( 107307 , 781125 , 1 , 1 )
					AddToPartition( Anton , Room )
					BeginPlot( Anton , "sasa_107307_AI" , 0 )
					Setplot( Anton , "dead" , "sasa_107307_Dead" , 30 )
					Sleep(20)
...
--※ AE ※--
				elseif WindCounter >= 16 then
					...
					--慢慢來，別急！
					BossTarget = OwnerID()
					CastSpellLv( OwnerID() , BossTarget , Wind , 7 )		--鐮風懸影
					Sleep(10)
					WindCounter = 0
--※ RAND ※--
				elseif RandCounter >= 7 then
					sasa_107307_AntonSkill()
					RandCounter = 0
				end
			end
		end
...
--※ RAND 7S. SKILL ※--
function sasa_107307_AntonSkill()
			...
			if PoisonCount >= 6 then
				...
				if 	( AntTarget ~= nil) 
				 and ( ReadRoleValue( AntTarget , EM_RoleValue_IsDead ) == 0 ) then
					CastSpellLV( OwnerID() , AntTarget , 496407 , 3 )				--腐蝕黏液
					Sleep(10)
				end
				PoisonCount = 0
			end
