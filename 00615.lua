...
--控制對話內容--
	if ( ReadRoleValue(OwnerID(), EM_RoleValue_SkillMax_Mining ) >= 80 )  then --檢查有大師以上資格的話
		SetSpeakDetail( OwnerID(), GetString ( "ST_110814_5" )   );--#了不起！你的成就已經超過我了！
	elseif ( ReadRoleValue( OwnerID() , EM_RoleValue_Skill_Mining ) == 60 ) then  --等級60提示要玩家去黑曜石要塞
		SetSpeakDetail( OwnerID(), "[SC_110814_MASTER_HINT]"  );--提示去達拉尼斯
	elseif ReadRoleValue( OwnerID() , EM_RoleValue_Skill_Mining ) >= 40  then --等級大於40則叫玩家去找另一個NPC
		SetSpeakDetail( OwnerID(), GetString ( "ST_110814_3" )   );--#哈哈！我的程度沒辦法再教你什麼了，你該去找專業導師邁恩．迪格學習了吧！
	elseif(    ReadRoleValue( OwnerID() , EM_RoleValue_Skill_Mining  ) == 20 ) then  --等級等於20時判斷是否可接任務
		--###技能達到進階標準時加入任務###--
		if not tolimit  then
			LoadQuestOption( OwnerID() );
		else --告知玩家所學工作技能超過了限制
			SetSpeakDetail( OwnerID(), GetString ( "ST_110814_2" )   );--#你的技能已經鍛鍊的很不錯啦！可惜，你學了太多其他的技能，這樣我就沒辦法傳授你工匠的秘訣囉！
		end
		--################################--
	else --0~40級的對話
		SetSpeakDetail( OwnerID(), GetString("ST_110814_0")   );--/*我就是礦工工頭歐路特，關於挖礦的事情儘管問我！我不一定答得出來就是了，哈哈！
	end

--控制對話選項--
	if( ReadRoleValue ( OwnerID(), EM_RoleValue_SkillMax_Mining ) >= 40) then --擁有工匠以上資格則出現可以放棄資格的選項
		AddSpeakOption( OwnerID(), TargetID( ), GetString("SO_DEGRADE_DIALOG") , "LuaS_110814_2", 0 ) --/*我想放棄目前的資格
	...
	if ( CheckFlag( OwnerID(), 540269 ) == true ) then --如果會採礦
		...
		AddSpeakOption( OwnerID(), TargetID( ), GetString ( "SO_110814_1") , "LuaS_110814_1", 0 );--#我想學習挖礦。
	end

--常態對話選項--
	AddSpeakOption( OwnerID(), TargetID( ), GetString("SO_FORMULA_STORE") , "Lua_110814_RecipeList_Touch", 0 )  --/*我想看看你賣的配方。--讓不會的人也能看看能做什麼東西	
...
function LuaS_110814_1()
	...
	SetSpeakDetail( OwnerID(), GetString("ST_110814_1")   );--/*你想學挖礦啊！我的確可以教你一點基礎的，但挖礦很辛苦喔！
								--/*首先，你必須有一個工具，我先送你一個吧！
...
function LuaS_110814_2()
	...
	AddSpeakOption( OwnerID(), TargetID( ), GetString("SO_GETVOC_NO"), "LuaS_110814_0", 0 ); --/*我再考慮看看。
