function yuyu_flag2_1_00()                                                                                                 
	local A = 10
	local X = Rand( A )                                                                                        --Rand(隨機旗子的數量)
	local DDD = CreateObjByFlag( 100652, 780205 , X , 1 );                             --參照指定物件的位置來產生物件　(產生的物件ID，旗子ID，隨機的旗號，產生的數量)
	WriteRoleValue( DDD , EM_RoleValue_PID , X )			       --將DDD的位置X值存在PID裡
                BeginPlot(DDD, "yuyu_flag2_1_01" , 0 )                                               
end

function yuyu_flag2_1_01()                                         
	sleep(30)
	AddToPartition( OwnerID() , 0 )                                                             --加進舞台(加入者 , 舞台層級)
	setplot(  OwnerID()  , "dead","yuyu_flag2_1_02",0 )			--死亡劇情
 	SetFightMode (  OwnerID()  , 0, 1, 0, 1 )
end

function yuyu_flag2_1_02()  
	local A = 10
	local X = Rand( A )                                                                                                  
	local DDD = CreateObjByFlag( 100652, 780205 , X , 1 );                      --參照指定物件的位置來產生物件　(產生的物件ID，旗子ID，隨機的旗號，產生的數量)
