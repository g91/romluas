...
----------RAND一個人出來打玫瑰		
				if table.getn(sway1) ~= 0 then
					...
					--玫瑰
					CastSpellLV( OwnerID(), sway1[anyone], 495025 , 0 )
					roseskill = 0
				end	
			end
			--執著
			if skill01 >= 30 then
				...
				CastSpellLV( OwnerID(), OwnerID() , 495465 , 0 )				
				...
				CastSpellLV( OwnerID(), OwnerID() , 495026 , 0 )
				sleep(20)
				skill01 = 0
			end
			if skill02 >= 20 then
...
--------------------------------------------------------------執政官流血技能
function Cang_129_oh_skill_e()
	BeginPlot( TargetID() , "Cang_129_oh_skill_do_e" , 0)
end

function Cang_129_oh_skill_do_e()
...
function Cang_129_oh_dead_e()	
	...
	BeginPlot( corpse, "Cang_129_oh_corpse_e", 0)
	--SetPlot( corpse, "dead", "Cang_129_oh_corpse_e", 0) 
	--KillID( corpse, corpse )
	--寶箱
	local Box = CreateObjByFlag( 104049, 780670, 10, 1)	--寶箱
	...
	while true do 
		...
		if table.getn( player )~= 0 then
			for i = 0, table.getn(player) do
				...
				CheckBuff( player[i] , 506986) == true then				--檢查有沒有111111
			...
			if table.getn(buf) >= 3 then					--如果集合理面有三個人
				...
				GetDistance( Buf[1], Buf[3]) <= 25 then
					CancelBuff( Buf[1] , 506986 )
					CancelBuff( Buf[2] , 506986 )
					CancelBuff( Buf[3] , 506986 )
					DelObj( OwnerID())
					break
				end	
			end	
...
--------------------------------------------------------------要離開的BUFF			
function Cang_129_oh_buffskill_out_e()
	...
	while true do 
		...
		if table.getn( player )~= 0 then
			for i = 0, table.getn( player ) do
				...
				CheckBuff( player[i] , 506987) == true then
			...
			if table.getn(buf) >= 3 then
				...
				GetDistance( Buf[1], Buf[3]) > 100 then
					CancelBuff( Buf[1] , 506987 )
					CancelBuff( Buf[2] , 506987 )
					CancelBuff( Buf[3] , 506987 )	
					DelObj( OwnerID())
					break
				end
			end	
