function Lua_apon_hall_timer() ---計時器怪物專用
	while 1 do 
		sleep(10)
		...
		while 1 do
			...
			counter1 = counter1 + 1
			counter2 = counter2 + 1
			if counter2 >= 480 and Check_Boss == 0 then ---進入狂暴刪掉計時器（600）
				Check_Boss = 1
				CancelBuff(Boss,504215)
				CancelBuff(Boss,504216)
				AddBuff(Boss,504260,0,-1) ---Mod_3
				--say (OwnerID(),"BYE BYE")
				--Delobj(OwnerID())
...
function Lua_apon_hall_2_Die()---Boss死亡劇情
...
function Lua_apon_hall_2_AI()---Boss本體AI
	...
	while 1 do 
		...
		local SkillLv = {0 , 8 , 0 , 0 , 0 , 0 , 0}  ---技能等級
...
------------------4. 見過地獄嗎？ 格拉默將達成螻蟻們的願望！  5.懺悔吧！悔恨吧！ 螻蟻！ 6. 無知的族類！你已無任何的機會了！再多的懺悔也無法阻止這一切的發生！   7. 螻蟻！用脆弱的肉體來體會真正的毀滅吧！！
		...
		local Luck ----隨機出招用
		local king_rebon = 0
		SetPlot( OwnerID(),"dead","Lua_apon_hall_2_Die",50 )  --死亡劇情，開門用
		while 1 do	
			sleep(10)
			...
			if  ( AttackTarget ~= 0 ) and ( CheckID( OwnerID() ) ) == true then
				...
				if CombatBegin == 0 then ---戰鬥啟動時發動
					...
					local Boss_Door_1 = CreateObjByFlag( 102583 , 780389 , 3 , 1 )---封住入口門
					...
					sleep(15)
					CastSpellLV( OwnerID() , BossTarget , 493714 , 8 ) --開場小菜
					sleep(10)
					ScriptMessage( OwnerID() , -1 , 2 , "[SC_102475_14]" , 1 )---就在此長眠吧！
					--Yell( OwnerID() , "[SC_102475_14]" , 5)
					sleep(20)
					
					local Timer = CreateObjByFlag( 102474 , 780385 , 1 , 1 )---種出時間記數器
					...
					Show(Timer,roomid1)
					local Mob_killer = CreateObjByFlag( 102557 , 780387 , 1 , 1 )----生出一個殺小機器人用的怪物 Vistoar
					...
					if counter_p1_1 >= 2 and SW_Mob == 0 then ---出追人小怪，只出一隻
						...
						for i = 1 , 100 , 1 do
							...
							if ReadRoleValue( MobTarget , EM_RoleValue_IsDead ) ~= 1 then  ----將隨機的一人設定成目標
								...
								local Mob = CreateObj( 102477 , Mob_X+80 , Mob_Y , Mob_Z ,Mob_Dir , 1 ) 
					...
					elseif counter_p1_2 >=(9+RD_time) and SW_Axe == 0 then
						...
						for i = 1 , PPL , 1 do 
							...
							if ReadRoleValue( Mobtarget_1 , EM_RoleValue_IsDead ) ~= 1 then  ----將隨機的一人設定成目標
								...
								local Axe = CreateObj( 102478 , Axe_X , Axe_Y , Axe_Z ,Axe_Dir , 1 ) ---將斧頭生在該目標的點上
					...
					if counter_p2_1 >= 2 and SW_Mod_2_1 == 0 and counter_p2_4 <= 27 then ---選三個人階段
						...
						for i = 1 , 3 , 1 do ---取三個玩家
							...
							if HateTable_Now[Rand_Chose] ~= nil then 
								...
								if i == 1 then ---102643---不在固定點生出，以在目標玩家附近生出
									...
									local Mob_1 = CreateObj( 102643 , Mob_X_1+80 , Mob_Y_1 , Mob_Z_1 ,Mob_Dir_1 , 1 ) 
								...
								elseif i == 2 then ---102476
									...
									local Mob_2 = CreateObj( 102476 , Mob_X_2+80 , Mob_Y_2 , Mob_Z_2 ,Mob_Dir_2 , 1 ) 
								...
								elseif i == 3 then  ----102560
									...
									local Mob_3 = CreateObj( 102560 , Mob_X_3+80 , Mob_Y_3 , Mob_Z_3 ,Mob_Dir_3 , 1 ) 
				...
				elseif CheckBuff(OwnerID() , Mode3) == true then ---狂爆狀態
					counter_p3_1 = counter_p3_1 + 1
					counter_p3_2 = counter_p3_2 + 1
					
					if counter_p3_1 >= 1 then ---狂爆時說的臺詞
						SkillIndex = 6
						sleep(10)
						ScriptMessage( OwnerID() , -1 , 2 , SkillString[SkillIndex] , 1 )
						Yell( OwnerID() , SkillString[SkillIndex] , 5)
				...
				if king_rebon == 1 then
					...
					Delobj(Clear_8)
					local king = Lua_DW_CreateObj( "flag" ,102475, king_flag , 2,1,1)---將Boss生一隻新的
					Delobj(OwnerID())
				end
			end
		end
...
function Lua_apon_hall_mob_dead_1() ---Mod_1 機器人專用
