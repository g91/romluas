...
function un_zone19_boss5_5_02()--FOR蠟燭***************************
...
function un_zone19_boss5_8_02() --FOR肉************************
...
function un_zone19_boss5_15_02()
local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)
	while 1 do
	sleep(10)
		if un_foodlistcount2[RoomID][1]==1 then
		Delobj(OwnerID())
		end
	end	
end
function un_zone19_boss5_13_02()----*****掛在控制器上面讓食物產生
	local FlagObjID = 780793  --旗幟物件編號 
	local FlagNum = GetMoveFlagCount( FlagObjID ) --旗幟數量
	local Obj = {118042,118043,118044,118045}--種植物件編號
...
function un_zone19_boss5_allconreal_1_02() 						   ----總中控(控制BOSS+演戲+轉階段)                                  
	local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)  --讀取出現在的RoomID，並宣告成RoomID
	local Ctrl = OwnerID()       							   --宣告Ctrl = OwnerID() ， 避免重複呼叫
	un_zone19skilltime2[RoomID] = {}
	un_zone19skilltime2[RoomID][1] = 25
	un_foodlistcount2[RoomID] = {}
	un_foodlistcount2[RoomID][1] = 0
	un_changeskillcon2[RoomID] = {}
	un_changeskillcon2[RoomID][1] = 0
	--un_zone19skilltime2[RoomID] = 25  --小怪階段時間
	--un_foodlistcount2[RoomID] = 0
	--un_changeskillcon2[RoomID] = 0  
	...
	Beginplot(littlemonster,"un_zone19_boss5_fight_02",0)         --讓littlemonster1執行範圍劇情的Script
	...
	local monster = {105635,105636,105637}					   --要出的小怪的table
	...
	local Boss                                                 --Boss
	Boss = CreateObjByFlag( 105352, 780838 , 1 , 1 )           --依據旗標生出Boss
	...
	Setplot(Boss,"dead","un_zone190_boss5Die_02",0) 
	local box
	...
	--say(Ctrl,"skillcount = "..un_changeskillcon2[RoomID][1])
		sleep(10)                                              --讓while迴圈每停一秒後才開始執行
		if un_changeskillcon2[RoomID][1] == 1 then                            --小怪階段*********************************
			un_zone19skilltime2[RoomID][1]=un_zone19skilltime2[RoomID][1]+1                              --計數用
			...
			un_foodlistcount2[RoomID][1]  = 0
				...
				Beginplot(foodCtrl,"un_zone19_boss5_13_02",0)
				...
				Beginplot(door,"un_zone190_doordel_2",0)
				AddToPartition(door,RoomID) 
				WriteRoleValue(Ctrl,EM_RoleValue_register3,1)              --宣告城1代表出過食物階段了
			end
			if un_zone19skilltime2[RoomID][1]%30==0 and un_zone19skilltime2[RoomID][1]<270 then          --每30秒執行一次
				for i = 1 , table.getn(monster) do             --出怪的數量 = for迴圈執行次數， table.getn()取得陣列數量
					...
					Beginplot(monster1,"un_zone19_monster_02",0)
					Beginplot(monster2,"un_zone19_monster_02",0)
					
				end
			end
			if un_zone19skilltime2[RoomID][1] == 250 then  
				ScriptMessage( Boss, -1 , 2 , "[SC_105099_09]" , 2 )
			end
			if un_zone19skilltime2[RoomID][1] >= 270 then                           --400秒以後結束出怪階段(判斷玩家有無任務需要進入的階段)
				un_zone19skilltime2[RoomID][1] = 25                                  --先階段歸0，避免再出怪
				Delobj(foodCtrl)                               --刪除食物中控器避免再出食物
				un_foodlistcount2[RoomID][1]  = 1
				WriteRoleValue(Ctrl,EM_RoleValue_register3,0)  --產出食物變術再宣告回0，讓重置時能再出食物
				un_changeskillcon2[RoomID][1] = 2                             --接著改成正常BOSS戰階段
				...
				for i = 1 , player do                          --幾個玩家執行幾次for迴圈，判斷所有玩家
					...
					CheckFlag( playername, 544639 ) ~= true then            --判斷玩家是否有完成此任務的旗標
						un_changeskillcon2[RoomID][1] = 3                                   --只要有玩家有任務就跳到任務階段
					end
				end
			end
		elseif un_changeskillcon2[RoomID][1] == 2 then                        --Boss階段*********************************
			ScriptMessage( Ctrl, -1 , 2 , skilltalk[2] , 2 )   --Boss開戰對話
			sleep(10)
			
			Beginplot(Boss,"un_zone19_Boss5AI_01_02",0)           --直接塞Script給Boss執行
			un_changeskillcon2[RoomID][1] = 6                                 --再把模式切換到待機，避免重複執行Script
			--Setplot(Boss,"dead","un_zone190_boss5Die_022_02",0)    --丟死亡劇情給BOSS
			
		elseif un_changeskillcon2[RoomID][1] == 3 then                        --Boss階段(演戲)***************************
			ScriptMessage( Ctrl, -1 , 2 , skilltalk[2] , 2 )
			Addbuff(Boss,502707,0,-1)                          --給Boss不能回血的BUFF
			Beginplot(Boss,"un_zone19_Boss5AI_02_02",0)           --直接塞Script給Boss執行
			un_changeskillcon2[RoomID][1] = 6                                 --再把模式切換到待機，避免重複執行Script
			--Setplot(Boss,"dead","un_zone190_boss5Die_02",0)       --丟死亡劇情給BOSS
			
		elseif un_changeskillcon2[RoomID][1] == 4 then                        --重置階段*********************************
			ScriptMessage( Ctrl, -1 , 2 , skilltalk[3] , 2 )
			Delobj(Boss)                                        --刪除BOSS
			delobj(door)
			un_changeskillcon2[RoomID][1] = 0                                 --再將階段寫回0，避免重生
			Boss = CreateObjByFlag( 105352, 780838 , 1 , 1 )   --依據旗標生出Boss
			WriteRoleValue(Boss,EM_RoleValue_register1,Ctrl)   --將Ctrl寫入Boss的EM_RoleValue_register1
			SetModeEx(Boss,EM_SetModeType_Strikback,false)     ---反擊
			SetModeEx(Boss,EM_SetModeType_Fight,false)    	   ---可砍殺
			SetModeEx(Boss,EM_SetModeType_Searchenemy,false)   ---鎖敵 
			AddToPartition(Boss,RoomID)                        --顯現Boss
			Beginplot(littlemonster,"un_zone19_boss5_fight_02",0) --讓範圍觸發戰鬥劇情重新Beginplot
			Setplot(Boss,"dead","un_zone190_boss5Die_02",0) 
			local searnpc = {}
		...
		elseif un_changeskillcon2[RoomID][1] == 5 then
			local searnpc = {}
			searnpc = SearchRangeNPC ( OwnerID() , 500 )
			for i=0 , table.getn(searnpc) do
				if ReadRoleValue(searnpc[i],EM_RoleValue_OrgID) ==105106 then
...
function un_zone19_boss5_fight_02()                     		   --小怪中控器script(範圍觸發)
	...
	--else
		if skill == 0 then
			SetPlot( fight,"range","un_zone19_boss5_fight_02_2_02",100 ) --範圍100內觸發script
		end
	--end	
end


function un_zone19_boss5_fight_02_2_02()                     	  	        --小怪中控器script
local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID) 
	...
	if ReadRoleValue(OwnerID(),EM_RoleValue_VOC)==0 or  ReadRoleValue( OwnerID() , EM_RoleValue_IsDead)==1 then
		--SetPlot( tar,"range","un_zone19_boss5_fight_02_2_02",100 )
		Beginplot(tar,"un_zone19_boss5_fight_02",0)
	else	
		un_changeskillcon2[RoomID][1] = 1   	--將控制器的Reg1寫成1代表模式1小怪模式
	end
end

function un_zone19_Boss5AI_01_02()                                --BossAI_Script正常階段******
	local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)  --讀取出現在的RoomID，並宣告成RoomID 
	...
	while 1 do
		...
		if hatelistcount(Boss)~=0 then                             --Boss仇恨列表有人=進入戰鬥
			...
			if skilltime%7 == 0 then
				...
				Targ=ReadRoleValue(OwnerID(),EM_RoleValue_AttackTargetID)--BOSS的當錢目標
                 --給隨機一人buff
				CastSpelllv( OwnerID(), Targ,497498 ,0 )           --對當前目標放毒爪
			...
			sleep(10)
				...
				Addbuff(player[rand],509598,0,6)  
			...
			sleep(10)
				...
				if table.getn(Play_Chose)<2 then                   --判斷如果BOSS仇恨列表<6人
					for i=1 , table.getn(Play_Chose) do            --對仇恨列表內所有人放招
						sysCastSpelllv(OwnerID(),Play_Chose[i],497511,0)--系統放傷害招
					end
				end
				if table.getn(Play_Chose)>=2 then                   --判斷如果BOSS仇恨列表>6人
					for i=1 , 2 do                                  --對仇恨列表內6人放招
						sysCastSpelllv(OwnerID(),Play_Chose[i],497511,0)--系統放傷害招
					end
				end
				sleep(30)
		...
		elseif 	hatelistcount(Boss)==0 and skill ==1 then           --Boss仇恨表沒人+曾經戰鬥過 =重置階段
			un_changeskillcon2[RoomID][1] = 4                                      --將控制器階段寫到重置4
		end
	end
end


function un_zone19_Boss5AI_02_02()                                   --BossAI_Script演戲階段******
	local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)  --讀取出現在的RoomID，並宣告成RoomID 
	local Boss = OwnerID()                                     --宣告Boss=OwnerID()避免重複宣告造成效能消耗
	local Ctrl = ReadRoleValue(Boss,EM_RoleValue_register1)    --讀取Boss的reg1為控制器的GID，並宣告成Ctrl
	local skilltime = 0                                        --宣告主要時間 skilltime = 0 
	...
	while 1 do
		...
		if hatelistcount(Boss)~=0 then                             --Boss仇恨列表有人=進入戰鬥
			...
			if Hp<=40 and mpcskill==1 then                         --血量低於70%，且階段再0的時候
				...
				sleep(20)
				SetStopAttack(ika)
				Beginplot(ika,"un_zone19_ika_02",0)
				SetModeEx(Boss,EM_SetModeType_Strikback,true) 		   ---反擊
				SetModeEx(Boss,EM_SetModeType_Fight,true)    		   ---可砍殺
				SetModeEx(Boss,EM_SetModeType_Searchenemy,true)	       ---鎖敵
...
function un_zone19_monster_02()                                      --小怪離開戰鬥判斷重置階段******
local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID) 
	...
	while 1 do                                                    --每秒執行while迴圈
		...
		if hatelistcount(own)==0 then                             --判斷小怪本身有無進入戰鬥，==0則仇恨表無人
			...
			if skilltime ==15 then                                 --未進入戰鬥持續10秒
				...
				un_changeskillcon2[RoomID][1] = 0                                --將階段寫回0，階段0 = 待機
				WriteRoleValue(Ctrl,EM_RoleValue_register3,0)
				SetPlot( Ctrlball,"range","un_zone19_boss5_fight_02_2_02",100 )--將觸發戰鬥的球重新寫回未觸發
				un_zone19skilltime2[RoomID][1] = 25
				Delobj(own)                                       --刪除小怪本身
			end
...
function un_zone19_ika_02()                        --演戲撿東西
	local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)
...
function un_zone190_boss5Die_02()
	local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)
	...
	un_changeskillcon2[RoomID][1] = 5
	ScriptMessage( OwnerID(), -1 , 2 ,"[SC_105099_06]" , 2 )
end
function un_zone190_boss5Die_022_02()
local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID) 
	un_changeskillcon2[RoomID][1] = 5
	ScriptMessage( OwnerID(), -1 , 2 , "[SC_105099_06]"  , 2 )
end

function un_zone19_boss5use_3_02()  --直接出任務BOSS
local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID) 
	un_changeskillcon2[RoomID][1] = 3
end

function un_zone19_boss5use_1_02()  --結束出小怪階段
local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID) 
	un_zone19skilltime2[RoomID][1]= 268
end
function un_zone19_hit1_02()                        --Boss丟毒水的判斷1
	local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)
	...
	AddToPartition(ball,RoomID )
	beginplot(ball,"un_zone19_hit2_02",0)
end

function un_zone19_hit2_02()                        --Boss丟毒水的判斷2
	AddBuff( OwnerID() ,506823 ,0 ,-1 )
	while true do
	sleep(10)
		sysCastSpellLv(OwnerID(),OwnerID(),497499,0)
	end	
end
function un_zone190_doordel_2()
local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID) 
	while 1 do
		sleep(50)
		if un_changeskillcon2[RoomID][1] == 0 or un_changeskillcon2[RoomID][1] == 4 or un_changeskillcon2[RoomID][1] == 5 then
			delobj(OwnerID())	
		end
	end
end