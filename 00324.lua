...
--控制對話內容--
	if ( ReadRoleValue(OwnerID(), EM_RoleValue_SkillMax_Tailor  ) >= 80 )  then --檢查有大師以上資格的話
		SetSpeakDetail( OwnerID(), GetString ( "ST_110417_5" )   );--#以閣下的程度，我已經無法以老師的身分來指導你了，但我們仍是朋友，希望有空的時候你能常回來談談裁縫。
	elseif ( ReadRoleValue( OwnerID() , EM_RoleValue_Skill_Tailor) == 60 ) then  --等級60提示要玩家去黑曜石要塞
		SetSpeakDetail( OwnerID(), GetString ( "ST_110417_4" )   );--#如果你想往更高境界邁進，就去尋找傳說中的礦物大師吧！聽說黑曜石要塞有他的消息。
	elseif ReadRoleValue( OwnerID() , EM_RoleValue_Skill_Tailor  ) >= 40  then --等級大於40則叫玩家去找另一個NPC
		SetSpeakDetail( OwnerID(), GetString ( "ST_110417_3" )   );--#我很樂意繼續指導你這樣的學生，但這樣對你並不好，我的母親瑞秋夫人才能夠幫助你更進一步。
	elseif(    ReadRoleValue( OwnerID() , EM_RoleValue_Skill_Tailor ) == 20 ) then  --等級等於20時判斷是否可接任務
		--###技能達到進階標準時加入任務###--
		if not tolimit  then
			LoadQuestOption( OwnerID() );
		else --告知玩家所學工作技能超過了
			SetSpeakDetail( OwnerID(), GetString ( "ST_110417_2" ) )  --#你的裁縫能力應該可以在更上層樓，不過你現在看起來並沒有多餘的心力可以學習更高深的裁縫技術；沒關係，等你需要更多裁縫的訓練的話，我隨時在這裡等候。
		end
		--################################--
	else --0~40級的對話
		SetSpeakDetail( OwnerID(), GetString("ST_110417_0")   );
		--/*裁縫師山姆．泰勒，很高興認識你，請問你是要訂做服裝呢？還是學習裁縫呢？
	end

--控制對話選項--
	if( ReadRoleValue ( OwnerID(), EM_RoleValue_SkillMax_Tailor ) >= 40) then
		AddSpeakOption( OwnerID(), TargetID( ), GetString("SO_DEGRADE_DIALOG" ), "LuaS_110417_2", 0 ) --/*我想放棄目前的資格
	...
	if ( ReadRoleValue( OwnerID() , EM_RoleValue_SkillMax_Tailor  ) > 0) then --如果會裁縫
		...
		AddSpeakOption( OwnerID(), TargetID( ), GetString("SO_110417_1") , "LuaS_110417_1", 0 );	--/*我想學習裁縫。
	end

--常態對話選項--
	AddSpeakOption( OwnerID(), TargetID( ), GetString("SO_FORMULA_STORE") , "Lua_110417_RecipeList_Touch", 0 )  --/*我想看看你賣的配方。--讓不會的人也能看看能做什麼東西	
	AddSpeakOption( OwnerID(), TargetID( ), GetString ( "SO_LIVING_SAY") , "LuaS_LIVING_SAY", 0 );--#什麼是生活技能？

end
--------------------------------------------------------------------------------------------------------------------------
function LuaS_110417_1()

	SetSpeakDetail( OwnerID(), GetString("ST_110417_1" )  );--/*好！現在就教你裁縫的基本。要做裁縫工作的話，身邊必須要有裁縫器具。然後你可以打開系統列中的「生活技能」選擇「裁縫」，只要材料準備齊全可以開始進行製造列表中的裁縫品了。

	AddSpeakOption( OwnerID(), TargetID( ), GetString ( "SO_IKNOW") , "LuaS_110417_3", 0 );--

end
--------------------------------------------------------------------------------------------------------------------------
function LuaS_110417_2()
	...
	AddSpeakOption( OwnerID(), TargetID( ), GetString("SO_GETVOC_NO"), "LuaS_110417_0", 0 ); --/*我再考慮看看。

end

--------------------------------------------------------------------------------------------------------------------------
function LuaS_110417_3()
