function Lua_apon_Zone122_TimerGate_AI() ---後段
	...
	while 1 do
		...
		local Door = 103515 ----隱形門
		local TimerGate_Boss = 103056 ---中控AI
		local Flag_Key_gate = 780380 ----紅藍色石柱用   
		local Flag_lastdoor = 780384 ---最後的阻擋門
		local Flag_Door = 780363 ----每一個隱形門
		local Flag_TimerGate_Boss = 780376 ---中控AI 的旗標
		local Flag_TimerGate_Keydoor = 780377
		local Flag_Fin_door = 780381 --- 最後的魔法門
		local Flag_Gate_Boss = 780378 ---- 中間的Boss 旗標 --- 1 = 第一隻 ＆ 2= 第二隻 
		local Flag_Gate_Boos_door = 780379 ---中間Boss 兩側的門 1 2 = 第一隻 ， 3 4 = 第二隻
		local Table_Door = {}
		------    警告訊息 -------   1. 殺了他們！殺了那些入侵者！   2.  1.  入侵者！你認為你逃得了嗎？  3. 別妄想可以活著走出去了！入侵者！
		local All_String_1 = {"[SC_102969_16]","[SC_102969_17]","[SC_102969_18]"}
		
		for i = 1 , 9, 1 do ----生門的迴圈
			--say(OwnerID(),"i="..i)
			...
			SetPlot( Table_Door[i] , "Collision" , "Lua_apon_Zone122_TimerGate_1" , 10 ) ----碰撞劇情，啟動隱形門生小怪
		end
		...
		local Boss_1 = CreateObjByFlag( 103066 , Flag_Gate_Boss , 1 , 1 )
		AddToPartition( Boss_1,roomid1 )
		BeginPlot( Boss_1 , "Lua_apon_Zone122_Gate_1",5 )
		SetPlot(Boss_1,"dead","Lua_apon_Zone122_Gate_Dead_1",10 )
		WriteRoleValue( OwnerID(), EM_RoleValue_Register3, Boss_1)
		
		local Boss_2 = CreateObjByFlag( 103067 , Flag_Gate_Boss , 2 , 1 )
		AddToPartition( Boss_2,roomid1 )
		BeginPlot( Boss_2 , "Lua_apon_Zone122_Gate_2",5 )
		SetPlot(Boss_2,"dead","Lua_apon_Zone122_Gate_Dead_1",10 )
		WriteRoleValue( OwnerID(), EM_RoleValue_Register4, Boss_2)
		
		local Gate_1 = CreateObj( 114342 , -4273 , 650 , 1041 , 269 ,1) ---藍色石柱
		...
		local Title_counter = 0 ---取得稱號用的計數
		local Check_Title = 0
		while 1 do 
			sleep(10)
			...
			--DeBugMsg(0,0,"Title_counter =="..Title_counter )
			...
			if Check_Title == 1 and Title_counter <= 450 then ---對副本中的每個人都給予稱號
			...
			if Check_run_1 == 1 and Check_run_2 == 1 then
				Check_Title = 1 ----獲得取得稱號的權利
				--if Check_String == 0 then
					ScriptMessage( OwnerID() , -1 , 2 , "[SC_102969_25]" , 1 )----忽然一股能量從兩旁的石柱中竄了出來 .....
					--Check_String = 1
				--end
				for i = 1 , 9 , 1 do ---生出六個終止追兵的特效球
					local Over_Ball = CreateObjByFlag( 103056 , 780383 , i , 1 )
					...
					BeginPlot( Over_Ball, "Lua_apon_Over_Ball", 0)
				end
			...
			if Check_start == 1 and Check_Begin == 0 then
				...
				Check_Counter = 0
				String_Counter = 0
				ScriptMessage( OwnerID() , -1 , 1 , "[SC_102969_15]" , 1 )
			end
...
function Lua_apon_Zone122_Gate_1() ---小 Boos 一號
...
function Lua_apon_Zone122_Gate_2() ---小 Boos 二號
	--SetPlot(OwnerID(),"dead","Lua_apon_Zone122_TimerGate_Boss_Dead_1",10 )
	local roomid1 = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	local Flag_Gate_Boos_door = 780379 ---中間Boss 兩側的門 1 2 = 第一隻 ， 3 4 = 第二隻
	local Door = 102460
...
function Lua_apon_Zone122_Gate_Dead_1()
	local roomid1 = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	...
	BeginPlot(Ball,"Lua_apon_Zone122_Gate_Dead_2",10)
end

function Lua_apon_Zone122_Gate_Dead_2()	
	local roomid1 = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	local Boss = ReadRoleValue(OwnerID(),EM_RoleValue_Register1)
	ScriptMessage( OwnerID() , -1 , 2 , "[SC_102969_20]" , 1 ) ----兩旁的魔法能量逐漸消失散去
...
-----玩家 = owner
------門 = target
function Lua_apon_Zone122_TimerGate_1()
	...
	elseif ReadRoleValue( OwnerID(), EM_RoleValue_VOC) ~= 0 then	
		...
		BeginPlot( TargetID() , "Lua_apon_Zone122_TimerGate_2",0 )
	end
end
...
function Lua_apon_Zone122_TimerGate_2() ----目的在於生小怪
	local roomid1 = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	...
	while 1 do
		...
		if Counter_mob >= 7 and STEP == 0 and Mob_Num <= 10 then
			STEP = 1
			for i = 1 , 2 , 1 do
				local Mob_1 = star_CreateObj( OwnerID() , Mob_1 , 0 , 0 , 0 , roomid1 , 0)
				BeginPlot( Mob_1 , "Lua_apon_Runwar_mob",10 )
				table.insert( Table_mob , Mob_1 )
			end
		
		elseif Counter_mob >= 30 and STEP == 1 and Mob_Num <= 10 then
			STEP = 0
			local Mob_2 = star_CreateObj( OwnerID() , Mob_2 , 0 , 0 , 0 , roomid1 , 0)
			BeginPlot( Mob_2 , "Lua_apon_Runwar_mob",10 )
...
function Lua_apon_Runwar_1() ---藍色符石柱檢查用
	SetPlot( OwnerID() , "touch" , "Lua_apon_Runwar_1_1" , 20 )
end

----Own = 玩家
----Tar = 符石
function Lua_apon_Runwar_1_1()
	if CheckBuff(TargetID(),505626) == true then
		ScriptMessage( OwnerID() , OwnerID() , 2 , "[SC_102969_21]" , 1 )---石柱上的機關已被啟動了 ！ 
		
	else
		if CountBodyItem( OwnerID(), 206703 ) >= 1 then
				SetPlot( OwnerID() , "touch" , "" , 0 )
				ScriptMessage( OwnerID() , OwnerID() , 2 , "[SC_102969_22]" , 1 )-------你慢慢的將手中的符石嵌進石柱之中 ....
				BeginPlot( TargetID() , "Lua_apon_Runwar_1_2",0 )
		elseif CountBodyItem( OwnerID(), 206704 ) >= 1 then
...
------Own = 符石
---------Tar = 玩家
function Lua_apon_Runwar_1_2()
	local Gate_AI = ReadRoleValue(OwnerID(),EM_RoleValue_Register1)
	AddBuff (OwnerID() , 505626 , 0 , -1)---判斷用
	DelBodyItem( TargetID(), 206703, 1 )
	WriteRoleValue( Gate_AI, EM_RoleValue_Register7, 1)
	WriteRoleValue( OwnerID(), EM_RoleValue_Register1, 1) ----之後玩家再點選，則會告訴玩家，此石柱已經被啟動了
end

-------------------------------------------------------------------------------------------------------------------------------
function Lua_apon_Runwar_2() ---紅色符石柱檢查用
	SetPlot( OwnerID() , "touch" , "Lua_apon_Runwar_2_1" , 20 )
end

----Own = 玩家
----Tar = 符石
function Lua_apon_Runwar_2_1()
	if CheckBuff(TargetID(),505626) == true then
		ScriptMessage( OwnerID() , OwnerID() , 2 , "[SC_102969_21]" , 1 )---石柱上的機關已被啟動了 ！ 
		
	else
		if CountBodyItem( OwnerID(), 206704 ) >= 1 then
				SetPlot( OwnerID() , "touch" , "" , 0 )
				ScriptMessage( OwnerID() , OwnerID() , 2 , "[SC_102969_22]" , 1 )-------你慢慢的將手中的符石嵌進石柱之中 ....
				BeginPlot( TargetID() , "Lua_apon_Runwar_2_2",0 )
		elseif CountBodyItem( OwnerID(), 206703 ) >= 1 then
			ScriptMessage( OwnerID() , OwnerID() , 2 , "[SC_102969_23]" , 1 )----------你試著將手中符石嵌進石柱中 ......  但是形狀似乎不太一樣
		else
			ScriptMessage( OwnerID() , OwnerID() , 2 , "[SC_102969_24]" , 1 )-------你仔細的觸摸著石柱，似乎有個凹槽可以放進某個物品......
		end
...
------Own = 符石
---------Tar = 玩家
function Lua_apon_Runwar_2_2()
	local Gate_AI = ReadRoleValue(OwnerID(),EM_RoleValue_Register1)
	AddBuff (OwnerID() , 505626 , 0 , -1)---判斷用
	DelBodyItem( TargetID(), 206704, 1 )
	WriteRoleValue( Gate_AI, EM_RoleValue_Register8, 1)
	WriteRoleValue( OwnerID(), EM_RoleValue_Register1, 1) ----之後玩家再點選，則會告訴玩家，此石柱已經被啟動了
end


function Lua_apon_Runwar_mob() ----隱形門，所產生的怪物使用
	local roomid1 = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	------1.  喔～不～你居然啟動了這 .....     2. 卑鄙的入侵者！不 .......      3.  可惡的入侵者！別以為你可以逃離這裡 ！！
...
function Lua_apon_Runwar_findoor()-----最後的魔法門用的，不斷的施放法術，影響所有的怪物
	while 1 do
		sleep(10)
