...
---	local sbs={}
	...
	local door1 = CreateObj(102419, -1289.3,1266.2,928.4,357.4,1)
		...
		BeginPlot( OwnerID(),"Lua_bk_rd04_op_door_102419",0 )
	local timecount = CreateObjByFlag(102418,780425,2,1)  -----在特定點生出時間計時器 ,時間計時10秒
		...
		BeginPlot( timecount ,"Lua_black_raid04_alltime_102395",0)
	...
	while 1 do 
		...
		if HateListCount( boss ) ~= 0 then	---- 確定他現在有打人
			...
			--	end
				...
				if time2 == 13 then    ---15秒來一顆大球
					...
					local bigball = CreateObjByFlag(102402,780425,1,1)  -----在特定點生大球 ,時間計時10秒
					...
					SetPlot( bigball , "dead" , "Lua_bk_rd04_bigball_dead_102395" , 10 )
					BeginPlot( bigball , "Lua_black_raid04_bigball_102395" , 0 )
					AddBuff(boss,504334,0,50) --檢查頓在不在
					AddBuff(bigball,504082,0,50) --------水晶反彈盾
					...
					BeginPlot(bigball,"Lua_bk_rd04_bossdead_102395",10)					
				end
				...
				if time2 == 60 then   ---剩下最後3秒時候來個大絕招打地板
					...
					CastSpell( OwnerID() , OwnerID() , 493790 )
		...
		---------------------------------------------------第二階段開始	結束	
			if CheckBuff(OwnerID() , Mode1) == true then---第一階段判定
				...
				if time1==18 or time1==36 or time1==54 then
					...
					CastSpell( OwnerID() , BossTarget , 493789 )----使用全體禁言術
				end
				...
				if time1==5 or time1==10 or time1==15 or time1==20 then  ----前面20秒內,每5秒生一個法
					...
					BeginPlot(  ma[m2] ,"Lua_bk_raid04_magic_102417",0)
					WriteRoleValue(ma[m2] , EM_RoleValue_Register3,boss)
					BeginPlot(ma[m2],"Lua_bk_rd04_bossdead_102395",10)
				end
		...
		elseif HateListCount( boss )  == 0 then		-------當王打到一半離開戰鬥
			...
			if CombatBegin==1 then
				...
				m1=0
				Hide(door1)
				local bking = CreateObjByFlag(102395,780425,4,1)---------------把王生在4的位置
				AddToPartition(bking , RoomID )
				SetModeEx( bking   , EM_SetModeType_Move , true)----可以移動
				-------------BeginPlot( OwnerID(),"Lua_bk_rd04_op_door_102419",0 )
				BeginPlot( bking,"Lua_bk_raid04_102395",0 )
				Delobj(timecount)
				Delobj(door)
				Delobj(door1)
				Delobj(boss)
...
--------------------------------------------------------------------------------------------------------------------------------
function Lua_black_ball_dead_102395()  ----電球死亡劇情
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )	
	local ball=OwnerID()
	...
	--------say(bmball,"xxxx")
	CastSpellLv(bmball,bmball,493792,1)
	BeginPlot( bmball ,"Lua_bk_raid04_bmball",10)----把劇情丟到身上去
	DelObj(ball)
end
--------------------------------------------------------------------------------------------------------------------------------
function Lua_bk_rd04_bigball_dead_102395()
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )	
...
--------------------------------------------------------------------------------------------------------------------------------碰到王之後狂暴劇情 把大球消失掉
function Lua_black_raid04_bigball_102395()  -------當球碰到王處發增加傷害劇情
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )	
	local bigball = OwnerID()
	local boss= ReadRoleValue(bigball ,EM_RoleValue_PID)
...
--	sleep(50)
	----sleep(100)
	--	if CheckBuff(boss ,504082 ) == false then
...
function Lua_black_rd04_504082()---------法術去判定時時間結束後讓王正常移動
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )	
	local boss = OwnerID()
...
function Lua_black_raid04_alltime_102395()------------------------時間計時器  時間到會狂暴
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )	
...
function Lua_bk_raid04_magic_102417()
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	local magic = OwnerID()
	while 1 do 
		sleep(10)
	...
	local y=table.getn(npcpl)-----取出來的是數量
		for i=0 , y ,1 do 
			if ReadRoleValue( npcpl[i] , EM_RoleValue_OrgID ) == 102395 then
...
function Lua_bk_raid04_bmball()----------小顆水晶球死亡之後產生爆炸劇情
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	local bmball = OwnerID()
	sleep(20)
	DelObj(bmball)
end
function Lua_bk_rd04_move_102395()---------水晶球出 朝王目標移動施放水晶吸取術
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	local bigball = OwnerID()
	local boss= ReadRoleValue(bigball ,EM_RoleValue_PID)
	CastSpell( boss , bigball , 493921 )
...
function Lua_bk_rd04_op_door_102419()--------open
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	local boss = OwnerID()
...
function Lua_bk_rd04_cl_door_102419()----------close
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	local boss = OwnerID()
	local door = ReadRoleValue( boss , EM_RoleValue_Register4 )	
...
function Lua_bk_rd04_bossdead_102395()-------BOSS死亡之後去刪除其他物件
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	...
	sleep(10)
		if ReadRoleValue( boss , EM_RoleValue_IsDead) == 1 and
			...
			BeginPlot( boss,"Lua_bk_rd04_op_door_102419",0 )
			DelObj(OwnerID())
		end
	end
end