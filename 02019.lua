--------------------------------------------------------------------------------主控制器
function Cang_130_tree_control_6e()
	...
	local Buff = 507380
	--生出boss
	Boss = CreateObjByFlag( 104475, 780711, 0, 1) 
	AddToPartition( Boss , RoomID )	
	WriteRoleValue( Boss , EM_RoleValue_Register1, OwnerID() )
	BeginPlot(Boss,"Cang_130_tree_6e",0)
	SetPlot( Boss, "dead" , "Cang_130_tree_dead_6e" ,0 )	
	
	local Door = CreateObjByFlag( 104481 , 780711 , 5 , 1 )
...
--------------開場執行：time時間階段，buff不可用物品，cy水晶產生，盾檢查，生門
				BeginPlot( OwnerID() , "Cang_130_tree_time_6e", 0)
				ScriptMessage( OwnerID() , -1 , 2 , CountSay[2], 1 )
				BeginPlot( OwnerID() , "Cang_130_tree_shief_6e", 0)
				Door = CreateObjByFlag( 104481 , 780711 , 6 , 1 )
				SetModeEX( Door  , EM_SetModeType_Gravity , false )--重力
				SetModeEx( Door  , EM_SetModeType_ShowRoleHead, false )--不秀頭像框
				SetModeEx( Door  , EM_SetModeType_Obstruct, true )--阻擋
				SetModeEx( Door  , EM_SetModeType_Mark, false )--標記
				...
				BeginPlot( Door , "Cang_130_tree_summomkill_6e", 0)				
				Doorstart = 1		
			end	
			count2 = count2 + 1
			count3 = count3 + 1
...
--------------生命之盾				
				if count1 >= 30 then 
					...
					CastSpellLV( OwnerID() , OwnerID() , 496003 , 1 )
...
--------------降治療技能				
				if count3 >= 10 then
					...
					CastSpellLV( OwnerID() , OwnerID() , SkillAoE[1] , 0 )
					Sleep(10)
					count3 = 0
				end		
...
--------------第二階段				
			elseif CheckBuff( OwnerID(), Mode2) == true then
				...
				if count4 >= 7 then
					...
					if Mconut < 3 then
						...
						if table.getn(mid_tree_1) < 4 then
							...
							BeginPlot( M_tree1 , "Cang_130_Midtree_skill_6e", 0)					
							count4 = 0
						end	
						Mconut = Mconut + 1
					end	
				...
				if count3 >= 15 then
					...
					CastSpellLV( OwnerID() , OwnerID() , SkillAoE[2] , 0 )
					Sleep(20)
					count3 = 0
				end			
				if countskill >= 7 then
...
--------------------------------------------------------------------------------計時
function Cang_130_tree_time_6e()
	...
	BeginPlot( OwnerID() , "Cang_130_tree_buff_6e", 0)
	sleep(1)
	BeginPlot( OwnerID() , "Cang_130_tree_cy_6e", 0)
	while true do
		sleep(10)
		C_time = C_time + 1 
		M1time = M1time + 1
	...
	while true do
		...
		if count3 == 0 then
			...
			for i = 0, 1 do
				...
				BeginPlot( bomb[i] , "Cang_130_tree_bomb_6e", 0)
				--sleep(1)
				--SetPlot( bomb[i] , "dead" , "Cang_130_tree_bombdead_6e" ,0 )
			end	
			count3 = 1	
		end		
		if count1 >= 5 then
			...
			SetPlot( cy, "touch" , "Cang_130_item_touch_6e" ,30 )
			count1 = 0
		end
	end
end
...
-------------------------------------------------------------------------------炸彈
function Cang_130_tree_bombdead_6e()	
	local RoomID = ReadRoleValue(OwnerID() , EM_RoleValue_RoomID)
	local count = 0
	...
	BeginPlot( SpellCaster , "Cang_mage_deadtime_6e" , 0)	
	sleep(1)
	BeginPlot( OwnerID() , "Cang_mage_deadtime_6" , 0)
end

function Cang_130_tree_bomb_6e()
	local count = 0
	local count1 = 0
	BeginPlot( OwnerID() , "Cang_130_tree_summomkill_6e", 0)
	local TempPlayer = SetSearchAllPlayer(ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)) --取現在該區內的所有玩家
	local ThesePlayer = {}
	local sway = {}
	for i=1,TempPlayer do
	...
	while true do
		...
		count1 = count1 + 1
		if count >= 10 then
			--SysCastSpellLV( OwnerID(), OwnerID() , 493624, 0)	
...
--------------------------------------------------------------------------------每秒檢查不可使用物品BUFF
function Cang_130_tree_buff_6e()
...
--------------------------------------------------------------------------------盾檢查
function Cang_130_tree_shief_6e()
	local mark = 0
	while true do		
		sleep(10)
...
--------------------------------------------------------------------------------王死自己刪除自己
function Cang_130_tree_summomkill_6e()
...
--------------------------------------------------------------------------------中樹
function Cang_130_Midtree_skill_6e()
	BeginPlot( OwnerID() , "Cang_130_tree_summomkill_6e", 0)
	sleep(1)
	SetPlot( OwnerID() , "dead" , "Cang_130_Midtree_dead_6e" ,0 )	
	local Boss = ReadRoleValue( OwnerID() , EM_RoleValue_PID )	
...
function Cang_130_Midtree_dead_6e()
	local RoomID = ReadRoleValue(OwnerID() , EM_RoleValue_RoomID)
	local count = 0
	local SpellCaster = star_CreateObj_NoAdd( OwnerID() , 104479 , 0 , 0 , 0 , RoomID , 90)
	SetModeEx( SpellCaster  , EM_SetModeType_HideName, false )--名稱
	SetModeEX( SpellCaster  , EM_SetModeType_Gravity , false )--重力
	SetModeEx( SpellCaster  , EM_SetModeType_NotShowHPMP, true )--血條
	SetModeEx( SpellCaster  , EM_SetModeType_ShowRoleHead, false )--不秀頭像框
	...
	AddToPartition( SpellCaster , RoomID )	--加進舞台	
	if count ==0 then 
		CastSpellLV( SpellCaster , SpellCaster , 496002 , 4 )
		count = 1
	end	
	BeginPlot( SpellCaster , "Cang_mage_deadtime_6e" , 0)
	DelObj( OwnerID() )
...
function Cang_mage_deadtime_6e()
	sleep(30)
	Delobj(OwnerID())
end
--------------------------------------------------------------------------------小樹死亡劇情
function Cang_130_smalltree_dead_6e()	
	--local RoomID = ReadRoleValue(OwnerID() , EM_RoleValue_RoomID)
	--local count = 0
	--local SpellCaster = star_CreateObj_NoAdd( OwnerID() , 104480 , 0 , 0 , 0 , RoomID , 90)
	--SetModeEx( SpellCaster  , EM_SetModeType_HideName, false )--名稱
	--SetModeEX( SpellCaster  , EM_SetModeType_Gravity , false )--重力
	--SetModeEx( SpellCaster  , EM_SetModeType_NotShowHPMP, true )--血條
	--SetModeEx( SpellCaster  , EM_SetModeType_ShowRoleHead, false )--不秀頭像框
	...
	--AddToPartition( SpellCaster , RoomID )	--加進舞台
	--if count ==0 then	
	--CastSpellLV( SpellCaster , SpellCaster , 496153 , 29 )
	--	count = 1
	--end		
	--BeginPlot( SpellCaster , "Cang_mage_deadtime_6e" , 0)	
	DelObj(OwnerID())
...
--------------------------------------------------------------------------------小樹
function Cang_130_Littletree_skill_6e()
	BeginPlot( OwnerID() , "Cang_130_tree_summomkill_6e", 0)
	SetPlot( OwnerID() , "dead" , "Cang_130_smalltree_dead_6e" ,0 )
	local count = 0
	local count1 = 0
	local Boss = ReadRoleValue( OwnerID() , EM_RoleValue_PID )		
...
--------------------------------------------------------------------------------物品
--物品使用
function Cang_130_item_touch_6e()
	local count = 0
	if count == 0 then 
