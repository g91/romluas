function ic_ha_de_139_01()											---1王死王劇情
	...
	if time1==1 then
		lua_star_BossKilledCount(105460)							---死亡刪除後面王BUFF AND 死亡計數器(1王號碼)
		ic_del_buff_139()											---刪除下之王的防駭BUFF
	end
end
function ic_ha_de_139_02()											---2王死王劇情						
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	local Count = SetSearchAllNPC(RoomID)
	local godfu=0 
	local time1=0
	...
	if time1==1 then
		lua_star_BossKilledCount(105515)						 	---死亡刪除後面王BUFF  AND 死亡計數器(2王號碼)
		ic_del_buff_139()											---刪除下之王的防駭BUFF
	end
end
function ic_ha_de_139_03()											---3王死王劇情
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	local Count = SetSearchAllNPC(RoomID)
	local godfu=0 
	local time1=0
	...
	if time1==1 then
		lua_star_BossKilledCount(105267)							---死亡刪除後面王BUFF  AND 死亡計數器(3王號碼)	
		ic_del_buff_139()											---刪除下之王的防駭BUFF
	end
end
function ic_ha_de_139_04()											---4王死王劇情
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	local Count = SetSearchAllNPC(RoomID)
	local godfu=0 
	local time1=0
	...
	if time1==1 then
		lua_star_BossKilledCount(105382)							---死亡刪除後面王BUFF AND 死亡計數器(4王號碼)
		ic_del_buff_139()											---刪除下之王的防駭BUFF
	end
end
function ic_ha_de_139_05()											---5王死王劇情
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	local Count = SetSearchAllNPC(RoomID)
	local godfu=0 
	local time1=0
	...
	if time1==1 then
		lua_star_BossKilledCount(105374)							---死亡刪除後面王BUFF AND 死亡計數器(5王號碼)
		ic_del_buff_139()											---刪除下之王的防駭BUFF
	end
end
function ic_del_buff_139() 											---刪除下個BOSS的防駭客機制的BUFF
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	local Count = SetSearchAllNPC(RoomID)
	...
	for  j=1 , boma do
		...
		if ReadRoleValue(NpcGID , EM_RoleValue_OrgID) == 105515			---2王(號碼)
			and ReadRoleValue( NpcGID , EM_RoleValue_IsDead ) == 0  then 
				if boss1xx == 1 then 									
					CancelBuff(NpcGID , 508071 )						---刪除防駭BUFF
				end
		end
		if ReadRoleValue(NpcGID , EM_RoleValue_OrgID) == 105267			---3王(號碼)
			and ReadRoleValue( NpcGID , EM_RoleValue_IsDead ) == 0 then 
				if boss1xx == 1 and boss2xx == 1 then 					
					CancelBuff(NpcGID , 508071 )						---刪除防駭BUFF
				end
		end
		if ReadRoleValue(NpcGID , EM_RoleValue_OrgID) == 105382			---4王(號碼)
			and ReadRoleValue( NpcGID , EM_RoleValue_IsDead ) == 0 then 
				if boss1xx == 1 and boss2xx == 1 and boss3xx == 1 then 	
					CancelBuff(NpcGID , 508071 )						---刪除防駭BUFF
				end
		end	
		if ReadRoleValue(NpcGID , EM_RoleValue_OrgID) == 105374			---5王(號碼)
			and ReadRoleValue( NpcGID , EM_RoleValue_IsDead ) == 0 then 
...
function ic_nohi_20_139_02()											---2王放防駭客機制的BUFF--放在BOSS的物件產生劇情
	sleep(10)
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )   	
	local Count = SetSearchAllNPC(RoomID)
	for i=1 , Count do
...
function ic_nohi_20_139_03()											---3王放防駭客機制的BUFF--放在BOSS的物件產生劇情
	sleep(10)
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )   	
	local Count = SetSearchAllNPC(RoomID)
	for i=1 , Count do
...
function ic_nohi_20_139_04()											---4王放防駭客機制的BUFF--放在BOSS的物件產生劇情
	sleep(10)
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )   	
	local Count = SetSearchAllNPC(RoomID)
	for i=1 , Count do
...
function ic_nohi_20_139_05()											---5王放防駭客機制的BUFF--放在BOSS的物件產生劇情
	sleep(10)
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )   	
	local Count = SetSearchAllNPC(RoomID)
	for i=1 , Count do
