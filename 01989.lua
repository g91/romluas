--門口：/GM ? zone 132 0 -571 343 1709
--控制器：/GM ? zone 132 0 -181.8 414.4 2060.7
...
function lua_sasa_104338_Counter_easy()			--模式計時器
	...
	while 1 do
		...
		if ( FlowerTarget ~= 0 ) and ( CheckID( FlowerBall ) == true) then
			...
			Boss = CreateObjByFlag( 104174 , 780702 , 1 , 1 )				----在旗標生物件
			AddToPartition( Boss , Room )										----把物件加到某層
			sleep(20)									-----為了不讓透明球在還沒放出特效法術前就被刪掉了
			DelObj(FlowerBall)
			BeginPlot( Boss , "lua_sasa_104174_Boss" , 0 )						----叫物件執行"內容",始動秒數
			Setplot( Boss , "dead" , "lua_sasa_104174_Bossdead" , 30)
				...
				if BossTarget ~= 0 or AnotherTarget ~= 0 then											-----王還有沒有其他可以打的目標
					...
					if Mode == 1 then												----假設模式=1
						...
						if counter1 >= ModeTime1 then								----計時的變數大於設定的秒數
							...
							AddBuff( Boss, Mode2Buff, 14 , -1 )						-----上Boss盾
							--sleep(50)	
					...
					elseif Mode == 2 then											----破盾回第一階段
						if CheckBuff( Boss , Mode2Buff ) == false then
							...
							if ReadRoleValue(Boss , EM_RoleValue_SpellMagicID ) == 495986 then		------Boss還在施放花苞守護
								SecondBoss = star_CreateObj_NoAdd( Boss , 104174 , 0 , 0 , 0 , Room , 0) 
								...
								DelObj(Boss)																									
								BeginPlot( SecondBoss , "lua_sasa_104174_Boss" , 0 )
								Setplot( SecondBoss , "dead" , "lua_sasa_104174_Bossdead" , 30)
					...
					if EnterFight == 1 then
						...
						if BossIsDead ~= 1 then											---Boss沒死才重生
							...
							sleep(50)
							Boss = CreateObjByFlag( 104174 , 780702 , 1 , 1 )
							AddToPartition( Boss , Room )
							BeginPlot( Boss , "lua_sasa_104174_Boss" , 0 )
							Setplot( Boss , "dead" , "lua_sasa_104174_Bossdead" , 30)
							WriteRoleValue( Boss , EM_RoleValue_register9 , OwnerID() )
							WriteRoleValue( Boss , EM_RoleValue_register8 , Exit )
...
function lua_sasa_104174_Skill_1()		-----Boss技能施放
	local Skill = { 495982 , 495983 }		-----潰傷 	--地刺藤襲
	local SkillLv = { 0 , 0 }
	--local SkillString = { "[]" , "[]" }
	local Luck = Rand(100)													----取亂數放技能
	local PlayerInHate = star_HateListSort(OwnerID())						----取780702恨表內的玩家
...
function lua_sasa_104174_Boss()			------Boss本體
	while 1 do
				...
				if mode == 1 then
					...
					if counter1 >= 15 then
						for i = 1 , 2 do
							...
							local mobtemp = CreateObjByFlag( 104339 , 780702 , flagNum , 1 )		--草
							...
							Setplot( mobtemp , "dead" , "lua_sasa_104339_dead" , 30)								
						end	
						counter1 = 0
					...
					if EnterMode2 == 0 then	
						...
						if table.getn(Player) >= 2 then							------大於2人就rand 2個
							for k=1 , 2 do						
								...
								SysCastSpellLv(OwnerID(), Player[lucky] , skill4 , 0 )
								Setplot( Player[lucky] , "dead" , "lua_sasa_104174_Playerdead" , 30)		--死亡劇情開跑
								sleep(1)
							...
							for m=1 , table.getn(Player) do
								...
								SysCastSpellLv(OwnerID(), Player[m] , skill4 , 0 )
								Setplot( Player[m] , "dead" , "lua_sasa_104174_Playerdead" , 30)		--死亡劇情開跑
								sleep(1)
							end
						end	
						EnterMode2 = 1
...
function lua_sasa_104174_Playerdead()
	...
	local MessageString = "[SC_104143_04][$SETVAR1="..PlayerName.."]" 		----在string中可以秀玩家的名字
...
function lua_sasa_104174_Bossdead()				-----Boss死亡
