...
-----戰鬥系統beginstart
---------------------------------------------------------------------------------------------------觸碰系統開場你們給我乖乖的去碰牠吧
function Lua_bk_r05_touchcount_102452()----------------------------------------------------------------------------------------------------  BOSS 1   ==   人馬無雙
	...
	SetPlot( to , "touch" , "Lua_bk_r05_touch_113417" , 50 )----寫開關劇情
...
function Lua_bk_r05_touch_113417()----------拉霸!
	local RoomID = ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)
	local to = TargetID()
	...
	---sleep(10)
	DelObj(count)
	local start = CreateObjByFlag( 102562,780427,1,1)-----------------開場控制者
		...
		BeginPlot( start,"Lua_bk_r05_start_102562",0 )
end
function Lua_bk_r05_start_102562()--------------------------判斷程式
	local RoomID = ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)
	local start = OwnerID()  ------開始
	local door=ReadRoleValue(start,EM_RoleValue_Register3)-----------------讀取門id
	local m1count = CreateObjByFlag( 102443,780427,1,1)---建立第一階段控制器
	...
	BeginPlot( m1count,"Lua_bk_r05_mode1_time_102443",0 )---把mode1  控制器產生出來	
	while 1 do 
	sleep(10)
...
function Lua_bk_r05_mode1_time_102443()    -------人馬時間控制器 
	local RoomID = ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)	
	...
	local Z0=0
	local Dir0=0
	local boss2 = CreateObjByFlag(102445,780427,2,1)----------------------------------------------開場囉建立馬
	...
	local boss3 = CreateObjByFlag(102444,780427,2,1)----------------------------------------------開場囉建立人
...
---	AttachObj( boss3 , boss2 ,0 , "p_link" , "p_link" )
	...
	BeginPlot(boss2 , "Lua_bk_r05_boss2_102445",0 )--------------------------------------執行第一階段馬
	SetPlot(boss2, "dead" ,"Lua_bk_r05_horse_dead_102445",0)------------------------死亡劇情
	BeginPlot(boss3 , "Lua_bk_r05_boss3_1_102444",0 )--------------------------------------執行第一階段人
	sleep(20)
	while 1 do 
	sleep(10)
		...
		if beginstart1==1 then --------------------------------------------------------------------------------------------當他進入戰鬥的時候
			...
			if time1==1 then --------------------合體放在旗標3
				...
				DelObj(boss3)
				boss3 = LuaFunc_CreateObjByObj ( 102444 , boss2 )----------------------------------------------開場囉建立人
				WriteRoleValue( boss3, EM_RoleValue_PID,m1count)-----------在人之中紀錄控制器的GID
				WriteRoleValue( boss2, EM_RoleValue_Register1,boss3)-------在馬身上寫上人的資料
				WriteRoleValue( boss3, EM_RoleValue_Register1,boss2)-------在馬身上寫上人的資料
				BeginPlot(boss3 , "Lua_bk_r05_boss3_1_102444",0 )--------------------------------------執行第一階段人
		...
		---		CastSpellLV( boss2 , boss3 , 493910, 0 )
			...
			if time1==31 then   --------------------------------------分開吧寶貝   分開放在4.5
				...
				boss3 = CreateObjByFlag(102444,780427,5,1)
			---	boss3 = LuaFunc_CreateObjByObj ( 102444 , boss3 )----------------------------------------------開場囉建立人
				WriteRoleValue( boss3, EM_RoleValue_PID,m1count)-----------在人之中紀錄控制器的GID
				WriteRoleValue( boss2, EM_RoleValue_Register1,boss3)-------在馬身上寫上人的資料
				WriteRoleValue( boss3, EM_RoleValue_Register1,boss2)-------在馬身上寫上人的資料
				BeginPlot(boss3 , "Lua_bk_r05_boss3_2_102444",0 )--------------------------------------執行第一階段人
...
---------------------------------------------------------------------------------------------------光暗天使控制器---------------------------------------------------------------------------------------------------
function Lua_bk_r05_mode2_time_102561()    -------光暗天使控制器
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )	
	...
	local alltime=0
		  Hide(boss3)	
	local fire0 =CreateObjByFlag(102555,780428,1,1)
		  ...
		  BeginPlot(fire0 ,"Lua_bk_r05_fire_check_102555",0 )
	--local boss4 = CreateObjByFlag(102447,780427,2,1)------開場先把光天使加入
	local boss4 = LuaFunc_CreateObjByObj ( 102447 , boss3 )
		  DelObj(boss3)
		...
		sleep(20)
	      BeginPlot(boss4 ,"Lua_bk_r05_boss4_102447",0 )--------------------------------------執行光天使劇情
	local boss5 = CreateObjByFlag(102446,780427,3,1)------開場先把暗天使加入
		...
		----	be=1----    戰鬥宣示
			...
			if alltime <=360 then 
				...
				black = ReadRoleValue(m2count ,EM_RoleValue_Register9)
					if black==0 then ---------剛開場光天使出來囉----
						...
						if time2 ==3 then 	
							...
							BeginPlot(fire0 , "Lua_bk_fire_rand_102555",0 )
						end
						if time2==21 then
					...
					if black==1 then 
						---boss5 = CreateObjByFlag(102446,780427,3,1)
						local AttackTarget = ReadRoleValue( boss4 , EM_RoleValue_AttackTargetID )
						boss5 = LuaFunc_CreateObjByObj(102446,boss4)-------出現在BOOS4所在的位置
						...
						SetPlot( boss5 ,"dead", "Lua_bk_r05_door_102563" ,50)
						BeginPlot(boss5 , "Lua_bk_r05_boss5_102446",50 )
						beginstart4=1
						beginstart3=0
				...
				if beginstart4==1 then ---------------------------------------------------黑天使出現時間!!!!
					...
					local AttackTarget = ReadRoleValue( boss4 , EM_RoleValue_AttackTargetID )
					boss4 = LuaFunc_CreateObjByObj(102447,boss5)
					SetAttack( boss4 , AttackTarget)
					--boss4 = CreateObjByFlag(102447,780427,2,1)----- 再次建立光天使
					WriteRoleValue( boss4, EM_RoleValue_PID,m2count)-----------在光天使之中紀錄控制器的GID
					DelObj(boss5)
					sleep(20)
					--AddtoPartition(boss4 , RoomID )
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
function Lua_bk_fire_rand_102555()		-------4個塔去做位置rand     ----轉吧轉吧七彩霓虹燈---隨機的去 rand光塔放了四種光塔劇情
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )	
	local fire0 = OwnerID()
	...
	for i=1 , 4 ,1 do
		...
		if fire==1 then 
			...
			SetPlot( fire1 , "touch" , "Lua_bk_r_range1_102461" , 40 )----寫入光塔1劇情
		...
		elseif fire==2 then
			...
			SetPlot( fire2 , "touch" , "Lua_bk_r_range2_102462" , 40 )----寫入光塔2劇情
		...
		elseif fire==3 then
			...
			SetPlot( fire3 , "touch" , "Lua_bk_r_range3_102463" , 40 )----寫入光塔3劇情				
		...
		elseif fire==4 then
			...
			SetPlot( fire4 , "touch" , "Lua_bk_r_range4_102464" , 40 )----寫入光塔4劇情
...
function Lua_bk_r_range1_102461()--------------------光塔一
...
function Lua_bk_r_range2_102462()--------------------光塔二	
	local RoomID = ReadRoleValue( TargetID() , EM_RoleValue_RoomID )					
	local fire0 = ReadRoleValue(TargetID(),EM_RoleValue_PID)
	SetPlot( TargetID()  ,"touch", "" ,0 )
...
function Lua_bk_r_range3_102463()--------------------光塔三
	local RoomID = ReadRoleValue( TargetID() , EM_RoleValue_RoomID )	
	local fire0 = ReadRoleValue(TargetID(),EM_RoleValue_PID)
	SetPlot( TargetID()  ,"touch", "" ,0 )
...
function Lua_bk_r_range4_102464()--------------------光塔四
	local RoomID = ReadRoleValue( TargetID() , EM_RoleValue_RoomID )	
	local fire0 = ReadRoleValue(TargetID(),EM_RoleValue_PID)
	SetPlot( TargetID()  ,"touch", "" ,0 )
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------第一階段馬階段---------------------------------------------------------------------------------------------------
function Lua_bk_r05_boss2_102445()--------------第一階段馬型狀態
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	local boss2 = OwnerID()
	...
	local mode1=504271
	local mode2=504272
	local skill={493907,493908,493902}-----1.踐踏 2.頭撞 3.全體擊倒
	local SkillLv = {0 , 0 , 1} 
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------		
function Lua_bk_r05_boss3_1_102444()
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	local boss3= OwnerID()
	local time1=0
	local time2=0
	...
	local mode1=504271
	local mode2=504272
	local skill={493901,493906,493905,493909}------1.直線型傷害  2. 閃電  目標一個 3.扇形AE 4.馬兒招喚
	local SkillLv = {1 , 0 , 0 , 0 } 
	local skilltarget={}
	local skill1_Rand={}--------------第一階段招式
...
---------------------------------------------------------------------------------------------------第一階段人行階段---------------------------------------------------------------------------------------------------
function Lua_bk_r05_boss3_2_102444()--------------第一階段人行階段
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	local boss3= OwnerID()
	local time1=0
	local time2=0
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------第二階段人型狀態----光天使模式---------------------------------------------------------------------------------------------------
function Lua_bk_r05_boss4_102447()--------------第二階段人型狀態----光天使模式
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	local boss4 = OwnerID()---------我就是打不死的光天使!!!!!!!!!!!!!!!
	local TempPlayer = SetSearchAllPlayer(RoomID)
	local ThesePlayer = {}
	local skill={493901,493906,493905}------1.直線型傷害  2. 閃電  目標一個 3.扇形AE
	local SkillLv = {1 , 0 , 0 } 
	local time1=0
	local time2=0
	local time3=0
...
---------------------------------------------------------------------------------------------------第二階段人型狀態---暗天使---------------------------------------------------------------------------------------------------
function Lua_bk_r05_boss5_102446()--------------第二階段人型狀態----暗天使模式	
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	local boss5=OwnerID()
	local m2count=ReadRoleValue(boss5,EM_RoleValue_PID)--------讀取第二階段的控制器
	local TempPlayer = SetSearchAllPlayer(RoomID)
	local ThesePlayer = {}
	local skill={493901,493903,493905,493904}------1.直線型傷害(隨機) 2.車子穿刺  3.扇形AE  4.SP 技能
	local SkillLv = {1 , 0 , 0 , 1 }  ---技能等級
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------	
---------------------------------------------------------------------------------------------------馬死掉之後---------------------------------------------------------------------------------------------------
function Lua_bk_r05_horse_dead_102445()
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	...
	local m2count=CreateObjByFlag(102561,780427,3,1)
		...
		BeginPlot(m2count , "Lua_bk_r05_mode2_time_102561",0 )		
		WriteRoleValue( m2count, EM_RoleValue_PID,start)------- 
		WriteRoleValue( m2count, EM_RoleValue_Register3,boss3)---
		
end
function Lua_bk_r05_fire_check_102555()----------判斷身上是否得到狀態
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	local fire0 = OwnerID()
	local m2count = ReadRoleValue(fire0,EM_RoleValue_PID)
	local black = 0
...
function Lua_bk_r05_door_102563()-------------------------把刪除去殺掉
	local RoomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	local boss5 = OwnerID()
