function Lua_Az134_104227_Ctrl()
	local Ctrl=OwnerID()
	local RoomID=ReadRoleValue(Ctrl,EM_RoleValue_RoomID)
	local Boss=CreateObjByFlag(104227,780758,0,1)
	...
	while true do
		...
		if hatelistcount(boss)~=0 then
			if Combatbegin==0 then
				CombatBegin=1								
				BeginPlot(Boss,"Lua_Az134_104227",0)
				BeginPlot(Crc,"Lua_Az134_Buff",0)
		...
		elseif hatelistcount(boss)==0 then
			if Combatbegin==1 then
				if ReadRoleValue(boss,EM_RoleValue_IsDead)==0 then
					...
					sleep(50)
					Boss=CreateObjByFlag(104227,780758,0,1)
...
--			say(boss,"ppl="..table.getn(HateList))
		...
		elseif SkCount%7==0 then
			BeginPlot(boss,"Lua_Az134_SandTrap",0)
...
function Lua_Az134_104227_Dead()
