--陣型產生範例
function LuaYuyu_gobolin_matrix()
...
--建立一個 table，在想產生物件的位置用 LuaFunc_CreateNPCByMatrix(ObjID,Matrix,A,B ) 產生物件。 其中四個數值依序代表 1.要生成的物件ID 2.作為基準的陣列 3.列位置 4.行位置
	local ObjMatrix ={}
	ObjMatrix[1] = LuaFunc_CreateNPCByMatrix(100636,Matrix,5, 0 )
	ObjMatrix[2] = LuaFunc_CreateNPCByMatrix(100997,Matrix,3, -3 )
	ObjMatrix[3] = LuaFunc_CreateNPCByMatrix(100997,Matrix,3, 3 )
	ObjMatrix[4] = LuaFunc_CreateNPCByMatrix(100635,Matrix,2, 1 )
	ObjMatrix[5] = LuaFunc_CreateNPCByMatrix(100635,Matrix,2,-1 )
	ObjMatrix[6] = LuaFunc_CreateNPCByMatrix(100914,Matrix,4, 1 )
	ObjMatrix[7] = LuaFunc_CreateNPCByMatrix(100914,Matrix,4, -1 )
...
--要求重新執行初始劇情
	BeginPlot( OwnerID() , "LuaYuyu_gobolin_matrix_Reset" , 0 )
end

function LuaYuyu_gobolin_matrix_Reset()
	sleep( 10 )
	BeginPlot( OwnerID() , "LuaYuyu_gobolin_matrix" , 0 )
end