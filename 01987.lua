function ic_104184_01()   ------主控器
	...
	local Boss=CreateObjByFlag(104184,780699,1,1) 	-----BOSS創在  旗標1
	local door=CreateObjByFlag(104204,780699,7,1)	-----阻擋門(王死才會消失)
	...
	while true do
		...
		if hatelistcount(boss)~=0 then    		---讀仇恨人數
			...
			if open==0 then
				open=1										--開打了
			 door2=CreateObjByFlag(104207,780699,6,1)-------入口阻擋門
			...
			door1=CreateObjByFlag(104207,780699,5,1)-------王身後阻擋門
            ...
            local NPCXX = SearchRangeNPC ( OwnerID() , 300 )----自身範圍300  掃區域找NPC
				for i = 0 , table.getn(NPCXX)	do
					if NPCXX[i] ~= OwnerID() then
						if	ReadRoleValue(NPCXX[i],EM_RoleValue_OrgID) == 	104186	then  ---找固定ID NPC
							DelObj( NPCXX[i] )
						end
						if	ReadRoleValue(NPCXX[i],EM_RoleValue_OrgID) == 	104185	then  ---找固定ID NPC
							DelObj( NPCXX[i] )
						end
					end
				end		
				...
				DelObj(door2)										--刪除阻擋門
				sleep(50)                                           --5秒
				Boss=CreateObjByFlag(104184,780699,1,1)           --再創造1之				
	...
	while 1 do 
		...
		if HPPercent >=0.71 then  ------血量>=71% 用這技能
			...
			if time1>=7 then 
				...
				--say(OwnerID(),"1")
				CastSpellLV( OwnerID() , BossTarget , 495838, 400 )----單體技能
				elseif Luck == 2 then 
				while 	ReadRoleValue(OwnerID() , EM_RoleValue_SpellMagicID  ) ~= 0   do--------有沒有正在施放技能
					sleep(10)
				end
	    ...
	    if HPPercent <=0.70 then------血量低於70% 用這技能
			...
			if time2>=40 then------大招 40秒固定放---全體
				...
				CastSpellLV( OwnerID() , OwnerID() , 496145 , 0 )
				sleep(30)
				while 	ReadRoleValue(OwnerID() , EM_RoleValue_SpellMagicID  ) ~= 0   do--------有沒有正在施放技能
					sleep(2)
				end			
...
function ic_104184_05 () -----小怪AI-1 法師
	...
	while true do
		...
		sleep(10)
		CastSpellLV( OwnerID() , BossTarget , 496146, 1 )---禁言技能
		sleep(50)
...
function ic_104184_06 () -----小怪AI-2 戰士
	local boss=ReadRoleValue(OwnerID(),EM_RoleValue_PID)
	local AttackTarget = ReadRoleValue( boss , EM_RoleValue_AttackTargetID )   ------ 找尋BOSS的攻擊目標  
	local BossTarget=0
	...
	while true do
		BossTarget = ReadRoleValue( ownerid(), EM_RoleValue_AttackTargetID )   ------ 找尋BOSS的攻擊目標  
		sleep(10)
		CastSpellLV( OwnerID() , BossTarget , 496147, 1 )---破甲技能
		sleep(50)
		if hatelistcount(boss)==0 then   --檢查王有沒有目標
			DelObj(OwnerID())            --沒有把自己殺死    
		end	
