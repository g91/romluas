...
function cl_374ai_scriptEZ() ----控制器的SCRIPT
	...
	local carzy_time = 0
	local No_374 = CreateObjByFlag( 105864 , 780889 , 0 , 1 ) ------------誰是boss要在哪裡產生
	...
	BeginPlot( No_374,"cl_136_No_374fightEZ", 0 ) ---塞入BOSS的戰鬥給他
	addbuff ( No_374 , 509063 , 0 , -1 ) ----給BOSS一個假的表演BUFF(光罩)
	for i = 1 , 4 , 1 do  ----四個表演用研究員
		local research = CreateObjByFlag( 105865 , 780889 , i+4 , 1 ) ------------for迴圈產生四個研究員
		...
		BeginPlot( research,"cl_136_researchEZ", 0 ) -----表演固定動作用劇情
	...
	BeginPlot( CrystalAI,"cl_No_374_CrystalfightEZ", 0 )
	...
	while 1 do
		...
		elseif stage == 3 then
			...
			WriteRoleValue ( CrystalAI , EM_RoleValue_PID , AI )
			BeginPlot( CrystalAI,"cl_No_374_CrystalfightEZ", 0 )
			No_374 = CreateObjByFlag( 105864 , 780889 , 0 , 1 ) ------------誰是boss要在哪裡產生
			AddToPartition ( No_374 , RoomID ) ----物件加入場景,例行公事
			WriteRoleValue ( No_374 , EM_RoleValue_PID , AI )
			BeginPlot( No_374,"cl_136_No_374fightEZ", 0 )
			WriteRoleValue ( CrystalAI , EM_RoleValue_Register2 , No_374 )
			WriteRoleValue ( AI , EM_RoleValue_Register1 , 0 )
		end
	end
end
function cl_136_researchEZ()
	...
	SetPlot( research, "dead","cl_136_research_deadEZ", 0 ) ------PlotType 觸發類型 (分為"touch"觸碰、"range"範圍、"dead"死亡、"collision"碰撞四種)
...
function cl_136_research_deadEZ()
	local research = OwnerID()
	local RoomID = ReadRoleValue( research , EM_RoleValue_RoomID ) ----這是AI讀取到的地圖
	local AI = ReadRoleValue ( research , EM_RoleValue_PID )
	local No_374 = ReadRoleValue ( research , EM_RoleValue_Register1 )
...
function cl_136_No_374fightEZ() ---BOSS放招式的function
	...
	SetPlot( No_374, "dead","cl_No_374_deadEZ", 0 ) ------PlotType 觸發類型 (分為"touch"觸碰、"range"範圍、"dead"死亡、"collision"碰撞四種)
	while 1 do
		sleep ( 10 )
			...
			if CombatBegin == 1 then
				...
				if skill_7 >= 4 then
					...
					if ( rand_skill >= 1 and rand_skill <= 10) or( rand_skill >=21 and rand_skill <= 30 ) or
					   ...
					   CastSpelllv ( No_374 , AttackTarget , 497327 , 20 )----對當前目標釋放一個巨大傷害
					...
					elseif ( rand_skill >= 11 and rand_skill <= 20 ) or( rand_skill >= 31 and rand_skill <= 40 ) or
					   ...
					   CastSpelllv ( No_374 , No_374 , 497326 , 5 )----前方扇形
					   sleep ( 20 )
					end
					skill_7 = 0
				end
				if skill_13 >= 7 then
					...
					if ( rand_skill >= 1 and rand_skill <= 10) or( rand_skill >=21 and rand_skill <= 30 ) or
					   ( rand_skill >= 41 and rand_skill <= 50 ) or ( rand_skill >= 61 and rand_skill <= 70 ) or
					   ( rand_skill >= 81 and rand_skill <= 90 ) then      ------------>當隨機出來的值等於以上的亂數就施放第一招
					   CastSpelllv ( No_374 , No_374 , 497325 , 0 ) --冰晶吐息
					   sleep ( 20 )   --------->放玩法術後整個迴圈停兩秒
					elseif ( rand_skill >= 11 and rand_skill <= 20 ) or( rand_skill >= 31 and rand_skill <= 40 ) or
					   ( rand_skill >= 51 and rand_skill <= 60 ) or( rand_skill >= 70 and rand_skill <= 80 ) or
					   ( rand_skill >= 91 and rand_skill <= 100 ) then   ------------>當隨機出來的值等於以上的亂數就施放第二招
					   ...
					   CastSpelllv ( No_374 , fire_Table[fire_number] , 497328 , 0 )----噴火
					   sleep ( 20 )
					end
					sleep ( 20 )
					skill_13 = 0
				...
				if skill_23 >= 13 then  ---施放追到死光束
					if skill_stop == 0 then
						...
						BeginPlot( ball,"cl_run_ballEZ", 0 )
				...
				if skill >= 16 then
					if skill_stop == 0 then
						...
						sleep ( 50 )
						sysCastSpelllv ( ball , ball , 497262 , 7 ) --請AI2隊帽子裡面的所有玩家施放-80%傷害的AE
						PlayMotion ( No_374 , ruFUSION_ACTORSTATE_CRAFTING_END )
						sleep ( 10 )
						SetIdleMotion( No_374 ,  ruFUSION_ACTORSTATE_NORMAL)--------保持地下模式
						SetModeEx( No_374 , EM_SetModeType_Fight, true) ---可砍殺
...
function cl_No_374_CrystalfightEZ() ---水晶控制器
	...
	cl_136_No_374_105387EZ()  -----執行產生小水晶控制器
	while 1 do
		sleep ( 10 )
		...
		if Crystal == 1 then --當Crystal變成1的時候代表真水晶以經點擊.假水晶必須產生並且開始計時30秒
			...
			if fix_Crystal_open == 0 then
				for i = 1 , 4 , 1 do
					...
					BeginPlot( fix_Crystal,"cl_fix_CrystalAIEZ", 0 )
				end
				fix_Crystal_open = 1
			end
			if Crystal_time == 30 then
				cl_136_No_374_105387EZ()
...
function cl_136_No_374_105387EZ() ---boss本身產水晶用
	local CrystalAI = OwnerID()
	local RoomID = ReadRoleValue( CrystalAI , EM_RoleValue_RoomID ) ----這是AI讀取到的地圖
	local AI = ReadRoleValue ( CrystalAI , EM_RoleValue_PID )
	local No_374 = ReadRoleValue ( CrystalAI , EM_RoleValue_Register2 )
	...
	BeginPlot( CrystalAII,"cl_136_No_374_CrystalAIEZ", 0 )
end
function cl_136_No_374_CrystalAIEZ() ---這一段是產生水晶AI產水晶用
	local CrystalAI = OwnerID()
	local RoomID = ReadRoleValue( CrystalAI , EM_RoleValue_RoomID ) ----這是AI讀取到的地圖
	...
	for i = 1 , 4 , 1 do
		...
		BeginPlot( Crystal,"cl_Crystal_touchEZ", 0 )
...
function cl_Crystal_touchEZ()
	local Crystal = OwnerID()
	local AI = ReadRoleValue ( Crystal , EM_RoleValue_Register5 )
	addbuff ( Crystal , 509700 , 0 , -1 )
	SetPlot( Crystal, "touch","cl_Crystal_CastEZ", 50 ) ------PlotType 觸發類型 (分為"touch"觸碰、"range"範圍、"dead"死亡、"collision"碰撞四種)
end
function cl_Crystal_CastEZ()
	local play = OwnerID()
	local Crystal = TargetID()
	if CheckBuff( play , 509148 ) == false then
		if	BeginCastBarEvent( play ,Crystal , "[SC_NO374_07]" , 20 , ruFUSION_ACTORSTATE_CRAFTING_BEGIN , ruFUSION_ACTORSTATE_CRAFTING_END,0 , "cl_Crystal_touch_OKEZ") ~=1	then
			ScriptMessage( play, play, 1,"[SAY_NPC_BUSY]", 0 );		--目標忙碌中
		end
	end
end
function cl_Crystal_touch_OKEZ(Executer,Result) ---檢查目標有沒有木棒~有的畫給予火把
	local Crystal = TargetID()
	local number = ReadRoleValue(Crystal , EM_RoleValue_Register1 )
	local function_table = {"cl_Crystal_redEZ" , "cl_Crystal_greenEZ" , "cl_Crystal_buleEZ" , "cl_Crystal_whiterEZ"}
...
function cl_No_374_deadEZ() ---BOSS死亡劇情
...
function cl_run_ballEZ()
	...
	while 1 do
		sleep ( 10 )
		sysCastSpelllv ( OwnerID() , OwnerID() , 497267 , 0 ) --請AI2隊帽子裡面的所有玩家施放-80%傷害的AE
