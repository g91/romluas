...
--  [Count][-Count/2... -2 , -1 , 0 , 1 , 2 ... Count/2]
	...
	local count = 8 --陣列的大小，輸入3 會是 3*3 的陣列；不過因為行數必須為奇數，所以輸入 4 會是 5*4 的陣列。
	local dis = 15 --陣列的間距
...
--建立一個 table，在想產生物件的位置用 LuaFunc_CreateNPCByMatrix(ObjID,Matrix,A,B ) 產生物件。 其中四個數值依序代表 1.要生成的物件ID 2.作為基準的陣列 3.列位置 4.行位置
	local ObjMatrix ={}
	ObjMatrix[1] = LuaFunc_CreateNPCByMatrix(100913,Matrix,2,-1 )
	ObjMatrix[2] = LuaFunc_CreateNPCByMatrix(100913,Matrix,2, 1 )
	ObjMatrix[3] = LuaFunc_CreateNPCByMatrix(100636,Matrix,3, 0 )
	ObjMatrix[4] = LuaFunc_CreateNPCByMatrix(100914,Matrix,4, 0 )
	ObjMatrix[5] = LuaFunc_CreateNPCByMatrix(100914,Matrix,4, 2 )
	ObjMatrix[6] = LuaFunc_CreateNPCByMatrix(100914,Matrix,4,-2 )
...
--要求重新執行初始劇情
	BeginPlot( OwnerID() , "LuaYuyu_big_matrix_Reset" , 0 )
end

function LuaYuyu_big_matrix_Reset()
	sleep( 10 )
	BeginPlot( OwnerID() , "LuaYuyu_big_matrix" , 0 )
end
