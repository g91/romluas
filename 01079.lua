...
function LuaYU_hello2009_2_00() -- 動作測試

	local RoomID = ReadRoleValue( OwnerID() ,EM_RoleValue_RoomID) 
	local BigPumpkin = CreateObjByFlag( 102644, 780327 , 0 , 1 );--旋轉大南瓜
...
--	AddBuff( BigPumpkin ,502707 ,1 ,-1 ); --非戰鬥中不回血的buff (ownerID,buff,LV,Time)
	...
	local fire = CreateObjByFlag( 102768, 780327 , 0 , 1 );--中間那團火
	...
	SetPlot( BigPumpkin , "dead","LuaYU_hello2009_2_Kingdead_00",0 )--旋轉大南瓜爆了的話
     
                SetPlot( fire,"range", "LuaYU_hello2009_onfire_01" , 30 )   -- 2011.11.01 修改此段function錯誤問題
...
--Say(fire,"fire")
		...
		sleep(65)
				...
				for q = 1 ,8 do  --產生南瓜
					...
					Pumpkin[q] = CreateObjByFlag( 113202, 780327 , q , 1 );
			...
			--以下隨機判斷產生南瓜位置
				...
				for i=1, 2 do
					...
					BeginPlot(  Y[i]  , "LuaYU_hello2009_2_motion_01" , 0 )  
				...
				for i=3, 4 do
					...
					BeginPlot(  Y[i]  , "LuaYU_hello2009_2_motion_02" , 0 )  
				...
				for i=5, 6 do
					...
					BeginPlot(  Y[i]  , "LuaYU_hello2009_2_motion_03" , 0 )  
				...
				for i=7, 8 do
					...
					BeginPlot(  Y[i]  , "LuaYU_hello2009_2_motion_04" , 0 )  
...
function LuaYU_hello2009_2_01() -- 生南瓜寶寶
--OwnerID--玩家

	if CheckBuff( OwnerID() , 504979) == true  then

		SetPlot( TargetID() ,"touch",  "" , 0 )  
		BeginPlot(  TargetID() , "LuaYU_hello2009_2_01_2" , 0 )  
...
function LuaYU_hello2009_2_01_2() -- 生南瓜寶寶
	...
	BeginPlot(  Monster , "LuaYU_hello2009_2_01_3" , 0 )  

--	sleep(20)
	Delobj(OwnerID())
end

function LuaYU_hello2009_2_01_3() -- 南瓜寶寶狂奔
	...
	sleep(30)
	FaceFlag ( OwnerID()  , 780327 , 0 )
	LuaFunc_MoveToFlag( OwnerID() , 780327 , 0 ,0 )
	...
	BeginPlot( OwnerID()  , "LuaYU_hello2009_2_thegun_01" , 0 )   
...
function LuaYU_hello2009_2_02() -- 生怪的南瓜
--Say( OwnerID(), "O= " )玩家
--Say( TargetID(), "T= " )南瓜

	if CheckBuff( OwnerID() , 504979) == true  then
		SetPlot( TargetID() ,"touch",  "" , 0 )  
		BeginPlot(  TargetID() , "LuaYU_hello2009_2_02_2" , 0 )  
		WriteRoleValue( TargetID()  , EM_RoleValue_Register+1 ,  555  ) --阻止多開
...
function LuaYU_hello2009_2_02_2() -- 
...
--	SetModeEx( Monster  , EM_SetModeType_Mark, false )--不標記.
	...
	WriteRoleValue( Monster  , EM_RoleValue_PID ,  TargetID()  ) 
	WriteRoleValue( Monster  , EM_RoleValue_Register ,  BigPumpkin  ) 
	BeginPlot(  Monster , "LuaYU_hello2009_2_02_4" , 0 )  
	SetAttack( Monster, TargetID())
--	sleep(5)
	Delobj(OwnerID())
end

function LuaYU_hello2009_2_02_3() -- 生邪惡南瓜怪
...
function LuaYU_hello2009_2_02_4() -- 邪惡南瓜怪AI
...
function LuaYU_hello2009_2_motion_01() -- 南瓜碎動
	while true do
		...
		if PumpkinNow == 555 then
			...
			SetPlot( OwnerID() ,"touch",  "LuaYU_hello2009_2_02" , 10 ) 
			...
			if PumpkinNow == 555 then
				...
				SetPlot( OwnerID() ,"touch",  "LuaYU_hello2009_2_01" , 10 )  -- 生南瓜
...
function LuaYU_hello2009_2_motion_02() -- 南瓜碎動
	while true do
		local PumpkinNow = ReadRoleValue( OwnerID()  ,EM_RoleValue_Register+1 ) --判斷可點的詭異南瓜目前的狀況  555代表有人點了
...
function LuaYU_hello2009_2_motion_03() -- 南瓜碎動
	while true do
		local PumpkinNow = ReadRoleValue( OwnerID()  ,EM_RoleValue_Register+1 ) --判斷可點的詭異南瓜目前的狀況  555代表有人點了
...
function LuaYU_hello2009_2_motion_04() -- 南瓜碎動
	while true do
		local PumpkinNow = ReadRoleValue( OwnerID()  ,EM_RoleValue_Register+1 ) --判斷可點的詭異南瓜目前的狀況  555代表有人點了
		local RAND_A = rand(8)
		local RAND_B = rand(1)
...
function LuaYU_hello2009_2_thegun_01() --南瓜籽
...
--	local MonsterGroup = Lua_Davis_BornByMultilateral( MonsterID , X , dis )
	...
	for i=1 , table.getn(MonsterGroup) do
			SetPlot( MonsterGroup[i] ,"touch",  "LuaYU_hello2009_2_thegun_02" , 10 ) 
			BeginPlot( MonsterGroup[i]   , "LuaYU_hello2009_2_thegun_03" , 0 )  
	end
...
--	local MonsterGroup = Lua_Davis_BornByMultilateral( MonsterID , X , dis )
	...
	for i=1 , table.getn(MonsterGroup2) do
			SetPlot( MonsterGroup2[i] ,"touch",  "LuaYU_hello2009_2_thegun_02" , 10 ) 
			BeginPlot( MonsterGroup2[i]   , "LuaYU_hello2009_2_thegun_03" , 0 )  
	end
...
function LuaYU_hello2009_2_thegun_02() --撿到的南瓜籽BUFF
...
function LuaYU_hello2009_2_thegun_03() --一段時間南瓜籽消失
	sleep(300)
...
function LuaYU_hello2009_2_Kingdead_00() -- 大南瓜爆了

	local fire = ReadRoleValue( OwnerID()  ,EM_RoleValue_Register+8) --那團火

	BeginPlot( fire   , "LuaYU_hello2009_2_Kingdead_01" , 0 )  

	return false
end

function LuaYU_hello2009_2_Kingdead_01() -- 大南瓜爆了
	...
	BeginPlot( ball   , "LuaYU_hello2009_2_Kingdead_02" , 0 )  

end

function LuaYU_hello2009_2_Kingdead_02() 
