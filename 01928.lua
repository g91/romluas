function ic_104108_01()   ------主控器
	...
	local Boss=CreateObjByFlag(104108,780699,1,1) 	-----BOSS創在  旗標1
	local door=CreateObjByFlag(104199,780699,7,1)	-----阻擋門(王死才會消失)
	...
	while true do
		...
		if hatelistcount(boss)~=0 then    		---讀仇恨人數
			...
			if open==0 then
				open=1										--開打了
			 door2=CreateObjByFlag(104198,780699,6,1)-------入口阻擋門
			...
			door1=CreateObjByFlag(104198,780699,5,1)-------王身後阻擋門
            ...
            local NPCXX = SearchRangeNPC ( OwnerID() , 300 )----自身範圍300  掃區域找NPC
				for i = 0 , table.getn(NPCXX)	do
					if NPCXX[i] ~= OwnerID() then
						...
						if	ReadRoleValue(NPCXX[i],EM_RoleValue_OrgID) == 	104145	then  ---找固定ID NPC
							DelObj( NPCXX[i] )
						end
					end
				end		
				...
				DelObj(door2)										--刪除阻擋門
				sleep(50)                                           --5秒
				Boss=CreateObjByFlag(104108,780699,1,1)           --再創造1之				
	...
	while 1 do 
		...
		if HPPercent >=0.71 then  ------血量>=71% 用這技能
			...
			if time1>=7 then 
				...
				--say(OwnerID(),"2")
				CastSpellLV( OwnerID() , OwnerID() , 495923, 5 )----自身範圍
				elseif Luck == 3 then 
				while 	ReadRoleValue(OwnerID() , EM_RoleValue_SpellMagicID  ) ~= 0   do--------有沒有正在施放技能
					sleep(10)
				end			
...
function ic_104108_05 () -----小怪AI-1 法師
...
function ic_104108_06 () -----小怪AI-2 戰士
	local boss=ReadRoleValue(OwnerID(),EM_RoleValue_PID)
	local AttackTarget = ReadRoleValue( boss , EM_RoleValue_AttackTargetID )   ------ 找尋BOSS的攻擊目標  
	local BossTarget=0
...
function ic_104108_04()----檢查用
	local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)
	local Search = SearchRangeNPC ( OwnerID() , 300 )----自身範圍300  掃區域找NPC
	---say(OwnerID(),"88888")
	for i = 0 , table.getn(Search)	do
		if Search[i] ~= OwnerID() then
			if	ReadRoleValue(Search[i],EM_RoleValue_OrgID) == 	104108	then  ---找固定ID NPC -12人王
				----say(OwnerID(),"1111111111")
				break
			elseif	ReadRoleValue(Search[i],EM_RoleValue_OrgID) ==	103945	then---神裝正常6人王
				break
			elseif	ReadRoleValue(Search[i],EM_RoleValue_OrgID) ==	104461	then---正常6人王
				break	
			elseif	ReadRoleValue(Search[i],EM_RoleValue_OrgID) ==	104184	then---簡單6人王
				break	
			end
			
			if i == table.getn(Search) and ReadRoleValue(Search[i],EM_RoleValue_OrgID) ~=	104108 then--12人王
				----say(OwnerID(),"77777")
				ScriptMessage(OwnerID(),OwnerID(),1,"[SC_104108_05]",2)---此區無法使用
			elseif	i == table.getn(Search) and ReadRoleValue(Search[i],EM_RoleValue_OrgID) ~=	103945 	then--神裝正常6人王
				ScriptMessage(OwnerID(),OwnerID(),1,"[SC_104108_05]",2)---此區無法使用
			elseif	i == table.getn(Search) and ReadRoleValue(Search[i],EM_RoleValue_OrgID) ~=	104461 	then--正常6人王
				ScriptMessage(OwnerID(),OwnerID(),1,"[SC_104108_05]",2)---此區無法使用	
			elseif	i == table.getn(Search) and ReadRoleValue(Search[i],EM_RoleValue_OrgID) ~=	104184 	then--簡單6人王
				ScriptMessage(OwnerID(),OwnerID(),1,"[SC_104108_05]",2)---此區無法使用	
			end
...
function ic_104108_03 ()------使用物品
	local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)
	local Y = Lua_BuffPosSearch( OwnerID() , 507265 )	
	local Search = SearchRangeNPC ( OwnerID() , 300 )
	for i = 0 , table.getn(Search) do
		if Search[i] ~= OwnerID() then
			if	ReadRoleValue(Search[i],EM_RoleValue_OrgID) == 	104108	then---BOSS是否存在 -12人王
				AddBuff( OwnerID(),507265,0 ,-1 )---淡淡血腥味-隱藏
				AddBuff( OwnerID(),507266,0 ,-1 )----血腥味濃
				AddBuff( OwnerID(),507267,0 ,-1 )----減傷BUFF				
				BOSS=Search[i]
				break
			elseif	ReadRoleValue(Search[i],EM_RoleValue_OrgID) ==	103945	then---神裝正常6人王
				AddBuff( OwnerID(),507265,0 ,-1 )---淡淡血腥味-隱藏
		...
		Y = Lua_BuffPosSearch( OwnerID() , 507265 )
		sleep(10)
		X=BuffInfo( OwnerID(), Y , EM_BuffInfoType_Power )---讀取技能等級
		----say(OwnerID(),"X="..X)
		if x ==4 then
		WriteRoleValue(boss,EM_RoleValue_register3,1)---回傳1給BOSS
		elseif	x==9 then
		WriteRoleValue(boss,EM_RoleValue_register3,2)---回傳2給BOSS
		elseif	x==19 then
		WriteRoleValue(boss,EM_RoleValue_register3,3)---回傳3給BOSS
		end
 end
 function ic_104108_07()----刪除BUFF用
	local RoomID=ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)
	local Search = SearchRangePlayer ( OwnerID() , 400 )
	-----say(OwnerID(),"88888")
	for i = 0 , table.getn(Search)	do
		if Search[i] ~= OwnerID() then
		---say(OwnerID(),"88888")
			CancelBuff( Search[i], 507265 )----刪除BUFF用
			CancelBuff( Search[i], 507266 )
		end
	end
end
