function lua_star_BossAI_102558()
	while 1 do
		sleep(10)
		local Mode = 0
		local Skill = { 493932 }	
			...
			if  ( AttackTarget ~= 0 ) and ( CheckID( OwnerID() ) ) == true then				
				...
				elseif Mode == 2 then													--------處於模式2		
					...
					if BeginMode == 1 then											--------空戰遊戲開始
						...
						Satellite = star_102558_Satellite(OwnerID() , Room)					-------生出衛星小怪
						sleep(30)
						GroundBotTemp = star_102558_GroundWar(OwnerID() , Room)				-------生出地面小怪
...
function star_102558_Satellite(BossGID , Room)		-------產生空中小怪
	local Satellite = {}
	local SatelliteID = 102626 	-------衛星小怪ID
	...
	for i=1 , 8 do
		...
		CallPlot( Satellite[i] , "star_102558_SatelliteFire" , BossGID )
		Setplot( Satellite[i] , "dead" , "star_102558_SatelliteDead" , 30)
...
function star_102558_SatelliteDead()
...
function star_102558_GroundWar(BossGID , Room)			------產生地面小怪
	local GroundBot = {}
	local GroundBotID = 102627	-----地面小怪ID
	local LuckyBotID = 102628	-----會掉強力子彈小怪ID
	...
	for i=1 , 6 do
		...
		if (LuckyBall > 30 and LuckyBall < 61) 
			...
			--Setplot( GroundBot[i] , "dead" , "star_102558_GroundDead" , 30)		------死亡劇情暫時不使用	
		elseif LuckyBall >= 0 and LuckyBall <= 30 then	
			GroundBot[i] = CreateObjByFlag( GroundBotID , 780422 , i+3 , 1 )		
			AddToPartition( GroundBot[i] , Room )
			WriteRoleValue( GroundBot[i] , EM_RoleValue_register1 , BossGID )		-----Boss GID寫入小怪身上
			--Setplot( GroundBot[i] , "dead" , "star_102558_GroundDead" , 30)
...
function star_102558_GroundDead()					------地面小怪死亡劇情
	local BossGID = ReadRoleValue( OwnerID() , EM_RoleValue_register1 )
...
function star_102558_SatelliteFire(BossGID)			-------空中小怪招式施放
...
function star_102558_BossSkill()											------Boss施放招式
	local Skill = { 493933 , 493934 , 493947 }	
...
function star_102558_BossDead()												-------Boss死亡劇情
...
function lua_star_102558_Counter()	-----模式計時器
	...
	local Room = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID )
	local Boss = CreateObjByFlag( 102558 , 780422 , 1 , 1 )				-----Boss主體
	SetModeEx( Boss , EM_SetModeType_Move , false )	------不移動
	AddToPartition( Boss , Room )
	BeginPlot( Boss , "lua_star_BossAI_102558" , 0 )
	Setplot( Boss , "dead" , "star_102558_BossDead" , 30)
	...
	local PosZ = BossObj:Z()
	local UpperPart = CreateObj( 102569 , PosX , PosY , PosZ , 0 , 1 ) 		-----Boss支架
	...
	local Exit = CreateObjByFlag( 102636 , 780422 , 11 , 1 )	-----阻擋門(出口)
	...
	while 1 do
		...
		if  ( BossTarget ~= 0 ) and ( CheckID( Boss ) ) == true then
			if EnterFight == 0 then  
				Entrance = CreateObjByFlag( 102636 , 780422 , 12 , 1 )		-----阻擋門(入口)
			...
			if BerserkCounter >= 480 then
				if IsBerserk == 0 then
					AddBuff( Boss , 504269 , 0 , -1 ) 
					IsBerserk = 1
				end	
			end	
		else	
			...
			if EnterFight == 1 or BossEnterFight == 1 then
				...
				for i=1 , 10 do
					...
					if CheckID( Boss ) == false and BossDead ~= -1 then
						...
						if counter2 == 5 then
							Boss = CreateObjByFlag( 102558 , 780422 , 1 , 1 )				-----Boss主體
							SetModeEx( Boss , EM_SetModeType_Move , false )	------不移動
							AddToPartition( Boss , Room )
							BeginPlot( Boss , "lua_star_BossAI_102558" , 0 )
							Setplot( Boss , "dead" , "star_102558_BossDead" , 30)
...
----------------------------------------------------------------------------------------------------------------------------------------------------------------非王AI部份
function star_102558_SuperAmmo()		------強力彈藥劇情(法術集合檢查)
	local RobotBuff = 504343	--------變身Buff
	local AmmoBuff = 504371		--------強力彈藥
	if CheckBuff(TargetID() , RobotBuff) == true then
		AddBuff( TargetID() , AmmoBuff , 0 , 40)
		return true
	else
		ScriptMessage( OwnerID() , OwnerID() , 1 , "[SC_102558_1]" , 1 )		-----只能對處於『仿守護者型態』的人物使用
		return false
	end	
end

function star_102558_UseAmmo()		------強力彈藥劇情(基本法術檢查)
	local AmmoBuff = 504371		--------強力彈藥	
	CancelBuff( OwnerID() , AmmoBuff )	
	return true
end

function star_102558_RobotSkillCheck()
	if ReadRoleValue( TargetID() , EM_RoleValue_OrgID ) == 102626 			-----普通版天火小怪
	 or ReadRoleValue( TargetID() , EM_RoleValue_OrgID ) == 103208 then		-----簡單板天火小怪
		return true
	else
		ScriptMessage( OwnerID() , OwnerID() , 1 , "[SC_102558_2]" , 1 )		-----你只能對水藍天火使用此種能力
		return false
	end	
end
