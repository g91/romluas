function bk_127_103861()----------------冰火五重天控制器
	...
	local CombatBegin=0
	local door=0
	local afdoor = CreateObjByFlag(103863,780596,2,1)-------------------------------------------奇美拉後門
	...
	local boss = CreateObjByFlag(103861,780596,3,1)------------------------------創立王
	...
	BeginPlot( boss ,"bk_127_103861_fire",0)
	SetPlot( boss , "dead" , "bk_127_103861_dead" , 10 )
	...
	while 1 do 
		...
		if x==1 then --王正在戰鬥之中
			if CombatBegin==0 then 
				door = CreateObjByFlag(103863,780596,1,1)-------------------------------------------奇美拉前門
		...
		elseif x==2 then --玩家死亡王離開戰鬥
			sleep(50)
			boss = CreateObjByFlag(103861,780596,3,1)
			SetModeEx( boss , EM_SetModeType_Mark, true)
			SetModeEx( boss , EM_SetModeType_HideName, true)
			SetModeEx( boss , EM_SetModeType_ShowRoleHead, true)
			--SetModeEx( boss , EM_SetModeType_NotShowHPMP, true)
			...
			BeginPlot( boss ,"bk_127_103861_fire",0)---------------------------------王劇情模式
			SetPlot( boss , "dead" , "bk_127_103861_dead" , 10 )
			x=0
			sleep(10)
			DelObj(door)
...
function bk_127_103861_fire()--------------王本身ai 奇美拉-紅
	local RoomID = ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)-----------------
			...
			if mob==0  then
				...
				if mob1 >= 15 then
					...
					if kp~=0 then --------看打哪個倒楣鬼
						...
						local caball = Lua_DW_CreateObj("obj",103865,BossTarget,0)
						...
						CastSpellLv(caball,caball,495543,15)
			...
			if mob==1 then
				...
				if mob2>=15 then 
					...
					---	say(boss,"xxxxxxxxx")
						...
						SetModeEx( caball , EM_SetModeType_Searchenemy, false)  ---鎖敵
						AddToPartition(caball , RoomID )
						CastSpellLv(caball,caball,495544,15)
						WriteRoleValue(caball, EM_RoleValue_Livetime,20)	---------------存在時間livetime
						sleep(20)
					end
					mob=0
