function LuaDan_112597_NPC_Create()
            
        local Rnpc = LuaFunc_SearchNPCbyOrgID( OwnerID() , 112597, 100 , 1)       --拉比兔
	...
	If CheckID( Rnpc[0] ) == true then 
		...
		if secondR == 5 then
			...
			local Npc = CreateObjByFlag( 112597, 780348, 1 , 1 )
			AddToPartition( Npc , 0)  
			  ...
			  BeginPlot( Npc , "LuaDan_112597_NPC_Rabbit00" , 0 ) 
...
function LuaDan_112597_NPC_Create2()
...
---------------------------------------------------------------------------------------------------------------------------------
function LuaDan_112597_NPC_Rabbit() 

SetSpeakDetail(OwnerID(), "[SC_DAN_SR_111700_21]" )    --只有我最勤勞，不要看其他兩隻拉比兔，我肯定是最快到達目的地的！

end
---------------------------------------------------------------------------------------------------------------------------------

function LuaDan_112597_NPC_Rabbit00() 
--say(OwnerID(), "3")
--say(TargetID(), "4")
		SetPlot( OwnerID() , "touch" ,"LuaDan_112597_NPC_Rabbit01",50)  
		SetSpeakDetail(OwnerID(), "[SC_DAN_SR_111700_21]" ) 
...
---------------------------------------------------------------------------------------------------------------------------------
function LuaDan_112597_NPC_Rabbit01() 
...
--say(TargetID(), "6")
	local Key    = CountBodyItem( OwnerID() , 203477 )  
			...
			if  Key >= 1 then		--方便測試課使用
				...
				BeginPlot( TargetID(), "Lua_Dan_112597_Parade" , 0 ) 

			else
				ScriptMessage( TargetID(), OwnerID(), 1 , "[SC_DAN_SR_111700_21]",C_SYSTEM )  
				BeginPlot( TargetID(), "LuaDan_112597_NPC_Rabbit02" , 0 ) 		
	  		end

end
---------------------------------------------------------------------------------------------------------------------------------

function LuaDan_112597_NPC_Rabbit02() 
--say(OwnerID(), "7")
--say(TargetID(), "8")					
---
		while true do
...
--			Say( TargetID(),"6")
					...
					BeginPlot( OwnerID(), "Lua_Dan_112597_Parade" , 0 )  --活動於1100分開始
...
--			Say( TargetID(),"8")                 
					...
					BeginPlot( OwnerID() , "Lua_Dan_112597_Parade" , 0 )  
...
--			Say( TargetID(),"10")                         
					...
					BeginPlot( OwnerID() , "Lua_Dan_112597_Parade" , 0 ) --用LubbyRabbit開始劇情 
			else 
...
---------------------------------------------------------------------------------------------------------------------------------
function Lua_Dan_112597_Parade() --路線一
	...
	local LubbyRabbit = OwnerID()    
		...
		SetFightMode( LubbyRabbit , 0, 1, 0, 0 )               --索敵、移動、逃跑、被攻擊 1 true.
		    	 ...
		    	 Say( LubbyRabbit , "[SC_DAN_SR_111700_2]")  				 --由NPC發送訊息
       			 SetPlot( LubbyRabbit , "touch" ,"Lua_Dan_112597_Touch", 50)                --點選拉比兔，拉比兔即開始遊行
...
--		Say( TargetID(),"2" )
		...
		LuaFunc_MoveToFlag( LubbyRabbit, 780294, 8, 0)        			--移動到指定的棋子 
		...
		sleep(CT)
			...
			Say( LubbyRabbit , "[SC_DAN_SR_111700_3]" )  
                	 ...
                	 BeginPlot (LubbyRabbit, "Lua_Dan_112597_createholes", 50)       --呼叫產生洞的Script
------			
		sleep(ST)
		...
		WriteRoleValue(LubbyRabbit ,EM_RoleValue_IsWalk , 0) 			--走吧！兔子。
               	LuaFunc_MoveToFlag( LubbyRabbit, 780294, 9, 0)        			--移動到下一支旗子位置
		...
		sleep(CT)
			...
			BeginPlot (LubbyRabbit, "Lua_Dan_112597_createholes1", 50) 	--呼叫產生洞的Script

		sleep(ST)
			BeginPlot (LubbyRabbit, "LuaDan_112597_NPC_Create2", 10)	--呼叫產生洞的Script
			ScriptMessage( LubbyRabbit , 0 , 1 , "[SC_DAN_SR_111700_4]" , C_SYSTEM ) --嘿！我沒有騙你吧，我們繼續走吧！
			Say( LubbyRabbit , "[SC_DAN_SR_111700_4]" )  
		...
		LuaFunc_MoveToFlag( LubbyRabbit, 780294, 10, 0)        			--移動到指定的棋子 54000替換長頸兔 ((三段路徑))
		...
		sleep(CT)
			...
			BeginPlot (LubbyRabbit, "Lua_Dan_112597_createholes12", 50)      --呼叫產生洞的Script
		...
		WriteRoleValue( LubbyRabbit, EM_RoleValue_IsWalk, 0) 			--跑吧！兔子。
		LuaFunc_MoveToFlag( LubbyRabbit, 780294, 11, 0)        			--移動到指定的棋子 54000替換長頸兔 ((三段路徑))
		...
		sleep(CT)
			...
			BeginPlot (LubbyRabbit, "Lua_Dan_112597_createholes13", 50)	--呼叫產生洞的Script
...
--結束Script		
		...
		if secondR ~= 5 then
			...
			BeginPlot (LubbyRabbit, "LuaDan_112597_NPC_Create", 10)	--呼叫產生兔子的Script	
			sleep(50)
			DelObj(LubbyRabbit)
		else
...
function Lua_Dan_112597_createholes() --780343
	...
	--Say( TargetID(),"2" ) --Hole
		...
		local Hole = {}
			...
			Hole[0] = CreateObjByFlag( 112459, 780343, 0 , 1 )    --洞1        
			AddToPartition( Hole[0] , 0)     

			Hole[1] = CreateObjByFlag( 112459, 780343, 1 , 1 )    --洞2       
			AddToPartition( Hole[1] , 0)     

			Hole[2] = CreateObjByFlag( 112459, 780343, 2 , 1 )    --洞3       
			AddToPartition( Hole[2] , 0)     

			Hole[3] = CreateObjByFlag( 112459, 780343, 3 , 1 )    --洞4        
			AddToPartition( Hole[3] , 0)     

			Hole[4] = CreateObjByFlag( 112459, 780343, 4 , 1 )    --洞5        
			...
			--SetPlot( Hole[0] ,"touch",  "Lua_Dan_112597_Hole" , 50 )
			BeginPlot (Hole[0] , "Lua_Dan_112597_Hole", 10)  
		

			--SetCursorType( Hole[1] , eCursor_Mine )
			--SetPlot( Hole[1] ,"touch",  "Lua_Dan_112597_Hole" , 50 )
			BeginPlot (Hole[1] , "Lua_Dan_112597_Hole1", 10)  
			

			--SetCursorType( Hole[2] , eCursor_Mine )
			--SetPlot( Hole[2] ,"touch",  "Lua_Dan_112597_Hole" , 50 )
			BeginPlot (Hole[2] , "Lua_Dan_112597_Hole2", 10)  
			

			--SetCursorType( Hole[3] , eCursor_Mine )
			--SetPlot( Hole[3] ,"touch",  "Lua_Dan_112597_Hole" , 50 )
			BeginPlot (Hole[3] , "Lua_Dan_112597_Hole3", 10) 
			

			--SetCursorType( Hole[4] , eCursor_Mine )
			--SetPlot( Hole[4] ,"touch",  "Lua_Dan_112597_Hole" , 50 )
			BeginPlot (Hole[4] , "Lua_Dan_112597_Hole4", 10) 
...
----------------------------------------------------------------------------------------------------------------
function Lua_Dan_112597_createholes1() --780344

			local Hole = {}

				
			Hole[0] = CreateObjByFlag( 112459, 780344, 0 , 1 )    --洞1        
			AddToPartition( Hole[0] , 0)     

			Hole[1] = CreateObjByFlag( 112459, 780344, 1 , 1 )    --洞2       
			AddToPartition( Hole[1] , 0)     

			Hole[2] = CreateObjByFlag( 112459, 780344, 2 , 1 )    --洞3       
			AddToPartition( Hole[2] , 0)     

			Hole[3] = CreateObjByFlag( 112459, 780344, 3 , 1 )    --洞4        
			AddToPartition( Hole[3] , 0)     

			Hole[4] = CreateObjByFlag( 112459, 780344, 4 , 1 )    --洞5        
			AddToPartition( Hole[4] , 0)     
...
function Lua_Dan_112597_createholes12() --780345

				local Hole = {}

				
			Hole[0] = CreateObjByFlag( 112459, 780345, 0 , 1 )    --洞1        
			AddToPartition( Hole[0] , 0)     

			Hole[1] = CreateObjByFlag( 112459, 780345, 1 , 1 )    --洞2       
			AddToPartition( Hole[1] , 0)     

			Hole[2] = CreateObjByFlag( 112459, 780345, 2 , 1 )    --洞3       
			AddToPartition( Hole[2] , 0)     

			Hole[3] = CreateObjByFlag( 112459, 780345, 3 , 1 )    --洞4        
			AddToPartition( Hole[3] , 0)     

			Hole[4] = CreateObjByFlag( 112459, 780345, 4 , 1 )    --洞5        
			AddToPartition( Hole[4] , 0)     
...
function Lua_Dan_112597_createholes13() --780346

		
				local Hole = {}

				
			Hole[0] = CreateObjByFlag( 112459, 780346, 0 , 1 )    --洞1        
			AddToPartition( Hole[0] , 0)     

			Hole[1] = CreateObjByFlag( 112459, 780346, 1 , 1 )    --洞2       
			AddToPartition( Hole[1] , 0)     

			Hole[2] = CreateObjByFlag( 112459, 780346, 2 , 1 )    --洞3       
			AddToPartition( Hole[2] , 0)     

			Hole[3] = CreateObjByFlag( 112459, 780346, 3 , 1 )    --洞4        
			AddToPartition( Hole[3] , 0)     

			Hole[4] = CreateObjByFlag( 112459, 780346, 4 , 1 )    --洞5        
			AddToPartition( Hole[4] , 0)     
	...
	local Random = Rand(100)
		     ...
		     local probability = {	20,		-- 資源
						...
						20 }       	-- 什麼都沒有 
                         ...
                         if Random > (100 - probability[1] )  then  --20% 機率　--神秘的結晶體用來打開得到資源--Lua_Dan_112597_Hole_Resource
  				
                   		SetCursorType( OwnerID() , eCursor_Mine )
				SetPlot( OwnerID() ,"touch",  "Lua_Dan_112597_Hole_Nothing" , 50 ) 
							
			 

                    	 elseif Random > (100 - probability[1] - probability[2])  then --40%活動藥水 --Lua_Dan_112597_Hole_Potion
				
				SetCursorType( OwnerID() , eCursor_Mine )
				SetPlot( OwnerID() ,"touch",  "Lua_Dan_112597_Hole_Bomb" , 50 ) 
			
						
                         

			 elseif Random > (100 - probability[1] - probability[2] - probability[3]) then --怪物 60% --Lua_Dan_112597_Hole_Monster
			 
				SetCursorType( OwnerID() , eCursor_Mine )
				SetPlot( OwnerID() ,"touch",  "Lua_Dan_112597_Hole_Resource" , 50 ) 
		
								
			 

			elseif Random > (100 - probability[1] - probability[2] - probability[3] - probability[4])  then --炸彈  80% --Lua_Dan_112597_Hole_Bomb
			
				SetCursorType( OwnerID() , eCursor_Mine )
				SetPlot( OwnerID() ,"touch",  "Lua_Dan_112597_Hole_Potion" , 50 ) 
                      
			else 

				SetCursorType( OwnerID() , eCursor_Mine )
				SetPlot( OwnerID() ,"touch",  "Lua_Dan_112597_Hole_Monster" , 50 ) --Lua_Dan_112597_Hole_Nothing
		     ...
		     local probability = {	20,		-- 資源
						...
						20 }       	-- 什麼都沒有 
                         ...
                         if Random > (100 - probability[1] )  then  --20% 機率　--神秘的結晶體用來打開得到資源--Lua_Dan_112597_Hole_Resource
                   		...
                   		SetCursorType( OwnerID() , eCursor_Mine )
				SetPlot( OwnerID() ,"touch",  "Lua_Dan_112597_Hole_Nothing" , 30 ) 
...
----------------------------------------------------------------------------------------------------------------
function Lua_Dan_112597_Hole_Bomb() --Target為炸彈
...
----------------------------------------------------------------------------------------------------------------
function Lua_Dan_112597_Hole_Monster() --Target為怪物
...
function Lua_Dan_112597_Hole_Resource() --Target為資源
...
function Lua_Dan_112597_Hole_Potion() --Target為藥水
...
function Lua_Dan_112597_Hole_Nothing() --Target為什麼都沒有
