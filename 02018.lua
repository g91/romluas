function bk_130_104097()---------------------------------------133RAID王
...
--	AddToPartition( afdoor , RoomID )
	local boss = CreateObjByFlag(104096,780687,2,1)------------------------------創立王
	...
	BeginPlot( boss ,"bk_130_104096",0)
	SetPlot( boss , "dead" , "bk_130_104096_dead" , 10 )
	...
	while 1 do 
		...
		elseif x==2 then --玩家死亡王離開戰鬥
			sleep(50)
			boss = CreateObjByFlag(104096,780687,2,1)
			AddToPartition( boss , RoomID )
			WriteRoleValue( boss, EM_RoleValue_PID,start)
			BeginPlot( boss ,"bk_130_104096",0)---------------------------------王劇情模式
			SetPlot( boss , "dead" , "bk_130_104096_dead" , 10 )
			x=0
			DelObj(door)
...
function bk_130_104096()--------------王本身ai
	...
	while 1 do 
		...
		if HateListCount( boss ) ~= 0 then	---- 確定他現在有打人
			...
			----	BeginPlot( boss ,"bk_hacker_17_130_06",0)--------------------------------防駭客機制
			...
			elseif mob== 1 then 
				...
				--			say(ownerid(),"xx"..kp)
								if CheckRelation( Boss, P150[kp] , EM_CheckRelationType_Attackable ) == true then 
									...
									BossTarget = P150[kp]	
									CastSpellLV( OwnerID() , BossTarget , 495865 , 15 )-------------光震彈
									sleep(20)
								end
							end
						end
