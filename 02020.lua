function Lua_Az130_103947_Ctrl()							--Raid帕依察控制器
	local Ctrl=OwnerID()
	local CombatBegin=0										--開戰旗標
	local RoomID=ReadRoleValue(Ctrl,EM_RoleValue_RoomID)
	local Boss=CreateObjByFlag(103947,780713,0,1)
	...
	SetPlot(Boss,"dead","Lua_Az130_103947_Dead",0)
	while true do
		sleep(10)
		...
		-----		BeginPlot( boss ,"bk_hacker_17_130_02",0)--------------------------------防駭客機制
				BeginPlot(Boss,"Lua_Az130_103947",0)
		...
		elseif hatelistcount(boss)==0 then
			if Combatbegin==1 then
				if ReadRoleValue(boss,EM_RoleValue_IsDead)==0 then
					...
					DelObj(door1)
					sleep(50)
					Boss=CreateObjByFlag(103947,780713,0,1)
					AddToPartition(Boss,RoomID)
					WriteRoleValue(Ctrl,EM_RoleValue_PID,Boss)
					WriteRoleValue(Boss,EM_RoleValue_PID,Ctrl)
					WriteRoleValue(Boss,EM_RoleValue_register5,door2)
...
function Lua_Az130_103947()									--Raid帕依察
	local Boss=OwnerID()
...
--		if mode==1 then
			...
			local EveryOne=DW_HateRemain(0)
				for l=1,table.getn(EveryOne),1 do
					SysCastSpellLv(Boss,EveryOne[l],496134,50)
				end
				--say(boss,"die")
...
--			say(boss,"amt="..Amt)
				...
				for i=1,Amt do
					...
					jell[i]=Lua_DW_CreateObj("obj",103953,BadLuck[i],0,1)
					...
					BeginPlot(jell[i],"Az_JellTimer_130",0)
					SetPlot(jell[i],"dead","Az_JellDead_130",0)
...
--				Say(boss,"Small")
			elseif SkCount%7==6 then
				BeginPlot(boss,"Lua_Az_LightingCould_130",0)
				--say(boss,"Lighting")
...
function Lua_Az_ThunderCheck_130()
	if CheckBuff(TargetID(),507369)==true then
		...
		if BuffPower==9 then
			SysCastSpellLv(TargetID(),TargetID(),496007,54)
...
function Lua_Az_DelayCheck_130()
...
function Lua_Az_LightingCould_130()
	local Boss=OwnerID()
	local RoomID=ReadRoleValue(Boss,EM_RoleValue_RoomID)
	local PickOne=Lua_RandomPlayerFromHateList(Boss,1)
	local obj1=Lua_DW_CreateObj("obj",103953,PickOne[1],0,1)
	...
	sleep(10)
	local obj2=Lua_DW_CreateObj("xyz",103953,XYZ,0,0,1)
	...
	local LazyBone=table.getn(Lazy)
		if LazyBone~=0 then
		for i=0 ,LazyBone do
			SysCastSpellLv(Boss,Lazy[i],496134,14)
...
function Lua_Az130_103947_Dead()
	--say(OwnerID(),"111")
	--say(TargetID(),"222")
