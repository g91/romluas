function Lua_Az132_104182_Ctrl()							--Raid帕依察控制器
	local Ctrl=OwnerID()
	local CombatBegin=0										--開戰旗標
	local RoomID=ReadRoleValue(Ctrl,EM_RoleValue_RoomID)
	local Boss=CreateObjByFlag(104182,780713,0,1)
	...
	SetPlot(Boss,"dead","Lua_Az132_104182_Dead",0)
	while true do
		sleep(10)
		...
		if hatelistcount(boss)~=0 then
			if Combatbegin==0 then
				...
				BeginPlot(Boss,"Lua_Az132_104182",0)
		...
		elseif hatelistcount(boss)==0 then
			if Combatbegin==1 then
				if ReadRoleValue(boss,EM_RoleValue_IsDead)==0 then
					...
					DelObj(door1)
					sleep(50)
					Boss=CreateObjByFlag(104182,780713,0,1)
					AddToPartition(Boss,RoomID)
					WriteRoleValue(Ctrl,EM_RoleValue_PID,Boss)
					WriteRoleValue(Boss,EM_RoleValue_PID,Ctrl)
					WriteRoleValue(Boss,EM_RoleValue_register5,door2)
...
function Lua_Az132_104182()									--Raid帕依察
	local Boss=OwnerID()
...
--					SysCastSpellLv(Boss,EveryOne[l],496138,50)
--				end
				--say(boss,"die")
...
--				ScriptMessage(boss,-1,2,"[SC_104141_09]",2)
			elseif SkCount%23==0 then												--水牢
				...
				if HateListCount(boss)>=2 then
					NoTank=DW_HateRemain(1)										
				elseif HateListCount(Boss)==1 then
					NoTank=DW_HateRemain(0)
				end
...
--			say(boss,"amt="..Amt)
				local jell={}
				for i=1,Amt do
					CastSpellLv(Boss,BadLuck[i],495958,4)
					AddBuff(BadLuck[i],507368,0,14)
					AddBuff(BadLuck[i],507369,0,-1)
					jell[i]=Lua_DW_CreateObj("obj",104341,BadLuck[i],0,1)
					...
					BeginPlot(jell[i],"Az_JellTimer_132",0)
					SetPlot(jell[i],"dead","Az_JellDead_132",0)
...
--					say(boss,"BadLuck="..BadLuck[i])
			...
			elseif SkCount%19==0 then
				...
				for k=1,Amt do
					...
					ppl=table.getn(Allppl)
					SysCastSpellLv(Boss,LuckyGuy[k],Sk[3],SkLv[3])
					--say(LuckyGuy[k],"biteme")
				end
--				PickOne=Lua_RandomPlayerFromHateList(Boss,1)
				ScriptMessage(boss,-1,2,SkStr[3],2)
--				Say(boss,"Small")
			elseif SkCount%7==6 then
				BeginPlot(boss,"Lua_Az_LightingCould_132",0)
				--say(boss,"Lighting")
...
function Az_JellTimer_132()
	local JellTime=0
	while true do
		sleep(10)
		JellTime=JellTime+1
--		say(OwnerID(),"Time="..JellTime)
...
function Lua_Az_ThunderCheck_132()
	if CheckBuff(TargetID(),507369)==true then
		...
		if BuffPower==9 then
			SysCastSpellLv(TargetID(),TargetID(),496007,44)
			--Say(TargetID(),"BuffLv=10")
		elseif BuffPower==8 then 
			SysCastSpellLv(TargetID(),TargetID(),496007,39)
			--Say(TargetID(),"BuffLv=9")
		elseif BuffPower==7 then 
			SysCastSpellLv(TargetID(),TargetID(),496007,34)
		...
		elseif BuffPower==5 then 
			SysCastSpellLv(TargetID(),TargetID(),496007,24)
			--Say(TargetID(),"BuffLv=6")
		elseif BuffPower==4 then 
			SysCastSpellLv(TargetID(),TargetID(),496007,19)
			--Say(TargetID(),"BuffLv=5")
		elseif BuffPower==3 then 
			SysCastSpellLv(TargetID(),TargetID(),496007,14)
		...
		elseif BuffPower==1 then 
			SysCastSpellLv(TargetID(),TargetID(),496007,4)
			--Say(TargetID(),"BuffLv=2")
		elseif BuffPower==0 then 
			SysCastSpellLv(TargetID(),TargetID(),496007,0)
			--Say(TargetID(),"BuffLv=1")
		end
	end
end

function Lua_Az_DelayCheck_132()
...
function Lua_Az_LightingCould_132()
	local Boss=OwnerID()
	local RoomID=ReadRoleValue(Boss,EM_RoleValue_RoomID)
	local PickOne=Lua_RandomPlayerFromHateList(Boss,1)
	local obj1=Lua_DW_CreateObj("obj",104341,PickOne[1],0,1)
	...
	sleep(10)
	local obj2=Lua_DW_CreateObj("xyz",104341,XYZ,0,0,1)
	...
	local LazyBone=table.getn(Lazy)
		if LazyBone~=0 then
		for i=0 ,LazyBone do
			SysCastSpellLv(Boss,Lazy[i],496138,4)
...
function Lua_Az132_104182_Dead()
	--say(OwnerID(),"111")
	--say(TargetID(),"222")
