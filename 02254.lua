function ic_105267_01()  																	---第10口袋固定死亡劇情用				[$VAR1]			 SC_105267_00												
	...
	local Boss=CreateObjByFlag(105267,780870,1,1) 											---BOSS創在  旗標1
	...
	SetModeEx( Boss , EM_SetModeType_Fight, false) 						---可砍殺
		...
		door1=CreateObjByFlag(105586,780870,3,1)									---王身後阻擋門
			...
			SetPlot( Boss,"dead","ic_105267_BOSSXX",0 )										---掛死亡劇情
	...
	while true do
		...
		if mobplay==1 and mob5==0 then
			BeginPlot(mob1 , "ic_105267_mob" , 0)
			BeginPlot(mob2 , "ic_105267_mob" , 0)
			BeginPlot(mob3 , "ic_105267_mob" , 0)
			BeginPlot(mob4 , "ic_105267_mob" , 0)
			--say(OwnerID(),"playmob")
			sleep(5)
		...
		elseif mobplay==2 and mob5==1 then	
			...
			if hatelistcount(boss)~=0 then    												---讀仇恨人數
				...
				Boss100=Lua_DW_CreateObj("flag",104693,780870,1,0)							---測試距離用
						...
						BeginPlot(Boss100 , "ic_105267_100" , 0)---測試距離			
					...
					BeginPlot(boss , "ic_105267_02" , 0)									---命令角色做事情
				end
			end	
			if hatelistcount(boss)==0 and open==1 or BOSS9898==1 and open==1 then			---開戰後仇恨表沒人
				if ReadRoleValue( boss , EM_RoleValue_IsDead)==0 then   					---王沒死 玩家滅團
					...
					sleep(20)                                          
					Boss=CreateObjByFlag(105267,780870,1,1)           						---再創造1之				
					...
					BeginPlot(isaac, "ic_105267_BUFFXX" , 0)								---刪除BUFF
...
function ic_105267_02()   																	 ---王AI(第10固定狂暴用)
	local isaac=ReadRoleValue(OwnerID(),EM_RoleValue_PID)
	local Boss=OwnerID()
		...
		while 1 do
			...
			if time0==1 then																  ---第2階段
				if time6==0 then
					if hatelistcount(boss)>=2 then	
						CastSpellLV( OwnerID(), OwnerID(), 497056, 4 )						  ---全體範圍技能
						sleep(20)
					...
					if hatelistcount(boss)==1 then											  ---1刷死(1次)
						if play1==0 or play2==0 or play3==0 or play4==0 then
							...
							sleep(10)		  
							sysCastSpellLV( BossTarget, boss, 497052, 0 )						---嘲諷		  
							sleep(10)
							SetAttack(Boss , BossTarget)										---叫BOSS打玩家
							sleep(10)
							CastSpellLV( OwnerID(), BossTarget, 497053, 5)				---衝鋒
							time0=0	
							time1=0
							play1=0
							play2=0	
					...
					if  boss9898==0 then
						...
						if play1==1 then
							...
							if play0==0 then
								...
								sleep(10)
								sysCastSpellLV( plya9898, boss, 497052, 0 )						---嘲諷
								SetAttack(Boss , plya9898)										---叫BOSS打玩家
								sleep(30)
								SetAttack(Boss , plya9898)										---叫BOSS打玩家
								sysCastSpellLV( plya9898, boss, 497052, 0 )					  	---嘲諷	
								SetAttack(Boss , plya9898)										---叫BOSS打玩家
								if CheckBuff( boss, 508780 ) == true and play0==0 then			---確認	
					...
					elseif	boss9898==1 then
						...
						if play2==1 then
							if play0==1 then
								...
								sysCastSpellLV( plya9898, boss, 497052, 0 )						---嘲諷	
								SetAttack(Boss , plya9898)										---叫BOSS打玩家	
								if CheckBuff( boss, 508780 ) == true and play0==1 then			---確認	
									AddBuff(Boss,508864,0,-1 )
									---WriteRoleValue(isaac,EM_RoleValue_register9,1)
									...
									local MessageString = "[SC_105267_03][$SETVAR1="..PlayerName .."]" 		----在string中可以秀出玩家的名字
										  ScriptMessage( OwnerID() , -1 , 2 , MessageString , 1 )
									sleep(20)
									CastSpellLV( OwnerID(), plya9898, 497053, 5 )				---衝鋒
									play0=2
									time0=0
								end	
							end		
					...
					elseif	boss9898==2 then
						...
						if play3==1 then
							if play0==2 then
								...
								sysCastSpellLV( plya9898, boss, 497052, 0 )						---嘲諷
								SetAttack(Boss , plya9898)										---叫BOSS打玩家
								if CheckBuff( Boss, 508780 ) == true and play0==2  then			---確認	
									AddBuff(Boss,508864,0,-1 )
									---WriteRoleValue(isaac,EM_RoleValue_register9,1)
									local PlayerName = GetName( plya9898)
									local MessageString = "[SC_105267_03][$SETVAR1="..PlayerName .."]" 		----在string中可以秀出玩家的名字
					...
					elseif	boss9898==3 then
						...
						if play4==1 then
							if play0==3 then
								...
								sysCastSpellLV(plya9898, boss, 497052, 0 )						---嘲諷
								SetAttack(Boss , plya9898)										---叫BOSS打玩家
								if CheckBuff( boss, 508780 ) == true and play0==3 then			---確認	
									AddBuff(Boss,508864,0,-1 )
									local PlayerName = GetName( plya9898)
									local MessageString = "[SC_105267_03][$SETVAR1="..PlayerName .."]" 		----在string中可以秀出玩家的名字
										  ScriptMessage( OwnerID() , -1 , 2 , MessageString , 1 )  
			...
			if time0==0  then
				...
				if time2>=19 then
					...
					CastSpellLV( OwnerID(), OwnerID(), 497057, 0)							---全體範圍技能
					sleep(20)
					time2=0
				end
				if time3>=13 then	
					if hatelistcount(boss)==1 then
						...
						local MessageString = "[SC_105267_02][$SETVAR1="..PlayerName.."]" 		----在string中可以秀出玩家的名字
							  ScriptMessage( OwnerID() , -1 , 2 , MessageString , 1 )
						CastSpellLV( OwnerID(), BossTarget, 497055,450)						---遠方範圍技能
						sleep(30)
						time3=0
					elseif hatelistcount(boss)>=2	then
						...
						local MessageString = "[SC_105267_02][$SETVAR1="..PlayerName .."]" 		----在string中可以秀出玩家的名字
							  ScriptMessage( OwnerID() , -1 , 2 , MessageString , 1 )
						CastSpellLV( OwnerID(), play1x[plays], 497055,450)					---遠方範圍技能
						sleep(30)
						time3=0
					end	
				end
