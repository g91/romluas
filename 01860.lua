...
--------------------------------------------------------------------------------啟動器
function Cang_127_edward_start_ez()
	local RoomID = ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)
	local Boss01 = CreateObjByFlag( 103866, 780586, 0, 1)	--招換	
	...
	BeginPlot( OwnerID(), "Cang_127_edward_time_ez" , 0)
	BeginPlot( OwnerID(), "Cang_127_edward_ez" , 20)
	SetPlot( Boss01,"dead", "Cang_127_edward_dead_ez" ,0 )	
	...
	while true do
		...
		if reset == 1 then
			...
			SetModeEx( Door  , EM_SetModeType_Obstruct, false )--阻擋
			DelObj( Door )			
			BeginPlot( start, "Cang_127_edward_restart_ez" , 0)
			sleep( 30 )
			DelObj( OwnerID())
		end	
	end
...
--------------------------------------------------------------------------------死亡劇情
function Cang_127_edward_dead_ez() --王死掉回傳	
	local RoomID = ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)
...
--------------------------------------------------------------------------------第二次之後的script
function Cang_127_edward_restart_ez()
	local RoomID = ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)
	local Boss01 = CreateObjByFlag( 103866, 780586, 0, 1)--招換	
	AddToPartition( Boss01 , RoomID )	--加進舞台	
	WriteRoleValue( OwnerID() ,EM_RoleValue_PID, Boss01 )--BOSS存入計時器 R1
...
--------------------------------------------------------------------------------狂暴機制
function Cang_127_edward_time_ez()--狂暴機制
	local Boss
	local Time = 0
	while true do
		sleep(10)
...
--------------第一階段 普通攻擊 產生盾
			if skillsp >= 15 and count == 0 then
				if count1 == 0 then
					BeginPlot( OwnerID(), "Cang_127_edward_weapon_ez" , 0)	--產生武器架
					count = 1
					count1 = 1
				end
			end	
--------------第二階段 特殊招				
			if skillsp >= 25 and count == 1 then	 
				if count2 == 0 then	--抓一個衰小玩家
					...
					CastSpellLV( Boss, sway[anyone] , 495357 , 2 )
					sleep(20)
...
--------------空中技能施放
			if count == 2 and bskill == 1 then
				...
				if table.getn(sway) ~= 0 then	
					...
					CastSpellLV( Boss, sway[anyone], 495684 , 1 ) 
				end	
				countskill = 0
			end	
		end
...
--------------------------------------------------------------------------------產生武器架
function Cang_127_edward_weapon_ez()
	...
	SetPlot( weapon01, "touch" , "Cang_127_edward_shield_ez" ,10 )
	...
	BeginPlot( weapon01, "Cang_127_edward_wpend_ez" ,10 )
	...
	SetPlot( weapon02, "touch" , "Cang_127_edward_shield_ez" ,10 )
	...
	BeginPlot( weapon02, "Cang_127_edward_wpend_ez" ,10 )		
	...
	SetPlot( weapon03, "touch" , "Cang_127_edward_shield_ez" ,10 )
	...
	BeginPlot( weapon03, "Cang_127_edward_wpend_ez" ,10 )		
	...
	SetPlot( weapon04, "touch" , "Cang_127_edward_shield_ez" ,10 )
	...
	BeginPlot( weapon04, "Cang_127_edward_wpend_ez" ,10 )	
	--end
end

--------------------------------------------------------------------------------武器架給玩家buff
function Cang_127_edward_shield_ez()		
...
--------------------------------------------------------------------------------技能檢查用
function Cang_127_edward_shieldcancel_ez()
	CancelBuff( TargetID() , 506327 )
end

--------------------------------------------------------------------------------幽冥之霧產生
function Cang_127_edward_summom_ez()
	local RoomID = ReadRoleValue(OwnerID(),EM_RoleValue_RoomID)
	...
	BeginPlot( SpellCaster , "Cang_127_edward_summomskill_ez" , 0)
end

function Cang_127_edward_summomskill_ez()
	AddBuff( OwnerID() , 506755 , 0 , -1 )
	sleep(50)
	CancelBuff( OwnerID() , 506755 )
	AddBuff( OwnerID() , 506756 , 0 , -1 )
end

--------------------------------------------------------------------------------武器架額外刪除設定
function Cang_127_edward_wpend_ez()
