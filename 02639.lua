-----通用野外怪物隨機強化效果通用函式(10%機率修正版)------------------------
function Cl_ZoneMonster_Bico_RandBuff() ---將此函示掛載物件產生劇情上
	...
	for i = 1 , table.getn( Buff_Table ) , 1 do --檢查TABLE內的所有編號，是否身上有存在任何一個
		if CheckBuff ( OwnerID , Buff_Table[i] ) == true then
			SetPlot( OwnerID , "dead" , "Cl_ZoneMonster_Bico_dead" , 0 ) ---當身上有TABLE內任一個BUFF時，給予怪物死亡劇情
			break
		end
	end
end
function Cl_RandBuff_Bico_For_ObjID(ZoneID) ---存放區域所需要產生的神祕袋子，每個袋子各有不同掉寶表，當開啟新的任務區域，則在底下新增區域編號
	local Obj_Table = {} --這裡為陣列
	Obj_Table[940] = 107450 ---940=測試區
	Obj_Table[948] = 107451 ---948=測試區
	Obj_Table[934] = 107451 ---934=測試區
	Obj_Table[24] = 107442	--Z24
	Obj_Table[25] = 107444	--Z25
	Obj_Table[26] = 107445	--Z26
	Obj_Table[27] = 107446	--Z27
	Obj_Table[28] = 107447	--Z28
	Obj_Table[29] = 107448	--Z29
	Obj_Table[30] = 107449	--Z30
	Obj_Table[32] = 107450	--Z32
--	Obj_Table[33] = 107451	--Z33
--	Obj_Table[34] = 109143	--Z34	
	return Obj_Table[ZoneID]
end
function Cl_ZoneMonster_Bico_dead()---怪物死亡劇情掉落物件袋子
	local OwnerID = OwnerID()
	...
	local Obj_Table = Cl_RandBuff_Bico_For_ObjID(ZoneID) ---取出怪物所在區域，專屬的神祕袋子ID
	local hateID = 0
	local Luck = 0
	for i = 0 , hate_Num-1 , 1 do
		...
		if ReadRoleValue( hateID , EM_RoleValue_IsPlayer ) == 1 then ---檢查物件順利是否為玩家，避免掉NPC所殺死的怪物也會掉落袋子
			Luck = DW_Rand(100)
			if Luck >= 70 and Luck <= 80 then ---當達成機率到達，則產生神秘袋子
...
function Cl_ZoneMonster_Bico_TreasureBox()  --神秘袋子產生劇情
	local OwnerID = OwnerID()
	local Right_Time = 0
	SetPlot( OwnerID ,"touch", "Cl_ZoneMonster_Bico_TreasureBox_true" ,40 )  --給予神秘袋子觸碰劇情，讓玩家撿取內容物
...
function Cl_ZoneMonster_Bico_TreasureBox_true() ---玩家的撿取劇情
	local OwnerID = OwnerID()
